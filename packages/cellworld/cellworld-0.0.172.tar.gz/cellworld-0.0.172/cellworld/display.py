...
class Display:
    ...
    def __init__(self,
        ...
        self.trajectory_zorder = trajectory_zorder
        self._draw_cells__()
        plt.tight_layout()
    ...
    def _draw_cells__(self):
        ...
        for cell in self.world.cells:
            ...
            self.cell_outline_polygons.append(self.ax.add_patch(RegularPolygon((cell.location.x, cell.location.y), self.world.configuration.cell_shape.sides, self.cells_size, facecolor=color, edgecolor=edge_color, orientation=self.cells_theta, zorder=self.cell_outline_zorder, linewidth=1, alpha=self.cell_outline_alpha, fill=self.cell_fill)))
            self.cell_polygons.append(self.ax.add_patch(RegularPolygon((cell.location.x, cell.location.y), self.world.configuration.cell_shape.sides, self.cells_size * self.outline, facecolor=color, orientation=self.cells_theta, zorder=self.cell_zorder, linewidth=0, alpha=self.cell_alpha, fill=self.cell_fill)))
        self.habitat_polygon = self.ax.add_patch(RegularPolygon((self.xcenter, self.ycenter), self.world.implementation.space.shape.sides, self.habitat_size, facecolor=self.habitat_color, edgecolor=self.habitat_edge_color, orientation=self.habitat_theta, zorder=self.habitat_zorder, alpha=self.habitat_alpha))
        ...
        for index, agent in enumerate(agents):
            ...
            for i in range(len(x)-1):
                ...
                if type(color) is numpy.ndarray:
                    ...
                    lcolor = color
                self.ax.plot([x[i], x[i+1]], [y[i], y[i+1]], color=lcolor, alpha=lalpha, linewidth=3, zorder=self.trajectory_zorder)
