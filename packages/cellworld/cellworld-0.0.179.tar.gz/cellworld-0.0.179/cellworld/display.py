...
class Display:
    ...
    def add_trajectories(self, trajectories: Trajectories, colors={}, alphas={}, start_frame=0, end_frame=None, start_time=0, end_time=None, zorder=4, auto_alpha=False):
        ...
        for index, agent in enumerate(agents):
            ...
            for step in agent_trajectory:
                if step.frame < start_frame or (end_frame and step.frame > end_frame):
                    continue
                if step.time_stamp < start_time or (step.end_time and step.time_stamp > end_time):
                    continue
            ...
            if agent in alphas:
                alpha = alphas[agent]
            if auto_alpha:
                alpha = [1/len(x) * v for v in range(len(x))]
...
def EpisodeRepresentation(episode: Episode, video_path: str, frames: list, world: World, cols=0, **kwargs):
    import cv2
    ...
    for i, frame in enumerate(frames):
        ax = axs[i]
        d = Display(world, cell_fill=False, background_color="black", habitat_fill=False, show_cell=False, show_occluded_cell=False, ax=ax)
        d.background_extent = [0, 1, .05, .95]
        d.set_background(cropped_frames[frame], 1)
        d.add_trajectories(episode.trajectories, colors={"prey": "blue", "predator": "red"}, alphas={"prey": .4, "predator": .4}, start_frame=pf, end_frame=f)
        pf = frame
