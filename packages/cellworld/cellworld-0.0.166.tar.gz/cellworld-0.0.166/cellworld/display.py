...
class Display:
        ...
        if video_file:
            ...
            while video_reader.isOpened():
                ...
                if video_frame is None:
                    break
                self.video_frames.append(video_frame[-980:, :, :])
            self.set_video_frame(250)
            self.video_extent = [self.xcenter - hsize, self.xcenter + hsize, self.ycenter - hsize, self.ycenter + hsize]
        self._draw_cells__()
        plt.tight_layout()
    ...
    def set_video_frame(self, frame_number: int):
        self.ax.imshow(self.video_frames[frame_number], extent=self.video_extent)
        self._draw_cells__()
