...
class Display:
    ...
    def __init__(self,
        ...
        self.cell_outline_zorder = 2
        self.cell_zorder = 3
        self.habitat_zorder = 1
        self.trajectory_zorder = 4
        self._draw_cells__()
        plt.tight_layout()
    ...
    def _draw_cells__(self):
        ...
        for cell in self.world.cells:
            ...
            self.cell_outline_polygons.append(self.ax.add_patch(RegularPolygon((cell.location.x, cell.location.y), self.world.configuration.cell_shape.sides, self.cells_size, facecolor=color, edgecolor=edge_color, orientation=self.cells_theta, zorder=self.cell_outline_zorder, linewidth=1)))
            self.cell_polygons.append(self.ax.add_patch(RegularPolygon((cell.location.x, cell.location.y), self.world.configuration.cell_shape.sides, self.cells_size * self.outline, facecolor=color, orientation=self.cells_theta, zorder=self.cell_zorder, linewidth=1)))
        self.habitat_polygon = self.ax.add_patch(RegularPolygon((self.xcenter, self.ycenter), self.world.implementation.space.shape.sides, self.habitat_size, facecolor=self.habitat_color, edgecolor=self.habitat_edge_color, orientation=self.habitat_theta, zorder=self.habitat_zorder))
