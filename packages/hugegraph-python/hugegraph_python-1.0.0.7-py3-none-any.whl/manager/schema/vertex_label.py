...
class VertexLabel(HugeGraphBase):
    ...
    def create(self):
        ...
        if res.status_code == 201:
            return 'create VertexLabel success, Detail: "{}"'.format(str(res.content))
        else:
    ...
    def append(self):
        ...
        if res.status_code == 200:
            return 'append VertexLabel success, Detail: "{}"'.format(str(res.content))
        else:
    ...
    def remove(self):
        ...
        if res.status_code == 202:
            return 'remove VertexLabel success, Detail: "{}"'.format(str(res.content))
        else:
    ...
    def eliminate(self):
        ...
        res = requests.put(url, data=json.dumps(data), auth=self._auth, headers=self._headers)
        if res.status_code == 200:
            return 'eliminate VertexLabel success, Detail: "{}"'.format(str(res.content))
        else:
