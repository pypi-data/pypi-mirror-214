...
class BaseSchema(Schema):
    ...
    def _remove_skip_values(cls, obj):
        if isinstance(obj, list):
            ...
            return {
                k: cls._remove_skip_values(v)
                for k, v in obj.items()
                if v not in cls.SKIP_VALUES
            }
        return obj
...
class LinkModel:
    source: List[Any] = field(
        default_factory=list
    )  # indices correspond to NodeModel.label
...
class ChartDataModel:
    ...
    measure: List[str] = field(
        default_factory=list,
        metadata=dict(validate=validate.ContainsOnly(Measures.to_list())),
    )
    ...
    marker: Optional[MarkerModel] = field(
        default=None, metadata=dict(plotly_exclude=True)
    )
    ...
    textposition: Optional[str] = field(
        default=None, metadata=dict(validate=validate.OneOf(Textpositions.to_list()))
    )
    base: Optional[int] = field(default=None)
    showlegend: Optional[bool] = field(default=None)
    type: Optional[str] = field(
        default=None
    )  # only for use of backwards plotly regeneration
    headers: Optional[List[str]] = field(default_factory=list)
    is_header_row: Optional[bool] = field(
        default=None, metadata=dict(data_key="isHeaderRow")
    )
    format: Optional[TableFormatModel] = field(default=None)
    context: Optional[str] = field(default=None)
    stratification: DictAny = field(
        default_factory=dict, metadata=dict(plotly_exclude=True)
    )
...
class ChartModel:
    type: str = field(
        init=True, metadata=dict(required=True)
    )  # Define the plotly chart type, such as bar, pie, etc.
    id: str = field(default="chart1")  # Optional?
    data: List[Union[ChartDataModel, DictAny]] = field(
        default_factory=list, repr=False, metadata=dict(validate=validate.Length(min=1))
    )
    mode: Optional[str] = field(default=None)
...
class AnnotationModel:
    ...
    xref: str = field(
        default="paper"
    )  # ( "paper" | "/^x([2-9]|[1-9][0-9]+)?( domain)?$/" )
    yref: str = field(
        default="paper"
    )  # ( "paper" | "/^y([2-9]|[1-9][0-9]+)?( domain)?$/" )
...
class LayoutModel:
    ...
    titlefont: Optional[DictAny] = field(
        default_factory=lambda: dict(color=ColorsNormalExt.BLUE, size=20)
    )
    ...
    is_animated: Optional[bool] = field(
        default=None, metadata=dict(data_key="isAnimated")
    )  # custom VRS props
    use_custom_tooltip: Optional[bool] = field(
        default=None, metadata=dict(data_key="useCustomTooltip")
    )  # custom VRS props
...
class ReportModel:
    ...
    layout: Union[LayoutModel, DictAny] = field(
        default_factory=dict, metadata=dict(required=False)
    )
    charts: List[ChartModel] = field(
        default_factory=list, metadata=dict(validate=validate.Length(min=1))
    )
    strata_label: str = field(
        default=None, metadata=dict(data_key="stratificationLabel")
    )
