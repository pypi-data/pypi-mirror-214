...
class SocketEventBreeze(socketio.ClientNamespace):
    ...
    def on_message(self, data):
        ...
        #print(self.breeze)
        if(self.breeze.ticks!=None):
            self.breeze.ticks(data)
        if(self.breeze.ticks2!=None):
            self.breeze.ticks2(data)
        ...
        def wrapper(link, timeout = 3):
            ...
            if cache_key in response_cache:
                ...
                if time.time() - cached_response["timestamp"] <= CACHE_EXPIRATION:
                    ...
                    response._content = base64.b64decode(cached_response_data["content"])
                    #response.url = link
                    return response
            ...
            try:
                ...
                print(f"The request timed out for: {link} as the server failed to respond")
