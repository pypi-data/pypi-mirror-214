...
import sqlalchemy
from sqlalchemy import Column, String, Integer, Float, Enum, Identity, ForeignKey, UniqueConstraint
...
class Plant(Component):
    ...
    def ignored_ranges(self) -> List[pd.Interval]:
        ...
        for event in self.operational_events:
            if event.ignored_range:
                intervals.append(pd.Interval(pd.to_datetime(event.event_start), pd.to_datetime(event.event_end), closed='both'))
    ...
    def _validate_raw_sensors(self, _, val, is_remove):
        ...
        if is_remove:
            val.mappings.clear()  # Removing a sensor from the plant clears all mappings
    ...
    def use_csv(self,
                ...
                calculate_virtuals: bool = True,
                datetime_template: Union[DatetimeTemplates, str] = None,
        ...
        """
        up = sunpeek.common.data_uploader.DataUploader_df(plant=self,
...
class Array(Component):
    ...
    plant = relationship("Plant", foreign_keys=[plant_id], backref=backref("arrays", cascade="all, delete-orphan"))
    name = Column(String)
...
class HeatExchanger(Component):
    ...
    plant_id = Column(Integer, ForeignKey('plant.id', ondelete="CASCADE"))
    plant = relationship("Plant", foreign_keys=[plant_id], backref=backref("heat_exchangers", cascade="all, delete-orphan"))
