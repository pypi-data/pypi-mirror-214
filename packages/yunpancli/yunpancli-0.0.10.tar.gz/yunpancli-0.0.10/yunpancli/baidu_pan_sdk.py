...
class BaiduPanSDK:
    ...
    # 分片上传
    def upload(self, remote_path, uploadid, block_list, file_path, access_token):
        block_need_upload = set(block_list)
        ...
        with open(file_path, "rb") as f:
            ...
            for seq in track(range(buffer_cnt), description="上传中..."):
                if seq not in block_need_upload:
                    continue
    ...
    def upload_file(self, local_file_path, remote_file_path, access_token):
        ...
        ret = self.upload(remote_file_path, uploadid, block_list, local_file_path, access_token)
        if not ret:
            return False
        # 3. 创建文件
