...
from tinybird.redis_client import TBRedisConfig, get_redis_test_client, get_redis_config
...
class TestJobExecutor(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        redis_config: TBRedisConfig = get_redis_config()
        ...
        self.job_executor_producer = JobExecutor(redis_client=redis_client, redis_config=redis_config, consumer=False, import_workers=0, query_workers=0, export_workers=0, export_sink_workers=0, branching_workers=0)
        self.job_executor_consumer = JobExecutor(redis_client=redis_client, redis_config=redis_config, consumer=True, import_workers=1, query_workers=1, export_workers=1, export_sink_workers=1, branching_workers=1)
    ...
    async def check_discover_new_queue(self, kind):
        ...
        if kind == JobKind.IMPORT:
            ...
            self.assertEqual(len(self.job_executor_consumer._export_sink_threadpool_executors), 0)
            ...
            self.assertEqual(len(self.job_executor_consumer._export_threadpool_executors), 0)
            self.assertEqual(len(self.job_executor_consumer._export_sink_threadpool_executors), 0)
        elif kind == JobKind.EXPORT_SINK:
            self.assertTrue(self.user.database_server in self.job_executor_producer._export_sink_threadpool_executors)
            self.assertEqual(self.job_executor_producer._export_sink_threadpool_executors[self.user.database_server]._thread_pool_executor, None)
            self.assertEqual(len(self.job_executor_consumer._query_threadpool_executors), 0)
            self.assertEqual(len(self.job_executor_consumer._import_threadpool_executors), 0)
            self.assertEqual(len(self.job_executor_consumer._export_threadpool_executors), 0)
            self.assertEqual(len(self.job_executor_consumer._branching_threadpool_executors), 0)
            await self.check_threadpool_exists(self.job_executor_consumer._export_sink_threadpool_executors)
            self.assertEqual(len(self.job_executor_consumer._query_threadpool_executors), 0)
            self.assertEqual(len(self.job_executor_consumer._import_threadpool_executors), 0)
            self.assertEqual(len(self.job_executor_consumer._export_threadpool_executors), 0)
            self.assertEqual(len(self.job_executor_consumer._branching_threadpool_executors), 0)
    ...
    async def test_discover_new_export_sink_queue(self):
        await self.check_discover_new_queue(JobKind.EXPORT_SINK)
    ...
    async def test_export_sink_workers_shutdown(self):
        await self.check_discover_new_queue(JobKind.EXPORT_SINK)
        await self.check_threadpool_executors_is_empty(self.job_executor_consumer._export_sink_threadpool_executors)
    ...
    async def test_discover_new_import_query_and_export_queue(self):
        ...
        job5 = FakeTestJob(JobKind.EXPORT_SINK, self.user)
        ...
        self.job_executor_producer.put_job(job5)
        self.assertTrue(self.user.database_server in self.job_executor_producer._import_threadpool_executors)
        self.assertTrue(self.user.database_server in self.job_executor_producer._query_threadpool_executors)
        self.assertTrue(self.user.database_server in self.job_executor_producer._export_threadpool_executors)
        self.assertTrue(self.user.database_server in self.job_executor_producer._branching_threadpool_executors)
    ...
    async def test_export_sink_workers_shutdown_after_job_finishes(self):
        job = FakeTestJob(JobKind.EXPORT_SINK, self.user, JobExecutor.ALIVE_QUEUES_TTL_IN_SECONDS + PYTHON_THREAD_TOLERANCE_IN_SECONDS)
        self.job_executor_producer.put_job(job)
        await self.check_threadpool_exists(self.job_executor_consumer._export_sink_threadpool_executors)
        await asyncio.sleep(JobExecutor.ALIVE_QUEUES_TTL_IN_SECONDS)
        self.assertTrue(self.user.database_server in self.job_executor_consumer._export_sink_threadpool_executors)
        await self.check_threadpool_executors_is_empty(self.job_executor_consumer._export_sink_threadpool_executors)
