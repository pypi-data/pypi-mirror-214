...
class Blackwood(Hand):
    ...
    def __init__(self, hand_cards, board):
        ...
        if '4NT' not in self.bid_history:
            ...
            for responder_call in calls[self.base_index+2::4]:
                if responder_call.name == '4NT':
                    if (self.opener_bid_one.is_major and
                        ...
                        self.check_agreed_suit(self.opener_bid_one.denomination, 'opener bid one')
                        return self.opener_bid_one.denomination
                    self.check_agreed_suit(Call(self.bid_history[responder_index-2]).denomination, 'responder_index-2')
                    return Call(self.bid_history[responder_index-2]).denomination
                if opener_call.name == '4NT':
                    self.check_agreed_suit(Call(self.bid_history[opener_index-2]).denomination, 'opener index -2')
                ...
                responder_index += 4
            opener_index += 4
        self.check_agreed_suit(Call(self.bid_history[self.base_index]).denomination, 'bid_history[self.base_index])')
        return Call(self.bid_history[self.base_index]).denomination

    def check_agreed_suit(self, agreed_suit, suit_type):
        if not agreed_suit:
            print(f'Suit failure in Blackwwod: {suit_type}')
