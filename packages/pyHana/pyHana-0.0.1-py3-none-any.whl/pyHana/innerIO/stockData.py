...
def SaveStockTrade(trcode, shCode, newData):            
...
# 중복 row가 있는 경우 기존 데이터는 제외 처리    
    ...
    stockTradeFileNm = conf.stockTradePath + "/" + trcode + partNm + ".pkl"
    
    # 기존 데이터 Read
    currData = dataProc.ReadPickleFile(stockTradeFileNm)                 
    ...
    currData[shCode] = dataProc.MergeData(currData[shCode], newData) 
        
    dataProc.WritePickleFile(stockTradeFileNm, currData) 
...
def ReadStockItemInfo(shCode=''):   
    """
    종목명 return
    """
    filePathNm = conf.stockTradePath + "/stockitem_info.pkl"
    ...
    if shCode =='':
        ...
        if currData['data'].get(shCode) and currData['data'][shCode].get('hName'):
            retVal = [[shCode, currData['data'][shCode].get('hName')]]            
...
def ReadOverseasIndexInfo(symbol):       
    filePathNm = conf.stockTradePath + "/OverseasIndex/" + symbol.replace('@','') + ".pkl"
    currData = dataProc.ReadPickleFile(filePathNm)

    return currData    

def SaveOverseasIndexInfo(trcode, symbol, hname, nation, kind, newData):            
# data 분석 시 증권사 IF를 최소화 하기 위해, 증권사에서 조회한 데이터를 파일로 저장 
...
# 중복 row가 있는 경우 기존 데이터는 제외 처리    
    ...
    # 기존 데이터의 우선순위는 2, 신규 데이터는 우선순위 1로 병합 sort후 dup 제거
    currData['data'] = dataProc.MergeData(currData['data'], newData) 
