...
class Sync(threading.Thread):
    def __init__(self, cmd, pool, *vargs, **kwargs):
        ...
        self.killed = False
        self.params = {'v': '1.4.3'}
        attr = getattr(datetime.datetime, cmd.sync_from)
        t = attr() if callable(attr) else attr
        self.last_sync = t.replace(tzinfo=datetime.timezone.utc)
    ...
    def sync(self):
        ...
        for study in results:
            ...
            if creation > self.last_sync:
                sync = SyncWorker(self, study)
                self.pool.apply_async(sync.sync_study,
...
class SyncWorker:
    def __init__(self, worker: Sync, study):
