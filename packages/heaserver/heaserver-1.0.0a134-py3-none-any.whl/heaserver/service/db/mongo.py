...
import logging
import configparser
from typing import AsyncGenerator, Literal, Optional, List, Dict, Any, IO, Generator
...
class Mongo(Database):
    ...
    async def get_all(self, request: web.Request, collection: str, var_parts=None, mongoattributes=None,
                      sub: Optional[str] = None, sort: Literal[-1, 1] | None = None) -> AsyncGenerator[root.DesktopObjectDict, None]:
        """
        ...
        async for result in (coll.find(q) if sort is None else coll.find(q).sort([('modified', sort), ('created', sort)])):
            result['id'] = str(result['_id'])
            del result['_id']
