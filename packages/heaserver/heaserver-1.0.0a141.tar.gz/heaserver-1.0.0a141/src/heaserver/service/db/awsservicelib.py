...
async def create_object(request: web.Request, type_: S3Object | None = None, activity_cb: Optional[
    ...
    if activity_cb:
        ...
        activity.description = f'Creating {_activity_object_display_name(bucket_id, folder_or_file_key)}'
...
async def copy_object(request: Request,
    ...
    if activity_cb:
        activity = DesktopObjectAction()
        activity.code = 'hea-duplicate'
        activity.owner = request.headers.get(SUB)
        activity.user_id = activity.owner
        activity.description = f'Copying {_activity_object_display_name(source_bucket_name, source_key_name)} to {_activity_object_display_name(target_bucket_name, target_folder_name)}'
        await activity_cb(request.app, activity)
...
async def archive_object(request: Request,
    ...
    try:
        ...
        if activity_cb:
            activity = DesktopObjectAction()
            activity.code = 'hea-archive'
            activity.owner = request.headers.get(SUB, NONE_USER)
            activity.user_id = activity.owner
            activity.description = f'Archiving {_activity_object_display_name(source_bucket_name, source_key)} to {storage_class}'
            await activity_cb(request.app, activity)
    ...
    if activity_cb:
        activity.status = Status.IN_PROGRESS
        await activity_cb(request.app, activity)
    response_ = await _archive_object(s3_client, source_bucket_name, source_key, storage_class)
    if response_.status == 201:
        if activity_cb:
            activity.status = Status.COMPLETED
            await activity_cb(request.app, activity)
...
async def unarchive_object(request: Request,
    ...
    if activity_cb:
        activity = DesktopObjectAction()
        activity.code = 'hea-unarchive'
        activity.owner = request.headers.get(SUB)
        activity.user_id = activity.owner
        activity.description = f'Unarchiving {_activity_object_display_name(bucket_name, source_key_name)}'
        await activity_cb(request.app, activity)
...
async def delete_object(request: Request, recursive=False,
    ...
    if activity_cb:
        ...
        activity.owner = request.headers.get(SUB)
        activity.user_id = activity.owner
        activity.description = f'Deleting {_activity_object_display_name(bucket_name, key)}'
        await activity_cb(request.app, activity)
    s3_client = await request.app[HEA_DB].get_client(request, 's3', volume_id)
