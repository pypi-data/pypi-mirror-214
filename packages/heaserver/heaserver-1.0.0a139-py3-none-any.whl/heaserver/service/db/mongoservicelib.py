...
from typing import AsyncGenerator, Literal, Type, IO, Optional
from heaobject.root import DesktopObject, HEAObjectDict
...
async def get_all(request: Request, collection: str, volume_id: Optional[str] = None) -> Response:
    ...
    """
    l = []
    async for desktop_object in get_all_gen(request, collection, volume_id):
...
async def get_all_gen(request: Request,
                      ...
                      sort: dict[str, Literal[-1, 1]] | None = None) -> AsyncGenerator[HEAObjectDict, None]:
    sub = request.headers.get(SUB)
    mongo = await _get_mongo(request, volume_id)
    async for r in mongo.get_all(request, collection, sub=sub, sort=sort):
