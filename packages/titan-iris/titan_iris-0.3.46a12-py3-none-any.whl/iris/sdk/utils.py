                ...
                if "experiments" in result:
                    ...
                    for experiment in experiments:
                        if "jobs" in experiment:
                            ...
                            for i in range(len(jobs)):
                                if i == 0 or jobs[i]["name"] != jobs[i - 1]["name"]:
                                    table.add_row(str(names[i]), str(statuses[i]), str(results[i]))

                elif "experiment" in result:
                    # Same as above, but for a single experiment (i.e. iris status)
                    ...
                    if "jobs" in experiment:
                        ...
                        for job in jobs:
                            ...
                            statuses.append(job["status"])
                            results.append(job["results"])
                        for name, status, result in zip(names, statuses, results):
                            table.add_row(str(name), str(status), str(result))
