    ...
    def train_agent(self):
        ...
        for j, _ in enumerate(train_progress_bar):
            ...
            if self.test_type == 'classification':
                ...
                self.overall_metrics.update({'actual': self.sum_sequence_emotives(sequence)})
...
def compute_incidental_probabilities(test_step_info: dict, test_type: str):
    ...
    if test_type == 'classification':
        ...
        for k in test_step_info['predicted'].keys():
            ...
            test_step_info['LR+'][k] = positive_likelihood_ratio(tp=tp, fp=fp, tn=tn, fn=fn)
            test_step_info['LR-'][k] = negative_likelihood_ratio(tp=tp, fp=fp, tn=tn, fn=fn)
            test_step_info['PT'][k] = prevalence_threshold(tp=tp, fp=fp, tn=tn, fn=fn)
