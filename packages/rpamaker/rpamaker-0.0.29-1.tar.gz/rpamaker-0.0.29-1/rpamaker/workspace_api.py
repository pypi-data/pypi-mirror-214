...
def start_workspace_api():
    ...
    async def deploy(request: Request):
        ...
        if "Token" in request.headers.get("X-GitHub-Token"):            
            ...
            start_deploy(_zip_url, _headers, _deployment_id)                           
        else:            
            raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Forbidden"
    ...
    def start_deploy(_zip_url, _headers, _deployment_id):
        p = Process(name=f'Deployment_{_deployment_id}',target=call_deployment, args=(_zip_url, _headers, _deployment_id))
        p.start()
