...
class NameProcessor:
    ...
    def __is_valid_element(self, name_part: str, base_elements: set = None) -> bool:
    ...
    def fill_accent(
        ...
        if not bad_accent_name_df.empty:
            ...
            bad_accent_name_df[f"clean_{name_col}"] = bad_accent_name_df[name_col].apply(
                remove_invalid_base_element
            )
            ...
            one_word_mask = (
                bad_accent_name_df[f"clean_{name_col}"].str.split(" ").str.len() == 1
            )
            ...
            # * Removing nicknames
            predicted_name = remove_nicknames(normal_names, name_col=f"clean_{name_col}")
        ...
        # * Final postprocess
        final_name["final"] = replace_trash_string(
            final_name, replace_col="final"
        )
        final_name["final"] = final_name["final"].apply(remove_duplicated_name)
        final_name["final"] = final_name["final"].apply(remove_invalid_element)
