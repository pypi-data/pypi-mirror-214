...
class ErrorCount:
    ...
    def error_report(self, permissive_mode: bool, ignore_permissive_mode: bool = False):
        if not (self.title_error + self.body_error + self.merge_error):
            ...
            if permissive_mode and ignore_permissive_mode:
                print(
                    emojize(
                        ":warning:  WARNING: 'permissive mode' is ignored when running to 'main' or 'master' branch"
                    )
                )
                sys.exit(1)
