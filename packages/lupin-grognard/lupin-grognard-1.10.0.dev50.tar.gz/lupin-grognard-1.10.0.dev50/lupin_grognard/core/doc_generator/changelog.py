...
from lupin_grognard.core.tools.utils import get_current_branch_name, info
...
class Changelog(JinjaGenerator):
    ...
    def _separate_jama_ref(self, commit: Commit) -> Tuple[List[str], str]:
        ...
        """
        if commit.body:
            if commit._is_last_description_line_startswith_jama():
                jama_ref = commit.body[-1].replace("JAMA:", "").strip()
