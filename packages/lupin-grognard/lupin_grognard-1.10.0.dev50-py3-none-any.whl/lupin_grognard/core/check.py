...
from lupin_grognard.core.commit.commit_validator import CommitValidator, CheckModes
...
def check_commit(
    ...
    commits = [
        CommitValidator(commit=c, error_counter=error_counter, mode_check=mode_check)
        for c in commits
    ]
    ...
    if mode_check == CheckModes.ALL_CHECKS_WITH_JAMA:
        ignore_permissive_mode = True  # ignore permissive mode to main and master branches
    else:
        ignore_permissive_mode = False

    error_counter.error_report(
        permissive_mode=permissive_mode, ignore_permissive_mode=ignore_permissive_mode
    )
