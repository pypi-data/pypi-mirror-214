...
def random_perspective(
    ...
    if n:
        ...
        if use_segments:  # warp segments
            segments = _resample_segments(segments)  # upsample
            for i, segment in enumerate(segments):
                ...
                # clip
                new[i] = _segment2box(xy, width, height)
        ...
        # filter candidates
        i = _box_candidates(box1=targets[:, 1:5].T * s, box2=new.T, area_thr=0.01 if use_segments else 0.10)
        targets = targets[i]
        targets[:, 1:5] = new[i]
