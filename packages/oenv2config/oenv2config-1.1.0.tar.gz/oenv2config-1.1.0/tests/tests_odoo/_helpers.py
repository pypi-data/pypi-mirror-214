...
def _try_find_addons_info() -> Tuple[str, int]:
    ...
    if current_env.odoo_version:
        ...
        if not os.path.exists(current_env.odoo_bin):
            ...
            odoov_path = pjoin(os.path.expanduser(_base_path), f"v{odoo_version}", f"odoo{odoo_version}", "odoo-bin")
...
def create_config(
    ...
    for env_file in env_files or []:
        ...
        efile_v = pjoin(os.path.dirname(__file__), "profiles", f"{env_file}-{odoo_version}.env")
        if os.path.exists(efile_v):
            profiles.append(efile_v)

    res_code = cli.execute_odoo_cli_helper(odoo_bin_path, profiles, odoo_rc, extra_odoo_args or [])
    if res_code:
...
def assertParser(
    case: unittest.TestCase, parser: configparser.ConfigParser, expected_values: Dict[str, Any], section="options"
):
    """
    ...
    for key, value in expected_values.items():
        ...
        if isinstance(value, bool):
            try:
                ...
                case.fail(f"{section}.{key}, not an 'boolean' value, {parser.get(section, key)}")
        elif isinstance(value, int):
            try:
                pvalue = parser.getint(section, key)
            except ValueError:
                case.fail(f"{section}.{key}, not an 'int' value, '{parser.get(section, key)}'")
        elif isinstance(value, float):
            try:
                pvalue = parser.getfloat(section, key)
            except ValueError:
                case.fail(f"{section}.{key}, not an 'float' value, {parser.get(section, key)}")
