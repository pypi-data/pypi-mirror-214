...
def env_to_section(section: Type[Section] = None, extra_env: Dict[str, str] = None) -> Section:
    """
    Convert environnement variables [os.envrion] to a Section [api.EnvConfigSection], with odoo compatible args,
    ...
    Returns:
        A Section [api.EnvConfigSection] or False if section Class is not found in CONVERTER
...
def env_to_dict(extra_env: Dict[str, str] = None) -> Dict[str, str]:
    """
    Convert environnement variables [os.envrion] to a dict, with odoo compatible args, by applying a mapper and
     converter.
    Args:
        extra_env: A dict to update environnement variables
    Returns:
