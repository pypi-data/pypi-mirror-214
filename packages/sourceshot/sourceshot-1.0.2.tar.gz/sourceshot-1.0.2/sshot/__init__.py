...
def _cli():
    ...
    try:
        ...
        return -1
    if args.config:
        config=color_config_from_json(args.config)
    else:
    ...
    try:
        ...
        return -1
    img=shot(source,config,verbose=verbose,lang=args.language,font=font,tabsize=tabsize,background=fromstr(args.background))
    if not args.output:
        try:
            image_show(img)
        except BaseException as e:
