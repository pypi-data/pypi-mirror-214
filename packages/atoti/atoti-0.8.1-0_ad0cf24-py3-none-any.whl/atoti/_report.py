...
class TableReport:
    ...
    _clear_reports: Callable[[str], None] = field(repr=False)
    _get_reports: Callable[[str], list[LoadingReport]] = field(repr=False)
    _table_name: str

    def clear(self) -> None:
        self._clear_reports(self.table_name)

    @property
    def reports(self) -> Sequence[LoadingReport]:
        """Reports of individual loading."""
        return self._get_reports(self.table_name)
