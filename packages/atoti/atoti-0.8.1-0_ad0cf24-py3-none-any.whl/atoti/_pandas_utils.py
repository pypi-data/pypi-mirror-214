    ...
    return [
        ...
        if (
            ...
            or (
                ...
                and _get_category_underlying_type(
                    # Pyright incorrectly infers that `dataframe[column]` is a `DataFrame` while it's actually a `Series`.
                    dataframe[column]  # pyright: ignore[reportGeneralTypeIssues]
                )
                == np.dtype(_M8_NS)
            )
        )
        and _contains_only_dates_without_time(
            # Pyright incorrectly infers that `dataframe[column]` is a `DataFrame` while it's actually a `Series`.
            dataframe[column]  # pyright: ignore[reportGeneralTypeIssues]
        )
    ]
