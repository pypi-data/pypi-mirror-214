...
class _UserDefinedAggregateFunction:
    ...
    @cached_property
    def _columns(self) -> Sequence[ColumnCoordinates]:
...
def _operand_to_udaf_operation(  # noqa: C901, PLR0911, PLR0912
    operand: Optional[Union[Operand[ColumnCoordinates], Operation]],
    /,
    *,
    get_column_data_type: Callable[[ColumnCoordinates], DataType],
) -> Operation:
    ...
    if isinstance(operand, ColumnCoordinates):
    ...
    if isinstance(operand, Condition):
        decombined_conditions = decombine_condition(  # type: ignore[var-annotated]
            ...
            allowed_target_types=(type(None), Constant, ColumnCoordinates, _Operation),
    ...
    def _do_distil(
        ...
        *,
        ...
        udaf_operation = _operand_to_udaf_operation(
            self._operation,
            get_column_data_type=lambda coordinates: java_api.get_column_data_type(
                coordinates.column_name, table_name=coordinates.table_name
            ),
        )
