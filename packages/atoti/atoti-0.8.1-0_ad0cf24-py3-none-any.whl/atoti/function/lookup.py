...
from atoti_core import ColumnCoordinates, Condition, Constant, condition_to_dict

from .._check_column_condition_table import check_column_condition_table
from .._measure_convertible import MeasureConvertibleCoordinates, MeasureOperation
...
def lookup(
    ...
    mapping: Condition[
        ColumnCoordinates,
        Literal["eq"],
        Union[Constant, MeasureConvertibleCoordinates, MeasureOperation],
    ...
    check_column_condition_table(
        mapping, attribute_name="subject", expected_table_name=column._table_name
    )
    return GenericMeasure(
        "LOOKUP",
        column._table_name,
        {
            subject.column_name: None
            if target is None
            else convert_operand_to_measure_description(target)
