...
def validate_input_file(file: str) -> str:
    ...
    elif file.endswith('.tac') or file.endswith('.conf') or file.endswith('.json') or file.endswith('.o') or file.endswith('.so'):
        validate_readable_file(file)
        return file

    raise Util.CertoraUserInputError(f"input file {file} is not in one of the supported types (.sol, .vy, .tac, .o, .so, .conf, "
                                     f".json)")
