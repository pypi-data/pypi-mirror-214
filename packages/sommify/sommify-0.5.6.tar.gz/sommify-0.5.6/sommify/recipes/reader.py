...
class RecipeReader:
    def __init__(
        self, es_url: str = "", es_user_name: str = "", es_pass: str = "", es_port : int = 9200
    ) -> None:
        ...
        self.es = elastic.get_es_connection(es_url, es_user_name, es_pass,es_port)
    ...
    def extract_ingredients(self, text):
        ...
        for chunk in chunks:
            ...
            if re.search(rf"\b(?:{r'|'.join(unique_ingredients)})(?:s|es)?\b", chunk):
                found.append(self.funnel(chunk))

            if re.search(re_.PROTEIN, chunk):
                found.append(I_label_protein(chunk))
    ...
    def categorize(
        ...
        roots = [
            token.text.lower() for token in self.nlp(title) if token.dep_ == "ROOT"
        ] + [
            token.text.lower() for token in self.nlp(alt_title) if token.dep_ == "ROOT"
        ]
    ...
    def read_terms(self, input_term: str) -> object:
        title, ings, steps = elastic.search_by_query_term(self.es, input_term)
