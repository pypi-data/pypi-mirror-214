...
from auto_screener.collect import find_name
...
__all__ = [
    ...
    "OrderBookScreener",
    "create_order_book_market",
    "market_order_book_screener",
    "market_order_book_recorder",
    "create_order_book_dataframe"
]

Market = Dict[str, Dict[str, pd.DataFrame]]
RecorderParameters = Dict[str, Union[Iterable[str], Dict[str, Callable]]]

def create_order_book_dataframe() -> pd.DataFrame:
    """
...
def create_order_book_market(data: Dict[str, Iterable[str]]) -> Dict[str, Dict[str, pd.DataFrame]]:
    """
    ...
    return {
        exchange.lower(): {
            symbol.upper(): create_order_book_dataframe()
            for symbol in data[exchange]
        } for exchange in data
    }
# end create_order_book_market
...
class MarketOrderBookRecorder(MarketRecorder):
    ...
    def order_book(self, exchange: str, symbol: str) -> pd.DataFrame:
        """
    ...
    async def record(self, data: OrderBook, timestamp: float) -> None:
        ...
        dataset = (
            self.market.
            setdefault(exchange, {}).
            setdefault(symbol, create_order_book_dataframe())
        )
    ...
    def create_screener(
        ...
        screener = OrderBookScreener(
            ...
            market=self.order_book(exchange=exchange, symbol=symbol)
        )
...
class MarketOrderBookScreener(BaseMultiScreener):
    ...
    >>> from auto_screener.feed import market_order_book_screener
    >>>
    ...
    def create_screener(
        ...
        if self.recorder.in_market(symbol=symbol, exchange=exchange):
            screener = self.recorder.create_screener(
                symbol=symbol, exchange=exchange, delay=delay,
                location=location, cancel=cancel
            )
    ...
    def order_book(self, exchange: str, symbol: str) -> pd.DataFrame:
        ...
        return self.recorder.order_book(exchange=exchange, symbol=symbol)
    # end order_book
...
def market_order_book_recorder(data: Dict[str, Iterable[str]]) -> MarketOrderBookRecorder:
    """
    ...
    return MarketOrderBookRecorder(
        market=create_order_book_market(data=data)
    )
# end market_order_book_recorder

def market_order_book_screener(
        data: Dict[str, Iterable[str]],
        location: Optional[str] = None,
        cancel: Optional[Union[Number, dt.timedelta]] = None,
        delay: Optional[Union[Number, dt.timedelta]] = None,
    ...
    screener = MarketOrderBookScreener(
        recorder=recorder or MarketOrderBookRecorder(
            market=market or create_order_book_market(data=data)
        ),
