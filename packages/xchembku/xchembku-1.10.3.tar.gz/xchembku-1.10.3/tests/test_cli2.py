...
class TestCli2Sqlite:
    ...
    def test(self, constants, logging_setup, output_directory):
        ...
        Cli2Tester(configuration_file).main(
            constants,
            configuration_file,
            output_directory,
        )
    ...
    async def _main_coroutine(self, constants, output_directory):
        ...
        try:
            ...
            async with xchembku_client1_context as dataface1:
                ...
                max_seconds = 5.0
                while True:
                    health = await dataface1.client_report_health()
                ...
                # Insert all three plates at once.
                await dataface1.upsert_crystal_plates(models)

                # Start a second concurrent client connection.
                xchembku_client2_context = ClientContext(xchembku_specification)
                async with xchembku_client2_context as dataface2:

                    # Check the filtered queries from the second connection.
                    # This verifies the previous upserts have been done.
                    await self.__check(
                        dataface2,
                    ...
                    # Second client requests the shutdown.
                    await dataface2.client_shutdown()
        finally:
            try:
