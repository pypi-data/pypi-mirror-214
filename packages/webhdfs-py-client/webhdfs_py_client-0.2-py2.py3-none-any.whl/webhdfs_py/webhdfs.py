...
class PyWebHdfsClient(object):
    ...
    >>> from webhdfspy.webhdfs import PyWebHdfsClient
    """
    ...
    def __init__(self, host='localhost', port='50070', user_name=None,
                ...
                request_extra_opts={}):
        """
        Cliente para interactuar con WebHDFS

        :param host: la ip address o hostname de HDFS namenode, defaults to 'localhost'
        :param port: el numero de puerto de WebHDFS de namenode, defaults to '50070'
        :param user_name: WebHDFS user.name, defaults to None
        :param path_to_hosts: mapeo, defaults to None
        :param timeout: timeout, defaults to 120
        :param proxy: , defaults to {"http": None, "https": None }
        :param base_uri_pattern: base URL, defaults to "http://{host}:{port}/webhdfs/v1/"
        :param request_extra_opts: DiCT con opciones extras para el request
        (e.j., SSL, HTTP authentication, etc.), defaults to {}
    ...
    def append_file(self, path, file_data, **kwargs):
        ...
        POST http://<HOST>:<PORT>/webhdfs/v1/<PATH>?op=APPEND[&buffersize=<INT>][&noredirect=<true|false>]
    ...
    def stream_file(self, path, chunk_size=1024, **kwargs):
        ...
        optional_args = kwargs
        # total_size = self.get_content_summary(path)["ContentSummary"]["length"]
        response = self._resolve_host(self.session.get, True,
    ...
    def make_dir(self, path, **kwargs):
        ...
        PUT http://<HOST>:<PORT>/webhdfs/v1/<PATH>?op=MKDIRS[&permission=<OCTAL>]
    ...
    def delete_file_dir(self, path, recursive=False):
        ...
        DELETE http://<host>:<port>/webhdfs/v1/<path>?op=DELETE[&recursive=<true |false>
    ...
    def get_content_summary(self, path):
        ...
        >>> hdfs.get_content_summary(my_folder)
        {
            "ContentSummary":
            {
                "directoryCount": 2,
    ...
    def get_home_dir(self):
        ...
        GET "http://<HOST>:<PORT>/webhdfs/v1/?op=GETHOMEDIRECTORY"
    ...
    def set_owner(self, path, **kwargs):
        ...
        >>> hdfs = PyWebHdfsClient(host='host',port='50070', user_name='hdfs')
        >>> my_dir = 'user/hdfs/data/new_dir'
        >>> hdfs.set_owner(my_dir, 755, owner='hdfs')
        """
        optional_args = kwargs
        response = self._resolve_host(self.session.put, False,
                                        path, operations.SETOWNER,
                                        **optional_args)
        if not response.status_code == http_client.OK:
            _raise_pywebhdfs_exception(response.status_code, response.content)
    ...
    def _resolve_host(self, req_func, allow_redirect, 
                    path, operation, stream=False, **kwargs):
        """
...
def _raise_pywebhdfs_exception(resp_code, message=None):
    ...
    if resp_code == http_client.BAD_REQUEST:
        ...
        raise errors.WebHdfsException(msg=message)
