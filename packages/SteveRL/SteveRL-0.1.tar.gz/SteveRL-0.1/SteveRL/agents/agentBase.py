...
class AgentBase:
    ...
    def __init__(self,net_dims:[int],state_dim:int,action_dim:int,gpu_id:int =0,args:Config =Config()):
        ...
        self.state_dim= args.state_dims
        self.action_dim =args.action_dims
        self.last_state =None
        self.logging_reward = 0

        self.devcie =torch.device(f'cuda:{gpu_id}' if (torch.cuda.is_available() and gpu_id>=0) else 'cpu')
        ...
        self.act =self.act_target =act_class(net_dims,state_dim,action_dim).to(self.devcie)

        self.cri =self.cri_target =cri_class(net_dims,state_dim,action_dim).to(self.devcie)\
    ...
    def explore_one_env(self,env,horizon_len:int,if_random:bool =False) -> Tuple[Tensor,...]:
        ...
        actions =torch.zeros((horizon_len,self.num_envs,self.action_dim),dtype=torch.float32).to(self.devcie)
        ...
        get_action = self.act.take_action
    ...
    def updata_net(self,buffer:Union[ReplayBuffer,tuple]) ->Tuple[float,...]:
        obj_critic =0.0
        obj_actor =0.0
