...
from train.config import Config, bulid_env
...
def train_agent(args:Config):
    ...
    '''init agent'''
    agent =args.agent_class(args.net_dims,args.state_dims,args.action_dims,gpu_id =args.gpu_id,args=args)


    state =env.reset()

    if args.num_envs ==1:
        assert  state.shape ==(args.state_dims,)
        assert isinstance(state,np.ndarray)
        state =torch.tensor(state,dtype=torch.float32,device=agent.device).unsqueeze(0)
    else:
        pass

    assert state.shape == (args.num_envs, args.state_dims)
    ...
    if args.if_off_policy:
        buffer = ReplayBuffer(
            ...
            state_dim=args.state_dims,
            action_dim=1 if args.if_discrete else args.action_dims,
    ...
    evaluator =Evaluator(cwd=args.cwd,env= eval_env,args=args,if_wandb=True)
