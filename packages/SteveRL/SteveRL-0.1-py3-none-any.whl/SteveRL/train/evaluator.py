...
class Evaluator:
    ...
    def evaluate_and_save(self,actor:torch.nn,steps:int,exp_r:float,logging_tuple:tuple):
        ...
        returns =rewards_step_ten[:,0]
        steps =rewards_step_ten[:,1]
        ...
        self.recorder.append((self.total_step,avg_r,std_r,exp_r,*logging_tuple))
        if self.wandb:
            if avg_r != None:
                wandb.log({'avg_r':avg_r})
            if std_r !=None:
                wandb.log({'std_r':std_r})
            if exp_r!=None:
                wandb.log({'exp_r':exp_r})
            if self.total_step!=None:
                wandb.log({'total_steps':self.total_step})
    ...
    def save_training_curve_jpg(self):
        ...
        draw_learning_curve(recorder,fig_title= fig_title,save_path =f"{self.cwd}/LearningCurve.jpg")
...
def get_cumulative_rewards_and_steps(env,actor,if_rander:bool =False)->Tuple[float,int]:
    '''用训练好的actor 执行环境看看得到的reward 是多少，
    相当于测试的是test 过程 而不是train过程
    返回的是轨迹的reward和步数
    '''
...
def draw_learning_curve(recorder:np.ndarray =None,fig_title:str ='learning_curve',
                        save_path:str ='learning-curve.jpg'):
    steps =recorder[:,0]
    r_avg =recorder[:,1]
    r_std =recorder[:,2]
    r_exp =recorder[:,3]
    ...
    ax01.polt(steps,r_exp,color= color01,alpha =0.5)
