...
def notarizationContextRawfile():
    return "../.." + str(notarizationPath) + "/../rawFile.csv"

def notarizationContext(_filename) :
    return "../.." + str(notarizationPath) + "/" + str(_filename)

def dayContext(_m, _filename) :
    if (_m >= 0) : raise Exception("No history at J", str(_m))
    day = json.load(open("../.." + str(notarizationPath) + "/.config.json"))["day"] + str(_m)
    if (day < 0) : raise Exception("No history at J", str(day))
    lastNotarization = json.load(open("../.." + str(notarizationPath) + "/../../Day" + str(day) + "/.dayConfig.json"))["lastNotarization"]
    return "../.." + str(notarizationPath) + "/../../Day" + str(day) + "/Notarization" + str(lastNotarization) + "/" + str(_filename)
...
def finish(*args) :
    ...
    if isToCache == 1 :
        ...
        for i in args : 
            try : 
                call(["mv",str(i),"../.." + str(notarizationPath)])
            except :
                print("Warning : ", str(i), "not found")
        print("Final result at :", "../.." + str(notarizationPath))