...
from es7s.shared import get_config, get_logger
...
class ShocksProvider(DataProvider[ShocksInfo]):
        ...
        try:
            ...
            if response.ok:
                return ShocksInfo(
                    ...
                    amount=self._get_tunnel_amount(),
                )
    ...
    def _get_tunnel_amount(self) -> int:
        return len(sub.run(["pgrep", "-f", "ssh\\s.*-L"], stdout=sub.PIPE).stdout.splitlines())
