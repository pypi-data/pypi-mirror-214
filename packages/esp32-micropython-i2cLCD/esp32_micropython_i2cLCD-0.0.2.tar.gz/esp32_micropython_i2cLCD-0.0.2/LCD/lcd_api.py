    ...
    def puts(self, txt, x=0, y=0, desactive_auto=True, moveto=None):
        if desactive_auto:
            ...
            if self.estabrilhando and self.estaaparecendoocurso:
                ...
                estavabrilhando = True
            else:
                if self.estabrilhando:
                    estavabrilhando = True
                    self.blink_cursor_off()
                if self.estaaparecendoocurso:
                    estavaaparecendoocurso = True
                    self.hide_cursor()
        self.cursor_x = x
        self.cursor_y = y
        ...
        for i in txt:
            if lex < 16:
                ...
                lex += 1
        if moveto:
            self.move_to(moveto[0], moveto[1])
        if desactive_auto:
            if estavabrilhando and estavaaparecendoocurso:
