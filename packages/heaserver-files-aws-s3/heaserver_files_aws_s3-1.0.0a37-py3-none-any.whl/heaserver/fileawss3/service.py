...
async def put_file(request: web.Request) -> web.Response:
    ...
    activity.description = f'Updating {awsservicelib._activity_object_display_name(bucket_id, key)}'
...
async def post_file_mover(request: web.Request) -> web.Response:
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Moving {awsservicelib._activity_object_display_name(bucket_id, key)} to {awsservicelib._activity_object_display_name(target_bucket_name, target_key)}'
    await publish_desktop_object(request.app, activity)
...
async def get_versions(request: web.Request) -> web.Response:
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Getting {awsservicelib._activity_object_display_name(bucket_name, key)} versions'
    await publish_desktop_object(request.app, activity)
...
async def get_version(request: web.Request) -> web.Response:
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Getting {awsservicelib._activity_object_display_name(bucket_name, key)} version {id_}'
    await publish_desktop_object(request.app, activity)
...
async def delete_version(request: web.Request) -> web.Response:
    ...
    activity = DesktopObjectAction()
    activity.generate_application_id()
    activity.code = 'hea-delete'
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Deleting {awsservicelib._activity_object_display_name(bucket_name, key)}'
    await publish_desktop_object(request.app, activity)
...
async def make_current_version(request: web.Request) -> web.Response:
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Making version {id_} of {awsservicelib._activity_object_display_name(bucket_name, key)} the current version'
    await publish_desktop_object(request.app, activity)
...
async def _get_file(request: web.Request) -> web.Response:
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Getting {awsservicelib._activity_object_display_name(bucket_name, file_id)}'
    await publish_desktop_object(request.app, activity)
...
async def _get_object_content(request: web.Request) -> web.StreamResponse:
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Getting {awsservicelib._activity_object_display_name(bucket_name, key)} content'
    await publish_desktop_object(request.app, activity)
...
async def _generate_presigned_url(request: web.Request):
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Getting pre-signed URL for {awsservicelib._activity_object_display_name(bucket_id, object_key)}'
    await publish_desktop_object(request.app, activity)
...
async def _put_object_content(request: web.Request) -> web.Response:
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Updating {awsservicelib._activity_object_display_name(bucket_name, file_id)} content'
    await publish_desktop_object(request.app, activity)
