...
class QueryExecutor(Generic[Model]):
    ...
    def fetch_fields(self, fields: List[str], with_total: bool = False, **kwargs):
        """
        :arg fields: include source fields
    ...
    def fetch(self, with_total: bool = False, **kwargs):
        """
        :arg kwargs: any additional arguments will be passed on to the opensearch-py call
        """
        model = self.__model_cls
        if with_total:
            hits, total = self.fetch_fields(model.default_fields(), True, **kwargs)
            return [model.parse_obj(hit) for hit in hits], total
        else:
            hits = self.fetch_fields(model.default_fields(), False, **kwargs)
