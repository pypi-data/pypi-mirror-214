...
1. The input sam file is obtained by mapping quality-filtered reads to ALL (not a subset of) contigs derived 
   from the quality-filtered reads (e.g., flye produced assembly.fasta or medaka polished assemblies).
2. Important!!! The input Sam file and MAGs need to be generated from the same set of sequences (with the same id). 
3. If one read was mapped to multiple contigs from different MAGs, the read will be exported to the extracted 
   reads of all above MAGs.
