...
def resolve_anorganizations_sync(obj, info, connection_args, **kwargs):
...
def resolve_users_sync(obj, info, connection_args, **kwargs):
...
def resolve_anorganization_memberships_sync(obj, indo, connection_args, **kwargs):
    ...
    if lookups:
        queryset = obj.filter(reduce(operator.and_, lookups)) \
                if isinstance(obj, QuerySet) \
                else Membership.objects.filter(reduce(operator.and_, lookups))
    else:
        queryset = obj if isinstance(obj, QuerySet) \
