...
from iqrfpy.peripherals.sensor.requests.read_sensors_with_types import ReadSensorsWithTypesRequest, SensorWrittenData
...
class ReadSensorsWithTypesRequestTestCase(unittest.TestCase):
    ...
    @parameterized.expand([
        ...
        [
            None,
            b'\x01\x00\x5e\x01\xff\xff\xff\xff\xff\xff'
        ],
        [
            [0, 1, 2],
    ...
    def test_set_sensors(self, sensors: Optional[List[int]], dpa: bytes):
        ...
        self.json['data']['req']['param']['sensorIndexes'] = sensors if sensors is not None else -1
        self.assertEqual(
            request.to_dpa(),
            dpa
        )
