...
class Common:
    ...
    def pnum_from_json(json: dict) -> int:
        ...
        -------
        pnum
            JSON API response peripheral number

        Raises
        ------
        JsonNadrMissingError
            Raised if Daemon API response does not contain the pnum key
        """
        try:
    ...
    def pcmd_from_json(json: dict) -> int:
        ...
        -------
        pnum
            JSON API response peripheral command

        Raises
        ------
        JsonNadrMissingError
            Raised if Daemon API response does not contain the pcmd key
        """
        try:
    ...
    def sensors_indexes_to_bitmap(sensors: List[int]) -> List[int]:
        ...
        ----------
        sensors: List[int]
            List of sensor indexes

        Returns
        -------
        bitmap
            List of bitmap values representing sensor indexes.
        """
        bitmap = [0] * 4
