...
class AWSHandler:
    ...
    def __init__(self, thunder, service):
        ...
        self._config =  Utils._config
        self._credentials =  Utils._aws_credentials 
        self._service= service
        self.init_client()
    
    def init_client(self):
        self._client = boto3.client(self._service, aws_access_key_id=self._credentials.get(Constant._AWS_ACCESS_KEY_ID),aws_secret_access_key=self._credentials.get(Constant._AWS_SECRET_ACCESS_KEY),region_name=self._config.get(Constant._AWS_REGION))
