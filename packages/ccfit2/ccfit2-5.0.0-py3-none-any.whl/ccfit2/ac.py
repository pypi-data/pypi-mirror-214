...
class Model(ABC):
    ...
    def flat_thresh(self) -> float:
        '''
        Threshold for fit of data to y=mx+b above which data is marked as flat
        '''
...
class Experiment():
    ...
    def from_file(cls, file: str, mass: float, mw: float,
        ...
        mm = Measurement.from_file(file, mass, mw, header_indices, data_header)

        all_exp = cls.from_measurements(mm, temp_thresh, field_thresh)
...
def plot_colecole(experiments: list['Experiment'], save: bool = False,
    ...
    ax.set_xlabel(r'$\chi^{,}$  (cm$^{3}$mol$^{-1}$)')
    ax.set_ylabel(r'$\chi^{,,}$ (cm$^{3}$mol$^{-1}$)')
...
def interactive_ac_model_select(experiments: list['Experiment']) -> Model:
    ...
    def callback(label, modstore):
        label = label.lstrip()
        setattr(modstore, 'model', supported_models[label])
        plt.close()
        fig.canvas.stop_event_loop()
        ut.cprint('\n {} has been selected\n'.format(label), 'green')
        return

    modstore = type('obj', (object,), {'model': None})
...
def _plot_fitted_colecole(experiments: list['Experiment'],
    ...
    # Set labels for the axes
    axes[1].set_xlabel(r'$\chi^{,}$  (cm$^{3}$mol$^{-1}$)')
    axes[1].set_ylabel(r'$\chi^{,,}$ (cm$^{3}$mol$^{-1}$)')
...
def _plot_fitted_susceptibility(experiments: list['Experiment'],
    ...
    axes[2].set_ylabel(r'$\chi^{,,}$ (cm$^{3}$mol$^{-1}$)')
...
def write_model_data(experiments: list['Experiment'], models: list['Model'],
    ...
    '''
    Creates file containing χ' and χ'' calculated using the model
...
def write_model_params(models: list[Model], file_name: str,
                       verbose: bool = True) -> None:
    '''
    Writes fitted and fixed parameters of a set of models to file.\n
