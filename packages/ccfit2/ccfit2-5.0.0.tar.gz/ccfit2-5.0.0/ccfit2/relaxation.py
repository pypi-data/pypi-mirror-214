...
class TauHDataset():
    ...
    def from_ccfit2_files(cls, file_names: str | list[str]) -> 'TauHDataset':
        ...
        headers = {
            'fields': ['H'],
            'lntau': ['<ln(tau)>'],
            'lntau_stdevs': ['sigma_ln(tau)'],
            'lntau_fu': ['fit_upper_ln(tau)'],
            'lntau_fl': ['fit_lower_ln(tau)']
        }
        ...
        for file, encoding in zip(file_names, encodings):
            ...
            optional_indices = [
                ...
                header_indices['lntau_fu'],
                header_indices['lntau_fl']
            ]
...
class LogFDQTMModel(LogTauHModel):
    ...
    UNITS_MM = {
        ...
        'Q_H': r'$\log_\mathregular{10}\left[\mathregular{s} \ \mathregular{Oe}^{p}\right]$', # noqa
        'p': ''
    }
    ...
    UNITS_HTML = {
        ...
        'Q_H': r'log<sub>10</sub>[s Oe<sup>p</sup>]',
        'p': ''
    }
...
class LogRamanModel(LogTauTModel):
    ...
    #: Model Parameter unit mathmode strings
    UNITS_MM = {
        'R': r'$\log_\mathregular{10}\left[\mathregular{s}^\mathregular{-1} \mathregular{K}^\mathregular{-n}\right]$', # noqa
        'n': ''
    }
...
class FitWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, dataset: TauTDataset | TauHDataset, usel: object,
        ...
        self.temp_grid = np.linspace(
            np.min(x_vars), np.max(x_vars), 1000
        )

        # Sum of all models, set to zero initially
        total_rates = np.zeros(np.shape(self.temp_grid))
        ...
        for color, model in zip(colors, supported_models):
            ...
            # Calculate rates
            rates = 10**(model.model(initials, self.temp_grid))
        ...
        gui.convert_log_ticks_to_lin(ax, np.log10(x_vars))
        ...
        for mit, model in enumerate(supported_models):
            model_widget = self.make_modelbox(model, scroll_container, mit + 1)
    ...
    def reset(self):
        ...
        for model in self.supported_models:
            ...
            self.tickdict[modelname].setCheckState(False)
    ...
    def update_modelplot(self, value, parname, model: LogTauTModel):
        ...
        new_rates = 10**model.model(parameters, self.temp_grid)

        _plot = self.model_plots[model.NAME.lower()]
        _plot.setData(self.temp_grid, new_rates)

        # Update total plot as sum of all models
        # use ydata of each model
        total_rates = np.zeros(self.temp_grid.shape)
        total_rates += np.sum(
            [
                _plot.yData
        ...
        self.tot.setData(self.temp_grid, total_rates)
    ...
    def toggle_plot(self, val: int,
        ...
        Calculates model data, adds/deletes line from plot
        and updates total model plot
        '''
    ...
    def make_modelbox(self, model: LogTauTModel, parent, index: int):
        '''
        ...
        on_off_shortcut = QtWidgets.QShortcut(
            QtGui.QKeySequence('{:d}'.format(index)),
            self
        )
        ...
        for parname in model.PARNAMES:
            ...
            cb = partial(self.update_modelplot, model=model, parname=parname)
...
def plot_fitted_times(dataset: TauTDataset | TauHDataset,
    ...
    _plot_fitted_times(dataset, model, fig, ax)
...
def plot_times(dataset: TauTDataset | TauHDataset, show: bool = True,
    ...
    ax.set_ylabel(r'$\ln{\tau}$ $\left(\ln\left[\mathregular{s}^\mathregular{-1}\right]\right)$') # noqa
...
def _plot_fitted_times(dataset: TauTDataset | TauHDataset,
    ...
    x_vars_grid = np.logspace(
        np.log10(np.min(x_vars)),
        np.log10(np.max(x_vars)),
        500
    )
    ...
    for logmodel in logmodels:
        ...
        # Trim model rates to sensible values
        valid_int = np.where(model_rates > 1E-6)[0]

        ax.plot(
            1. / np.array(x_vars_grid)[valid_int],
            np.log(1. / np.array(model_rates)[valid_int]),
            lw=1.5,
            label=logmodel.NAME,
            ls='--'
        )
    ...
    ax.legend(
        loc='lower right', fontsize='10', numpoints=1, ncol=1, frameon=False
    )
    ax.set_ylabel(r'$\ln{\tau}$ $\left(\ln\left[\mathregular{s}^\mathregular{-1}\right]\right)$') # noqa
...
def plot_fitted_rates(dataset: TauTDataset | TauHDataset,
    ...
    _plot_fitted_rates(dataset, model, fig, ax)
...
def qt_plot_fitted_rates(app: QtWidgets.QApplication,
    ...
    # Add plot
    _plot_fitted_rates(dataset, model, window.sc.fig, window.sc.ax)
...
def _plot_fitted_rates(dataset: TauTDataset | TauHDataset,
    ...
    for logmodel in logmodels:
        ...
        ax.plot(
            np.array(x_vars_grid)[valid_int],
            np.array(model_rates)[valid_int],
            lw=1.5,
            label=logmodel.NAME,
            ls='--'
        )
...
def plot_rates(dataset: TauTDataset | TauHDataset, show: bool = True,
    ...
    gui.set_rate_xtick_formatting(ax, x_vars)
...
def _plot_rate_residuals(dataset: TauTDataset | TauHDataset,
    ...
    model_rate = np.zeros(len(x_vars))
    ...
    # Axis labels
    ax.set_ylabel(
        r'$\log_\mathregular{10}[\tau^\mathregular{-1}_{\mathregular{exp}}]$ - $\log_\mathregular{10}[\tau^\mathregular{-1}_{\mathregular{fit}}]$' # noqa
    )
...
def write_model_params(model: LogTauTModel | MultiLogTauTModel | LogTauHModel | MultiLogTauHModel, # noqa
    ...
    # Write each term's name, fit vars, stdev, and fix vars
    for logmodel in logmodels:
        f.write('{}\n'.format(logmodel.NAME))
        if len(logmodel.fit_vars.keys()):
            f.write('Fitted Terms: Value and Standard Deviation:\n')
            for name in logmodel.fit_vars.keys():
                f.write('{:>6} {: 1.8E} {: 1.8E} {}\n'.format(
                    name,
                    logmodel.final_var_values[name],
                    logmodel.fit_stdev[name],
                    logmodel.UNITS[name]
                ))
        if len(logmodel.fix_vars.keys()):
            f.write('Fixed Terms: Values\n')
            for name in logmodel.fix_vars.keys():
                f.write('{:>6} {: 1.8E} {}\n'.format(
                    name,
                    logmodel.final_var_values[name],
                    logmodel.UNITS[name]
...
def write_model_data(dataset: TauTDataset | TauHDataset,
    ...
    f.write('{:>9} '.format(x_var_label))
    for logmodel in logmodels:
        f.write('{:>7} rate '.format(logmodel.NAME))
    if len(logmodels) > 1:
        f.write('  Total rate (s^-1)')
    f.write('\n')

    for grid_pt in x_vars_grid:
        f.write('{: 9.5f} '.format(grid_pt))
        total = 0.
        for logmodel in logmodels:
            rate = 10**logmodel.model(
                logmodel.final_var_values,
                [grid_pt]
            )[0]
            f.write('{:.6E} '.format(rate))
            total += rate
        if len(logmodels) > 1:
            f.write('{:.6E}'.format(total))
        f.write('\n')
