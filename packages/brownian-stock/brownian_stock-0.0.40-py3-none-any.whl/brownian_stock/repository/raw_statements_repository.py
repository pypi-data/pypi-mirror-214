...
from ..schemas import StatementSchema, create_table_query
...
class RawStatementsRepository:
    ...
    def insert_statements_df(self, df: pl.DataFrame) -> None:
        ...
        try:
            ...
            if not validate_dataframe(df, StatementSchema):
...
def preprocess_before_insert(df: pl.DataFrame) -> pd.DataFrame:
    ...
    df = cast_with_model(df, StatementSchema, strict=False)
    validate_dataframe(df, StatementSchema)
    return df
