...
def get_wait_resources(redis: Redis) -> dict[str, Resource]:
    ...
    return {
        ...
        if name in job_resources or name == 'job'
    }
...
def lock_resources_semaphore(redis: Redis):
    for resource in get_job_resources():
        print('lock_resources_semaphore: increment +1', resource)
        redis.hincrby(resource.key, 'usage_current')


def unlock_resources_semaphore(redis: Redis):
    for resource in get_job_resources():
...
def main(line_buffer_size: int = 100):
    ...
    lock_resources_semaphore(redis)
    ...
    redis.delete(key)
    unlock_resources_semaphore(redis)
