...
def hold1():
    string = '''
        # Main
        X_train, X_test, y_train, y_test = holdout(X, y, percentage=0.75)
        # Funcion
        def holdout(X, y, percentage):
            X_train = X.sample(round(percentage * len(X)))
        
            y_train = y.iloc[X_train.index]
        
            X_test = X.iloc[~X.index.isin(X_train.index)]
            y_test = y.iloc[~y.index.isin(y_train.index)]
