...
def rank2():
    string = '''
        ...
        def ranking(Y, R, movies_idx, num_peliculas):
            ...
            id_ord_val = np.argsort(num_valoraciones, axis=0)[::-1]
            id_ord_media = np.argsort(peliculas_recomendadas, axis=0)[::-1]
        
            print(f"Primeras {num_peliculas} películas mejor valoradas (Ordenadas por valoración):")
            for i in range(num_peliculas):
                print(f"{i} Nº {id_ord_val[i]} con {peliculas_recomendadas[id_ord_val[i]]} puntos"
                      f", valorada {num_valoraciones[id_ord_val[i]]} ({movies_idx[id_ord_val[i]]})")
        
            print(f"\nPrimeras {num_peliculas} peliculas mejor valoradas (Ordenadas por media):")
            for i in range(num_peliculas):
                print(
                    f"{i} Nº {id_ord_media[i]} con {peliculas_recomendadas[id_ord_media[i]]} puntos"
                    f", valorada {num_valoraciones[id_ord_media[i]]} ({movies_idx[id_ord_media[i]]})")
      '''
    print(string)
