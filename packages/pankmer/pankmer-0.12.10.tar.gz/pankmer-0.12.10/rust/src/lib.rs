...
use crate::score_list_io::{load_scores, dump_scores, compress_scores,
                           compress_scores_exclusive, subset_scores};
...
pub type PKTbl = HashMap<Kmer, usize>;
...
fn index(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(compress_scores, m)?)?;
    m.add_function(wrap_pyfunction!(compress_scores_exclusive, m)?)?;
