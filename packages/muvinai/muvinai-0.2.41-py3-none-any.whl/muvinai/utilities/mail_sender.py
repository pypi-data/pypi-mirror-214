...
def send_mail_inactivo(
    ...
    plan_merchants_central = [doc['_id'] for doc in db.merchants.find({'negocio_central': True}, {'_id': 1})]
    plan_merchants_central.append(ObjectId('6178652dcfb117ad58a2cd3d'))

    if receiver.get('status') == 'baja':
        if receiver.get('plan_corporativo', None):
            template = 'inactivacion-por-baja-corpo'
            global_vars.append({'name': 'email_contacto', 'content': 'atencion.socios@sportclub.com.aelifr'})
            ...
            global_vars.extend([
                {'name': 'url', 'content': 'https://www.sportclub.com.ar'},
    ...
    if receiver.get('status') == 'activo':
        if receiver['merchant_id'] in plan_merchants_central and 'seller_merchant_id' not in receiver:
