...
def checkin(**kwargs):
    ...
    checkin_message[0] = f'总共签到的任务个数为{checkin_count}个，不执行签到的任务数为{not_checkin_count}个,签到失败个数为{checkin_error_count}个\n'
    ...
    log_info('###############################all checkin complete  ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '  ###################################\n', my_logger=checkin_logger)
