...
def correct_schema(sql_schema, open_ai_data):
    promptX = f"""
    Hello GPT, I have my collage project in SQL, 
    I have designed existing database schema but i am not sure weather it is correct or not.

    I want you to act as SQL Expert and Correct my mistakes and help me to get done my project.

    I want you help to redesign schema with all tables and normalization and modify table details if you find it may cause performance issue. also i want you to test constraints in it. if you feel any table fields are required to added you can add them.

    Here is my SQL schema
    `{sql_schema}`

    I want you to only response with SQL Statement, Don't include any kind of text in response.
    I don't want any kind of explanations and any text only and only SQL response so i can directly 
    run into sql editor, also dont add starting or ending likes in response which you usually write.
    """
