...
class ChalkAPIClientImpl(ChalkClient):
    ...
    def _raise_http_error(http_error: HTTPError, exception_cls: Type[ChalkBaseException], **exception_kwargs: Any):
        trace_id = None
        if hasattr(http_error.response, "headers") and isinstance(http_error.response.headers, Mapping):
            trace_id = http_error.response.headers.get(_CHALK_TRACE_ID_KEY)
        ...
        raise exception_cls(errors=[chalk_error], trace_id=trace_id, **exception_kwargs)
        ...
        try:
            if r.status_code >= 400:
                try:
                    ...
                    r.raise_for_status()
                raise exception_cls(
                    errors=exception.errors, trace_id=exception.trace, detail=exception.detail, **exception_kwargs
                )
        except HTTPError as e:
            self._raise_http_error(http_error=e, exception_cls=exception_cls, **exception_kwargs)
