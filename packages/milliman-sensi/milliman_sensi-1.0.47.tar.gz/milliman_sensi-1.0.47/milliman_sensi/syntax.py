...
class Syntax:
    def __init__(self, expression = '', col = '', condition = '', value = ''):
        ...
        Args:
            expression (str, optional): The expression to be evaluated. Defaults to ''.
            col (str, optional): The column name. Defaults to ''.
            condition (str, optional): The condition to be evaluated. Defaults to ''.
            value (str, optional): The value to be evaluated. Defaults to ''.
        """        
...
def extract_target_column(param_string):
    """Extracts column from param_string if param_string contains [ and ends with ]
...
def parse_param(input_syntax):
    ...
    result = "$" + (f"..*['{eco}']" if 'eco' in eco else f"..*[@.name is '{eco}']") + (f"..*['{driver}']" if 'driver' in driver else f"..*[@.name is '{driver}']") + f".{param_expression}.filename"
...
def query(data, expression):
    ...
    Raises:
        SensiSyntaxError: if data is empty or expression is invalid or the query fails
...
def get_selection_from_dataframe(selection, dataframe):
    ...
    Raises:
        SensiSyntaxError: if dataframe is empty or selection is invalid
    ...
    if dataframe is None or dataframe.empty:
        logger.error(f"Dataframe is empty: {dataframe}")
        raise SensiSyntaxError("Dataframe is empty")
...
def select_from_dataframe(condition, operation, dataframe):
    ...
    if not selected.empty:
        ...
        logger.error(f"Selection is empty: {selected}")
        raise SensiSyntaxError("Selection is empty")
    ...
    if value:
        if not applying_operation:
            ...
            try:
                ...
                if operation == '+':
                    for column in selected_dict.keys():
                        selected_dict[column] = {k: mp.nstr(mp.mpf(v) + value, PRECISION) for k, v in selected_dict[column].items()}
                elif operation == '-':
                    for column in selected_dict.keys():
                        selected_dict[column] = {k: mp.nstr(mp.mpf(v) - value, PRECISION) for k, v in selected_dict[column].items()}
                elif operation == '*':
