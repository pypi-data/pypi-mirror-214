...
def read_csv_from_filepath(filepath):
    ...
    try:
        # Read the csv file as a series using '~' as the delimiter which should not be present in the csv file
        sensi_file = pd.read_csv(
            filepath, sep=r"~", header=None, quoting=csv.QUOTE_NONE
        ).squeeze("columns")
        if not isinstance(sensi_file, pd.Series):
            logger.error(
                f'{filepath} contains the delimiter "~" which is not allowed in sensi csv files'
            )
            raise ValueError(
                'File contains the delimiter "~" which is not allowed in sensi csv files'
            )
    ...
    try:
        ...
        sensi_file = sensi_file.map(
            lambda x: re.sub(
                r'"([^"]*)"', lambda m: re.sub(r";", "_SEMI_COL", m.group()), x
            )
        )
...
def sensi_config_is_valid(sensi_config):
    ...
    Raises:
        SensiIOError: If the sensi config is not a dataframe, if the header is not correct, if the number of columns is not correct or a value in 'Apply stress' is incorrect
    """
    ...
    sensi_config_header = (
        sensi_config.iloc[0].drop("_count_sep").dropna().values.tolist()
    )
    if sensi_config_header != SENSI_CONFIG_HEADER:
        logger.error(
            f"Sensi config header is incorrect. Expected {SENSI_CONFIG_HEADER}, got {sensi_config_header}"
        )
        raise SensiIOError(
            f"Sensitivity configuration file header is incorrect. Expected {SENSI_CONFIG_HEADER}, got {sensi_config_header}"
        )
    ...
    if not (sensi_config["_count_sep"] == len(SENSI_CONFIG_HEADER) - 1).all():
        ...
        logger.error(
            f"Sensi config has the wrong number of columns. Rows with wrong number of columns: {rows_with_wrong_number_of_columns}"
        )
        raise SensiIOError(
            f"Sensitivity configuration file has the wrong number of columns. Rows with wrong number of columns: {rows_with_wrong_number_of_columns}"
        )

    # Checking if the 'Apply stress' column has the correct values
    logger.debug(f'Checking sensi config values in "Apply stress"')
    apply_stress_values = sensi_config.iloc[
        1:, SENSI_CONFIG_HEADER.index("Apply stress")
    ]
    apply_stress_values_check = apply_stress_values.map(
        lambda x: isinstance(x, bool) or x.lower() in ["true", "false"]
    )
    if not apply_stress_values_check.all():
        rows_with_wrong_apply_stress_values = apply_stress_values[
            ~apply_stress_values_check
        ].index.tolist()
        logger.error(
            f'Sensi config has the wrong values in "Apply stress". Rows with wrong values: {rows_with_wrong_apply_stress_values}'
        )
        raise SensiIOError(
            f'Sensitivity configuration file has the wrong values in "Apply stress". Rows with wrong values: {rows_with_wrong_apply_stress_values}'
        )
...
def sensi_param_is_valid(sensi_param):
    ...
    Raises:
        SensiIOError: If the sensi param is not a dataframe, if the first column is not the 'Name' column or if the sensi param has the wrong number of columns
    """
    ...
    if not sensi_param.iloc[0, 0] == "Name":
        ...
        raise SensiIOError(
            'Sensitivity parameters file first column is not the "Name" column'
        )
    ...
    if not sensi_param["_count_sep"].nunique() == 1:
        ...
        logger.error(
            f"Sensi param has the wrong number of columns. Rows with wrong number of columns: {rows_with_wrong_number_of_columns}"
        )
        raise SensiIOError(
            f"Sensitivities parameters file has the wrong number of columns. Rows with wrong number of columns: {rows_with_wrong_number_of_columns}"
        )
...
def get_sensi_and_param_lists(sensi_config, sensi_param):
    """Gets the sensi and param lists from the sensi config and sensi param files
    ...
    Raises:
        SensiIOError: If the sensi config and sensi param files do not have the same number of rows

    Returns:
        tuple: (sensi_list(dict), sensi_list_without_stress (list), param_list(dict))
    """
    logger.info(f"Getting sensi and param lists")

    logger.debug(f"Checking if sensi config and sensi param are valid")
    # Check if the values in the 'Stress name' column are a subset of the columns in the sensi param file except the 'Name' column
    if not set(sensi_config["Stress name"]).issubset(
        set(sensi_param.columns.tolist()[1:])
    ):
        # Get the values in the 'Stress name' column that are not a subset of the columns in the sensi param file except the 'Name' column
        sensi_config_stress_names_not_in_sensi_param = set(
            sensi_config["Stress name"]
        ) - set(sensi_param.columns.tolist()[1:])
        logger.error(
            f"Stress names in the sensi config file are not a subset of the stress names in the sensi param file. Stress names not in the sensi param file: {sensi_config_stress_names_not_in_sensi_param}"
        )
        raise SensiIOError(
            f"Stress names in the Sensitivity configuration file are not a subset of the stress names in the Sensitivity parameters file. Stress names not in the Sensitivity parameters file: {sensi_config_stress_names_not_in_sensi_param}"
        )
    ...
    for sensi_name in sensi_names:
        ...
        stress_dict = (
            sensi_config[sensi_config["Scenario"] == sensi_name]
            .set_index("Stress name")["Apply stress"]
            .to_dict()
        )
        # Add keys from stress dict to sensi list if the value is True or 'true' (case insensitive)
        sensi_list[sensi_name] = [
            stress_name
            for stress_name, apply_stress in stress_dict.items()
            if apply_stress.lower() == "true"
        ]

    # Get the list sensi_list_without_stress
    logger.debug(f"Getting sensi list without stress")
    sensi_list_without_stress = [
        sensi_name for sensi_name, stress_list in sensi_list.items() if not stress_list
    ]
    ...
    for stress_name in stress_names:
        ...
        stress_df[stress_name] = (
            stress_df["Name"] + "=" + stress_df[stress_name].astype(str)
        )
    ...
    # Sort the param map unsorted by the order of the stress names in the 'Stress name' column of the sensi config file where 'Apply stress' is True or 'true' (case insensitive)
    param_map = {
        stress_name: param_map_unsorted[stress_name]
        for stress_name in sensi_config[
            sensi_config["Apply stress"].str.lower() == "true"
        ]["Stress name"].tolist()
    }
...
def read_sensitivities(env_dir):
    ...
    Raises:
        SensiIOError: If either the sensi config or sensi param file is not valid
    ...
    sensi_list, sensi_list_without_stress, param_map = get_sensi_and_param_lists(
        sensi_config, sensi_param
    )
    ...
    for root, dirs, files in os.walk(base_rsrc_dir):
        for item in dirs + files:
            ...
            dest_path = os.path.join(
                sensi_rsrc_dir, os.path.relpath(path, base_rsrc_dir)
            )
...
def create_dir_for_one_sensi_from_base(sensi_path, base_dir):
    ...
    """
    logger.info(
        f"Creating directory for one sensitivity from base directory {base_dir}"
    )
    ...
    logger.debug(
        f"Copying resources and resources_admin from base directory to sensi directory"
    )
    try:
        ...
        logger.error(
            f"Error copying resources and resources_admin from base directory to sensi directory: {e}"
        )
        return SensiIOError(
            f"Error copying resources and resources_admin from base directory to sensi directory"
        )

    # Delete 'settings_calibration.json' and 'settings_simulation.json' from sensi directory if they exist
    logger.debug(
        f"Deleting settings_calibration.json and settings_simulation.json from sensi directory if they exist"
    )
    calib_settings_path = os.path.join(
        sensi_path, "resources", "settings_calibration.json"
    ).replace("\\", "/")
    sim_settings_path = os.path.join(
        sensi_path, "resources", "settings_simulation.json"
    ).replace("\\", "/")
    ...
    try:
        sensi_name = os.path.basename(sensi_path)
        settings_path = os.path.join(sensi_path, "resources", "settings.json").replace(
            "\\", "/"
        )
        settings_json = read_json_file(settings_path)
        settings_json["gen_param"]["name"] = "{}_{}".format(
            settings_json["gen_param"]["name"], sensi_name
        )
...
class SensiConfig:
        ...
        if len(sensi_dirs) > 0:
            ...
            for sensi in self.sensi_list.keys():
                ...
                if sensi in sensi_dirs.keys() and len(self.sensi_list[sensi]) > 0:
                    ...
                    res = create_dir_for_one_sensi_from_base(
                        sensi_dirs[sensi], self.base_dir
                    )
            ...
            for sensi in self.sensi_list.keys():
                if len(self.sensi_list[sensi]) > 0:
                    logger.debug(f"Creating table for sensi: {sensi}")
                    res = create_dir_for_one_sensi_from_base(
                        os.path.join(parent_dir, sensi), self.base_dir
                    )
                    processed_sensi_dirs[sensi] = res
    ...
    def _apply_stress_to_sensi(self, sensi_name, sensi_dirpath):
        ...
        Returns:
            (str or SensiIOError): The result of applying the stress to the sensitivity or an error message
        """
        ...
        for stress_name in self.sensi_list[sensi_name]:
            ...
            for command in self.param_map[stress_name]:
                ...
                try:
                    ...
                    logger.error(
                        f"Error parsing command {command} for stress {stress_name} to sensi {sensi_name}: {e}"
                    )
                    message = SensiIOError(
                        f"Unable to parse command {command} for stress {stress_name}"
                    )
                    break
                ...
                if syntax.expression.startswith("$"):
                    try:
                        path_to_file = syn.get_input_file_path(
                            self.settings_json, syntax.expression, sensi_dirpath
                        )
                        applied = syn.apply_syntax_to_file(
                            path_to_file, syntax, self.settings_json
                        )
                        if applied:
                            counter += 1
                            logger.debug(
                                'Applied "{}" with col: "{}", condition: "{}" and value: "{}" on input file {}'.format(
                                    syntax.expression,
                                    syntax.col,
                                    syntax.condition,
                                    syntax.value,
                            ...
                            message = 'Applied "{}" modification(s) on input files of {}'.format(
                                counter, sensi_name
                            )
                        else:
                            logger.error(
                                'Unable to apply expression "{}" on input file {}'.format(
                                    syntax.expression, path_to_file
                                )
                            )
                            message = SensiIOError(
                                "Failed to apply {} stress on {}".format(
                                    sensi_name, Path(path_to_file).name
                                )
                            )
                            break
                    except syn.SensiSyntaxError as e:
                        logger.error(
                            f"Unable to apply {syntax.expression} on {path_to_file}: {e}"
                        )
                        message = SensiIOError(
                            "Unable to apply {} stress on {}".format(
                                sensi_name, Path(path_to_file).name
                            )
                        )
                        break
                    ...
                    try:
                        table_name = syn.query(
                            self.settings_json, "$.framework.sensi_1.name"
                        )[0]
                        expression = syntax.expression
                        if not syntax.expression.startswith("framework"):
                            expression = "framework.sensi[{}].{}".format(
                                table_name, syntax.expression
                            )
                        ...
                        message = SensiIOError(
                            "Unable to add {} to settings_modif because no table name is found in settings_json".format(
                                syntax.expression
                            )
                        )
                        logger.error(
            ...
            logger.debug(
                f"Applied {counter} commands for stress {stress_name} to sensi {sensi_name}"
            )
        ...
        if len(settings_modif_express) > 0 and len(settings_modif_values) > 0:
            ...
            settings_modif_pd = pd.DataFrame(
                {"id": settings_modif_express, "value": settings_modif_values}
            )
        ...
        if not isinstance(message, SensiIOError):
            message = "Applied {} modification(s) on {}".format(
                total_applied, sensi_name
            )
    ...
    def apply(self, sensi_dirs={}):
        ...
        for sensi_name, sensi_dirpath in sensi_dirs_to_process.items():
            ...
            if not os.path.exists(sensi_dirpath):
                ...
                processed_sensi_messages[sensi_name] = SensiIOError(
                    "Sensitivity path does not exist"
                )
                continue
            ...
            processed_sensi_messages[sensi_name] = self._apply_stress_to_sensi(
                sensi_name, sensi_dirpath
            )
