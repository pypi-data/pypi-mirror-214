...
def test_read_json_successful():
    data = sio.read_json_file(
        f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/resources/settings.json"
    )
...
def test_read_json_invalid_file():
    with pytest.raises(ValueError):
        sio.read_json_file(
            f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/resources/invalid_settings.json"
        )
...
def test_find_file_in_directory_successful():
    ...
    assert (
        res
        == f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config.csv"
    )
...
def test_find_file_in_directory_with_target_dir_inexistent():
    assert (
        sio.find_file_in_directory("searched_file.csv", "inexistent_directory") is None
    )
...
def test_read_csv_from_filepath_successful():
    ...
    assert isinstance(
        sio.read_csv_from_filepath(sensi_config_input_csv), type(pd.DataFrame())
    )
...
def test_read_csv_from_filepath_with_non_allowed_extension():
    csv_file_input = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_with_non_allowed_extension.txt"
    with pytest.raises(ValueError):
        sio.read_csv_from_filepath(csv_file_input)


def test_read_csv_from_filepath_with_file_containing_unallowed_characters():
    csv_file_input = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_with_unallowed_characters.csv"
    with pytest.raises(ValueError):
        sio.read_csv_from_filepath(csv_file_input)


def test_read_csv_from_filepath_with_file_using_comma_as_delimiter():
    csv_file_input = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_with_comma_as_delimiter.csv"
...
def test_read_csv_from_filepath_with_file_containing_semicolons_in_values():
    csv_file_input = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_with_semicolons_in_values.csv"
...
def test_validate_sensi_config_with_incorrect_header():
    sensi_config_input_csv = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_incorrect_header.csv"
    res = sio.validate_sensi_config(sensi_config_input_csv)
    assert (
        res
...
def test_validate_sensi_config_with_missing_header():
    sensi_config_input_csv = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_missing_header.csv"
    res = sio.validate_sensi_config(sensi_config_input_csv)
    assert (
        res
...
def test_validate_sensi_config_with_extra_header():
    sensi_config_input_csv = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_extra_header.csv"
    res = sio.validate_sensi_config(sensi_config_input_csv)
    assert (
        res
...
def test_validate_sensi_config_with_incorrect_values_in_Apply_stress():
    ...
    assert (
        res
        == 'Sensitivity configuration file has the wrong values in "Apply stress". Rows with wrong values: [1, 3]'
    )
...
def test_validate_sensi_config_with_invalid_character():
    sensi_config_input_csv = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_with_invalid_char.csv"
    res = sio.validate_sensi_config(sensi_config_input_csv)
    assert res == "File is not a valid csv file"


def test_validate_sensi_config_with_invalid_separator():
    sensi_config_input_csv = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_with_invalid_sep.csv"
    res = sio.validate_sensi_config(sensi_config_input_csv)
    assert (
        res == 'File contains the delimiter "~" which is not allowed in sensi csv files'
    )


def test_validate_sensi_config_with_file_containing_semicolons_in_values():
    sensi_config_input_csv = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_config_with_semicolons_in_values.csv"
    res = sio.validate_sensi_config(sensi_config_input_csv)
    assert isinstance(res, type(pd.DataFrame()))
...
def test_copy_dir_existing_destination():
    ...
    try:
        ...
        assert not os.path.isfile(
            os.path.join(sensi_rsrc_dir, "existing_dir", "file.txt")
        )
    finally:
        shutil.rmtree(temp_dir)
...
def test_create_dir_for_one_sensi_from_base_successful(tmpdir):
    ...
    path_to_sensi = sio.create_dir_for_one_sensi_from_base(sensi_path, base_dir)
...
def test_create_dir_for_one_sensi_from_base_dir_inexistent():
    sensi_path = (
        f"{TEST_DIR}/Central_RN_Simulation_20201218_152857/sensitivities/Sensi_1"
    )
    base_dir = f"{TEST_DIR}/Central_RN_Simulation_20201218_152857_inexistent"
    res = sio.create_dir_for_one_sensi_from_base(sensi_path, base_dir)
