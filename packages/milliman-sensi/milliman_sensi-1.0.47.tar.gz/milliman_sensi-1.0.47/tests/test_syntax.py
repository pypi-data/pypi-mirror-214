...
def test_parse_param_successful():
    param_string = '''"file::eco[GBP].driver_1.data.swaptions.mkt['COL3'].where('COL1'==0,10,20 && 'COL2'>0 || 'COL4'<0)"=(+100)'''
...
def test_select_from_dataframe_less_than(test_dataframe):
    ...
    expected = pd.DataFrame({"Maturity": [1, 1, 30, 50],
                             ...
                             "Weigth": [1, 1, 1, 0],
                             "p_y": [1, 1, 1, 1]}).set_index([pd.Index([0, 1, 7, 8])])
    res = syn.select_from_dataframe(condition, operation, dataframe)
    assert(res.equals(expected))
...
def test_select_from_dataframe_greater_than(test_dataframe):
    ...
    operation = ">"
    dataframe = test_dataframe
    expected = pd.DataFrame({"Maturity": [2, 20, 20, 25],
                             "Tenor": [4, 5, 6, 7],
                             "Volatility": [0.004941, 0.005789, 0.006, 0.006529],
                             "Strike": [-0.020, 0.015, -0.005, 0.010],
                             "Weigth": [1, 1, 0, 0],
                             "p_y": [1, 1, 1, 1]}).set_index([pd.Index([3, 4, 5, 6])])
    res = syn.select_from_dataframe(condition, operation, dataframe)
    assert(res.equals(expected))
...
def test_select_from_dataframe_not_equal(test_dataframe):
    ...
    operation = "!="
    dataframe = test_dataframe
    expected = pd.DataFrame({"Maturity": [1, 1, 2, 20, 20, 25, 30, 50],
                             ...
                             "p_y": [1, 1, 1, 1, 1, 1, 1, 1]}).set_index([pd.Index([0, 1, 3, 4, 5, 6, 7, 8])])
    res = syn.select_from_dataframe(condition, operation, dataframe)
    assert(res.equals(expected))
...
def test_interpret_condition_greater_or_equal(test_dataframe):
    ...
    expected = pd.DataFrame({"Maturity": [50, 50],
                             ...
                             "Strike": [0.02, -0.02],
                             "Weigth": [0, 1],
                             "p_y": [1, 1]}).set_index([pd.Index([8, 9])])
    res = syn.interpret_condition(condition, dataframe)
    assert(res.equals(expected))
...
def test_interpret_condition_less_or_equal(test_dataframe):
    ...
    expected = pd.DataFrame({"Maturity": [1, 1],
                             ...
                             "Strike": [0.0, 0.0],
                             "Weigth": [1, 1],
                             "p_y": [1, 1]}).set_index([pd.Index([0, 1])])
    res = syn.interpret_condition(condition, dataframe)
    assert(res.equals(expected))
