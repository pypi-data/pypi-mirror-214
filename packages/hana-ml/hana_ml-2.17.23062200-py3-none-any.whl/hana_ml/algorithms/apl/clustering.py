...
class _AutoClusteringBase(APLBase):
    ...
    def _get_metrics(self, cond):
        """
        Returns metrics on the clustering quality.
        ...
        Dictionary or pandas DataFrame
            If no segment column is given, a dictionary with metric name as key and metric value
            as value.

            If a segment column is given, a pandas DataFrame which contains the metrics for each
            segment.

        Examples
        --------
        >>> model._get_metrics_by_cluster()
    ...
    def _get_metrics_by_cluster(self, cond, label=None):
        """
        Returns various metrics for each cluster.
        ...
        -------
        Dictionary or pandas DataFrame
            If no segment column is given, a nested dictionary:
            {'metric_name' : {'cluster_id': value}}.
        ...
        if segment_column is None:
            ...
            df_ind = df_ind[['OID', 'DETAIL', 'KEY', 'VALUE']]
            df_ind.columns = [segment_column, 'Cluster', 'Metric', 'Value']
            df_kl = self._get_cluster_kl()
            return pd.concat([df_kl, df_ind])

    def _get_cluster_kl(self):
        """
        Returns the Kullback-Leibler (KL) divergence between the cluster population and the
        overall population.
        ...
        -------
        Dictionary or pandas DataFrame
            If no segment column is given, a nested dictionary: {'cluster_id' : OrderedDictionary}
            where OrderedDictionary is {feature_name : KL_value}.

            If a segment column is given, a pandas DataFrame which contains the KL values for each
            segment.
        ...
        if segment_column is None:
            ...
            df_ind = df_ind[['OID', 'DETAIL', 'VARIABLE', 'KEY', 'VALUE']]
            df_ind.columns = [segment_column, 'Cluster', 'Variable', 'Metric', 'Value']
            return df_ind
...
class AutoUnsupervisedClustering(_AutoClusteringBase):
    ...
    def _predict_clusters(self, data, mode, nb_distances):
        ...
        if mode == 'closest_distances':
            ...
            for val_type in ['PredictedRankNodeId', 'PredictedRankDistances']:
                ...
                if nb_distances == 'all':
                    apply_config_data.append((param_path, 'all', None))
                else:
                    apply_config_data.append((param_path, 'individual', None))
                    apply_config_data.append((param_path + '/' + str(nb_distances), '', None))
    ...
    def get_metrics(self):
        ...
        Dictionary or pandas DataFrame
            If no segment column is given, a dictionary object containing a set of clustering
        ...
        metrics = self._get_metrics(cond)

        cond = "VARIABLE='clusterId' and TARGET='clusterId' and DETAIL is not null"
        metrics_by_cluster = self._get_metrics_by_cluster(cond=cond)

        segment_column = getattr(self, 'segment_column_name', None)
        if segment_column is None:
            metrics.update(metrics_by_cluster)
        else:
            metrics = pd.concat([metrics_by_cluster, metrics])
            metrics.sort_values(by=[segment_column, 'Cluster', 'Variable'], inplace=True,
                                na_position='first', ignore_index=True)
            metrics['Cluster'].replace([np.nan], [None], inplace=True)
            metrics['Variable'].replace([np.nan], [None], inplace=True)
...
class AutoSupervisedClustering(_AutoClusteringBase):
    ...
    def get_metrics(self):
        ...
        metrics_by_cluster = self._get_metrics_by_cluster(cond=cond, label=self.label)
