...
class HANAGeneratorForCAP(object):
    ...
    def _generate_base_predict_procedure(self, obj, cds_gen=False, model_position=None):
        ...
        self._base_predict_proc_filename = self.namespace.replace(".", "-") + "-" + self._base_predict_proc_name.replace("_", "-") + ".hdbprocedure"
        self._cons_predict_proc_filename = self.namespace.replace(".", "-") + "-" + self._cons_predict_proc_name.replace("_", "-") + ".hdbprocedure"
                ...
                if model_position:
                    if model_position['in'] == idx:
                        ...
                        if self._materialize_ds_data:
                            ...
                            self._cdses[context].append(cds)
                        # else:
                        #     hdbtable, hdbview, cds = cds_convert.create_cds_artifacts(data=inp, namespace=self.namespace, context=context, entity=entity, annotation=['@cds.persistence.exists'], hdbtable_name="{}::{}.{}".format(self.namespace, context.capitalize(), entity.capitalize()))
                        #     if self._tudf:
                        #         self._cdses[context].append(cds)
        ...
        count = 0
        for output in obj._predict_output_table_names:
            output_tt = []
            output_df = conn.table(output)
            output_df = _handle_reserved_words(output_df)
            for key, val in output_df.get_table_structure().items():
        ...
        if self._tudf:
            ...
            #     self._hdbviews[self._consume_tudf[0].replace('_', '-').lower() + '.hdbview'] = "VIEW {} AS {};".format(self._consume_tudf[0], first_sel)
        ...
        function_inputs = "\nin_0 = {};".format(first_sel) + "\nin_1 = SELECT * FROM {};".format(self._consume_tudf[1])
    ...
    def _generate_base_score_procedure(self, obj, cds_gen=False, model_position=None):
        ...
        self._base_score_proc_filename = self.namespace.replace(".", "-") + "-" + self._base_score_proc_name.replace("_", "-") + ".hdbprocedure"
        self._cons_score_proc_filename = self.namespace.replace(".", "-") + "-" + self._cons_score_proc_name.replace("_", "-") + ".hdbprocedure"
    ...
    def generate_artifacts(self, obj, cds_gen=False, model_position=None, tudf=False):
        """
        ...
        with open(os.path.join(db_path, synonym_filename), "w+") as hdbsynonymfile:
        ...
        for kkey, vval in self._cdses.items():
            if len(vval) > 0:
                ...
                for cds in vval:
                    cds_content += cds + "\n"
                cds_content += "}"
        cds_filename = self.namespace.replace(".", "-") + "-cds-{}".format(self._base_fit_proc_name.replace("_", "-"))
...
class HanaGenerator(object):
    ...
    generation_merge_type : int, optional
        ...
        - 2: GENERATION_MERGE_PARTITION: A partition operation is merged into the respective
          related operation and generated as 1 (i.e. prodedure in HANA).
