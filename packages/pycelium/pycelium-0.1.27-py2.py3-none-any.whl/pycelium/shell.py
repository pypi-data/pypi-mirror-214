...
def jinja2regexp(template, repl=r"(?P<\2>\\w+)"):
    ...
    param = r"(\{\{\s+(?P<key>[^\s}]+)\s*\}\})"
    # repl = r'(?P<\2>' + wildcard + ')'
    pattern = re.sub(param, repl, template, count=0, flags=0).replace(" ", "\s")
...
class Executor:  # TODO Used??
    ...
    async def expect(self, cmdline, default_match_prefix=".*\n", **reactions):
        """
...
class Handler:
    ...
    def get_hook(self, state):
        ...
        if func is None:
            ...
            if candidates:
                best = max(candidates)
                self.hooks[blueprint] = func = getattr(self, candidates[best])
        return func
...
class STM:
    ...
    def my_tasks(self, skip_internal=True):
        if skip_internal:
            internals = set([id(f.__func__.__code__.co_code) for f in (self.main,)])
        else:
            internals = set()
    ...
    async def main(self):
        ...
        while self.running:
            ...
            if self.queue._queue:
                ...
                # event auto-stimulation
                auto_event = auto.intersection(self.allow_events.get(self.state, {}))
                params = {}
            ...
            for d in transitions.values():
                ...
                for self.new_state, callbacks in d.items():
                    ...
                    # dispatch func
                    await self.dispatch("do", func1, func2, *callbacks, **params)
                ...
                for (
                    ...
                    if re.match(_event, event):
                        for (
                            ...
                            preconditions = [
                                eval(exp.format_map(ctx), ctx) for exp in preconditions
                            ]
                            if not preconditions or all(preconditions):
                                lambdas = [func.format_map(ctx) for func in lambdas]
                                if lambdas:
                                    foo = 1
                                candidates.setdefault(_state_a, {}).setdefault(
                                    _state_b, []
                                ).extend(lambdas)
    ...
    def add_transition(
        ...
        # direct map
        holder = self.transitions.setdefault(state_a, {}).setdefault(event, [])

        transition = [state_b, preconditions, lambdas]
        if transition not in holder:
            holder.append(transition)
        else:
            print(f"transition {transition} already added for {state_a} : -> {event}")
    ...
    async def _enter_boot(self, *args, **kw):
        ...
        self.log.debug(f"{self.__class__.__name__}: _enter_boot: '{self.name}'")
        self.ctx["sleep"] = 1
    ...
    async def do_sequential(self, *args, **kw):
        ...
        for topic, sequence in lines.items():
            ...
            self.reactor.new_fiber(self._sequential_execution(topic, sequence))
...
class DefaultExecutor(ShellSTM):
            ...
            try:
                for cmd in commands:
                    ...
                    if process.exit_status != 0:
                        self.log.warning(f"Executing: '{cmd}' FAILED: {output}")
                        break
    ...
    async def put(self, source, target):
        ...
        self.log_commands.info(f"PUT {source} ---> {self.connection._host}:{target}")
    ...
    async def get(self, source, target):
        ...
        self.log_commands.info(f"GET {target} <--- {self.connection._host}:{source}")
        try:
    ...
    async def do_Disconnect(self):
        ...
        if self.trying:
            if self.retry > 0:
                self.log.debug(f"Last conntecion attempt was failed. Retraying again.")
            else:
                self.log.debug(
        ...
        if self.connection:
            self.log.debug(f"ignoring duplicated {self.EV_CONNECT} events ...")
            return
        ...
        try:
            ...
            self.connection = await asyncssh.connect(known_hosts=None, **params)
        ...
        c = self.connection  # <class 'asyncssh.connection.SSHClientConnection'>
