...
strategy_option = Annotated[str, typer.Option(help="strategy to run asdasdf")]
...
def run(
    ...
    broker = Broker(broker_url, local_host)
    ...
    if strategy:
        try:
            stored_strategy = broker.http.strategy.get(strategy)
        except Exception:
    ...
    with std.status("Execution.."):
        try:
            ...
            backtest = broker.http.backtest.run(stored_strategy["name"])
