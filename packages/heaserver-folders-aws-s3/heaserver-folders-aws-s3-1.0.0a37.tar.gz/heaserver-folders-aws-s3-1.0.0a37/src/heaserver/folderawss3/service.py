...
async def put_folder(request: web.Request) -> web.Response:
    ...
    if key != folder.key:
        ...
        activity.created = datetime.now().astimezone()
        ...
        try:
            ...
            activity.modified = datetime.now().astimezone()
...
async def _move_folder(request: web.Request) -> web.Response:
    ...
    if not awsservicelib.is_folder(key):
        return response.status_bad_request(f'Object {key} not a folder')
    try:
        _, target_bucket_id, target_key = await awsservicelib._copy_object_extract_target(await request.json())
