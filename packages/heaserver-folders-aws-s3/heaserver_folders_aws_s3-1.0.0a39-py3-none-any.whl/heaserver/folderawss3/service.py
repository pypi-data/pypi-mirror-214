...
async def get_items(request: web.Request) -> web.Response:
    ...
    try:
        ...
        activity = DesktopObjectAction()
        activity.generate_application_id()
        activity.code = 'hea-get'
        activity.owner = request.headers.get(SUB)
        activity.user_id = activity.owner
        activity.description = f'Listing all items in {awsservicelib._activity_object_display_name(bucket_name, folder_key)}'
        await publish_desktop_object(request.app, activity)
...
async def post_folder_mover(request: web.Request) -> web.Response:
    ...
    responses:
      ...
      '404':
        $ref: '#/components/responses/404'
    """
    return await _move_folder(request)
...
async def post_folder_uploader(request: web.Request) -> web.Response:
    ...
    """
    logger = logging.getLogger(__name__)
    look_at_me = await _post_file_object_in_folder(request)
    logger.info(f"final response's location {look_at_me.headers.get(hdrs.LOCATION)}")
    return look_at_me
...
async def put_folder(request: web.Request) -> web.Response:
    ...
    if key != folder.key:
        ...
        activity = DesktopObjectAction()
        activity.generate_application_id()
        activity.code = 'hea-update'
        activity.owner = request.headers.get(SUB)
        activity.user_id = activity.owner
        activity.description = f'Renaming {awsservicelib._activity_object_display_name(bucket_id, key)} to {folder.key}'
        await publish_desktop_object(request.app, activity)
        try:
            ...
            await publish_desktop_object(request.app, activity)
            return awsservicelib.handle_client_error(e)
        except ValueError as e:  # Getting the s3_client failed, or other internal server error.
            activity.status = Status.FAILED
            await publish_desktop_object(request.app, activity)
            raise e
...
async def get_folders(request: web.Request) -> web.Response:
    ...
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Listing all folders in bucket {bucket_name}'
    await publish_desktop_object(request.app, activity)
...
async def _move_folder(request: web.Request) -> web.Response:
    try:
        volume_id = request.match_info['volume_id']
        bucket_id = request.match_info['bucket_id']
        id_ = request.match_info.get('id')
    ...
    activity = DesktopObjectAction()
    activity.generate_application_id()
    activity.code = 'hea-move'
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.description = f'Moving {awsservicelib._activity_object_display_name(bucket_id, key)} to {awsservicelib._activity_object_display_name(target_bucket_id, target_key)}'
    await publish_desktop_object(request.app, activity)
    try:
        s3_client: S3Client = await request.app[HEA_DB].get_client(request, 's3', volume_id)
        activity.status = Status.IN_PROGRESS
...
async def _delete_folder(request: web.Request, recursive=False) -> web.Response:
    ...
    """
    return await awsservicelib.delete_object(request, recursive=recursive, activity_cb=publish_desktop_object)
...
async def _post_folder_in_parent(request: web.Request) -> web.Response:
    ...
    activity = DesktopObjectAction()
    activity.generate_application_id()
    activity.description = f'Creating folder {folder.display_name} in bucket {bucket_id}'
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    activity.code = 'hea-create'
    await publish_desktop_object(request.app, activity)
...
async def _post_object(request: web.Request, item: AWSS3FolderFileItem) -> web.Response:
    ...
    try:
        ...
        item_folder_id = awsservicelib.decode_folder(folder_id)

        if folder_id != awsservicelib.ROOT_FOLDER.id and not awsservicelib.is_folder(item_folder_id):
            item_folder_id = None
    except KeyDecodeException:
        item_folder_id = None
    ...
    activity.generate_application_id()
    activity.code = 'hea-create'
    activity.description = f'Creating item {item.display_name} in bucket {bucket_id}'
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    await publish_desktop_object(request.app, activity)
...
async def _post_file_object_in_folder(request: web.Request) -> web.Response:
    """
    Creates a new object in a bucket. The volume id must be in the volume_id entry of the request.match_info dictionary.
    The bucket id must be in the bucket_id entry of request.match_info. The folder id must be in the folder_id entry of
    request.match_info.
    ...
    :param request: the HTTP request (required).
    :param obj: the heaobject.folder.AWSS3FileFolderItem to create (required).
    :return: the HTTP response, with a 201 status code if successful with the URL to the new item in the Location
    header, 403 if access was denied, 404 if the volume or bucket could not be found, or 500 if an internal error
    occurred.
    """
    ...
    activity.generate_application_id()
    activity.code = 'hea-create'
    activity.description = f'Creating item {obj.display_name} in bucket {bucket_id}'
    activity.owner = request.headers.get(SUB)
    activity.user_id = activity.owner
    await publish_desktop_object(request.app, activity)
    ...
    try:
        ...
        response_ = await loop.run_in_executor(None, partial(s3_client.head_object, Bucket=bucket_id, Key=obj_name))
        activity.status = Status.COMPLETED
        await publish_desktop_object(request.app, activity)
