...
# %% auto 0
__all__ = ['Spectra', 'SpectraTfm', 'ToAbsorbanceTfm', 'SpectraBlock', 'RandWAvgTfm', 'AvgTfm', 'AvgBatchTfm', 'SNVTfm',
           'NormalizeTfm', 'TrimTfm', 'DerivTfm', 'GADFTfm', 'GADFBatchTfm', 'ResizeTfm', 'To3DTfm', 'RandomCropTfm',
           'Analytes', 'AnalytesTfm', 'AnalytesBlock', 'LogTfm', 'asTabular']
...
class GADFTfm(Transform):
    ...
    def rescale(self, x):
        m, M = x.min(), x.max()
        return ((x - M) + ( - m)) / (M - m) if self.neg else (x - m) / (M - m)
...
class GADFBatchTfm(Transform):
    ...
    def encodes(self, 
        ...
        return (torch.matmul(torch.transpose(K, 1, 2), X) - 
                torch.matmul(torch.transpose(X, 1, 2), K))
