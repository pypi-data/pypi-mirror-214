...
def inspect_format(path=None, content=None):
    ...
    fmt, zf, component_names = None, None, None
    ...
    if peek.startswith(ZIP_SIGNATURE):
        ...
        if 'xl/workbook.xml' in component_names:
            fmt = 'xlsx'
        elif 'xl/workbook.bin' in component_names:
            fmt = 'xlsb'
        elif 'content.xml' in component_names:
            fmt = 'ods'
        else:
            fmt = 'zip'
...
def open_workbook(filename=None,
    ...
    """
    fmt, zf, component_names = inspect_format(filename, file_contents)
    ...
    if fmt and fmt not in ('xls', 'xlsx'):
        raise XLRDError(FILE_FORMAT_DESCRIPTIONS[fmt]+'; not supported')
