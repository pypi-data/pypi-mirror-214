...
def step_impl_response_inbound(context,stream_name):
    ...
    assert list(expected_result.columns)==list(context.sync_inbound_result.columns),f"Column headings didn't match. Expected: {expected_result.columns}, actual: {context.apply_result.columns}"
    pandas.set_option('display.max_columns', 10)
    pandas.set_option('display.width', 150)
    print(f"expected_result: {expected_result}")
