...
def add_map(ax,add_county_line = False,add_worldmap = True,title = None,sup_fontsize = 12,linewidth = [0.3,0.3,0.2],color = ["k","k","k"]):
    ...
    if meteva_base.customized_basemap_list is None:
        if add_worldmap:
            meteva_base.tool.plot_tools.add_china_map_2basemap(ax, name="world", edgecolor='k', lw=0.3, encoding='gbk', grid0=None)  # "国界"
        meteva_base.tool.plot_tools.add_china_map_2basemap(ax, name="nation", edgecolor='k', lw=0.3,encoding = 'gbk')  #"国界"
        meteva_base.tool.plot_tools.add_china_map_2basemap(ax, edgecolor='k', lw=0.3,encoding = 'gbk')  #"省界"
        if add_county_line:
            meteva_base.tool.plot_tools.add_china_map_2basemap(ax, name="county", edgecolor='k', lw=0.2, encoding='gbk')  # "省界"
        ax.set_title(title,fontsize = sup_fontsize* 0.9)
        return ax
    else:
        for i in range(len(meteva_base.customized_basemap_list)):
            shpfile  = meteva_base.customized_basemap_list[i]
            # print(shpfile)
            encoding = 'utf-8'
            try:
                shp1 = meteva_base.tool.plot_tools.readshapefile(shpfile, default_encoding=encoding)
                lines = meteva_base.tool.plot_tools.LineCollection(shp1, antialiaseds=(1,), zorder=100)
...
def add_contourf(ax,grd,cmap ="rainbow",clevs= None,add_colorbar = True,cut_colorbar = True,title = None,title_fontsize = 8,clip = None):
    ...
    grid1 = meteva_base.grid([slon,elon,grid0.dlon],[slat,elat,grid0.dlat])
    grd1 = meteva_base.interp_gg_linear(grd,grid1,outer_value=np.nan)
    ...
    cmap1,clevs1 = meteva_base.tool.color_tools.def_cmap_clevs(cmap=cmap,clevs=clevs,vmin=vmin,vmax = vmax,cut_colorbar = cut_colorbar)
    ...
    if clip is not None:
        try:
            meteva_base.tool.maskout.shp2clip_by_shpfile(im,ax,clip)
        except:
            if isinstance(clip, str): clip = [clip]
            if isinstance(clip[0], str):
                meteva_base.tool.maskout.shp2clip_by_region_name(im, ax, clip)
            else:
                meteva_base.tool.maskout.shp2clip_by_lines(im, ax, clip)
...
def add_mesh(ax,grd,cmap ="rainbow",clevs= None,add_colorbar = True,title = None,title_fontsize = 8):
    ...
    vmax = np.nanmax(grd1.values)
    vmin = np.nanmin(grd1.values)
    cmap1,clevs1 = meteva_base.tool.color_tools.def_cmap_clevs(cmap=cmap,clevs=clevs,vmin=vmin,vmax = vmax)
    norm = BoundaryNorm(clevs1, ncolors=cmap1.N-1)
...
def add_barbs(ax,wind,color = "k",skip = None,title = None,title_fontsize = 8,length = None):
    ...
    grid0 = meteva_base.get_grid_of_data(wind)
    grid1 = meteva_base.grid([slon,elon,grid0.dlon],[slat,elat,grid0.dlat])
    wind1 = meteva_base.interp_gg_linear(wind,grid1,outer_value=np.nan)
...
def add_scatter(ax,sta0,cmap = "rainbow",clevs = None,point_size = None,fix_size = True,threshold = 2,min_spot_value = 0,mean_value = 2,
    ...
    sta_without_iv = meteva_base.sele.not_IV(sta)
    data_names = meteva_base.get_stadata_names(sta_without_iv)
    ...
    cmap1, clevs1 = meteva_base.tool.color_tools.def_cmap_clevs(cmap=cmap, clevs=clevs, vmin=vmin_v, vmax=vmax_v)
    ...
    if point_size is None:
        ...
        sta_dis = meteva_base.sta_dis_ensemble_near_by_sta(sta_id1, nearNum=2)
...
def add_scatter_text(ax,sta0,color = "k",cmap = None,clevs = None,tag = 2,
    ...
    sta = meteva_base.sele_by_para(sta0,lon=[x0,x1],lat=[y0,y1])
    sta_without_iv = meteva_base.sele.not_IV(sta)
    data_names = meteva_base.get_stadata_names(sta_without_iv)
    sta_without_iv = sta_without_iv.sort_values(by=data_names[-1], ascending=True)
...
def add_lines(ax,graphy,cent = None,color = "k",linestyle = "-",linewidths = None,title = None,title_fontsize = 8):
    ...
    if isinstance(graphy,dict):
        ...
        df_list = meteva_base.split(graphy,used_coords=["level","time","dtime","id"])
        for df in df_list:
            lon = df["lon"].values
            lat = df["lat"].values
            ...
            if cent is not None:
                ...
                cent1 = meteva_base.sele_by_para(cent,level = level,time = time,dtime = dtime,id = id)
