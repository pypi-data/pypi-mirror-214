...
def launch_sidechain_task_menu():
    ...
    if (selection_text == 'Build witness server'):
        print("\nIn order to '{}', run: 'claudia witness build --repo <absolute_path_to_local_repo>'".format(selection_text))
        if (get_confirmation("Are you sure you want to run this command?")):
            relaunch_wizard = False
            repo_path = get_valid_repo_path()
            if (repo_path == "" or repo_path.replace("'", "").replace('"', '').lower() == 'back'):
        ...
        if (get_confirmation("Are you sure you want to run this command?")):
            ...
            network = inquirer.prompt(
                [inquirer.List("confirmation", message="Please choose network type. Current Selection",
                               choices=["local-mainnet", "devnet", "testnet", "local-sidechain"], default="local-mainnet")],
                theme=GreenPassion()
            )['confirmation']
...
def launch_misc_task_menu():
    ...
    elif (selection_text == 'Enable a feature'):
    ...
    elif (selection_text == 'Disable a feature'):
...
def launch_command_run_e2e_tests(lib, client_type, network, tag, feature, invalidate_cache):
    if(network == "local-sidechain"):
        print("You have requested to run the tests on local-sidechain. If this is the first time you are running this on this network, please run the following manually:")
        print("\n\txbridge-cli bridge create-account --from-locking --bridge sidechain --from snoPBrXtMeMyMHUVTgbuqAfg1SUTb --to rh1HPuRVsYYvThxG2Bs1MfjmrVC73S16Fb --amount 10000000000 --no-close-ledgers")
        input("Without this, the tests will fail! When ready, press enter to continue...")
    if (lib == 'py'):
        if (invalidate_cache != 'false'):
