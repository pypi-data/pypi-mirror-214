...
XCHAIN_BRIDGE_BOOTSTRAP_RELATIVE_FILE_PATH="${CONFIGS_DIR_NAME}/sidechain/bridge_bootstrap.json"
XCHAIN_CLI_CONFIG="$HOME/.config/xbridge-cli/config.json"
...
print_time_elapsed() {
  echo "  Time elapsed: $(($SECONDS / 3600))hr $((($SECONDS / 60) % 60))min $(($SECONDS % 60))sec"
}
  ...
  if [ "${l_build_witness_opt}" = "true" ]; then
    ...
    if [ ! "$(docker ps -aq --filter name=${WITNESS_BUILD_CONTAINER_NAME})" ] || [ "${REMOUNT_VOLUME}" = "true" ]; then
      ...
      docker run \
        ...
        -v "${RIPPLED_HOME_DIR}":"${RIPPLED_BUILD_CONTAINER_RIPPLED_HOME}" \
        -v "${host_script_dir}":"${RIPPLED_BUILD_CONTAINER_SCRIPT_DIR}" \
        -v "${LOG_BASE_DIR}":"${RIPPLED_BUILD_CONTAINER_LOG_DIR}" \
        "${DOCKER_IMAGE}" > /dev/null 2>&1
...
start_sidechain() {
  ...
  if [ "${l_start_sidechain_opt}" = "true" ]; then
    if [ "${MODE}" = "${build}" ]; then
      ...
      create_xchain_bridge true "${network}" "${host_script_dir}"
    else
...
create_xchain_bridge() {
  l_create_xchain_bridge="$1"
  network="$2"
  host_script_dir="$3"

  if [ "${l_create_xchain_bridge}" = "true" ]; then
    echo "- Create xchain bridge"

    rm -f "${XCHAIN_CLI_CONFIG}"
    bootstrap_file_path="${host_script_dir}/${XCHAIN_BRIDGE_BOOTSTRAP_RELATIVE_FILE_PATH}"
    bridge_name="${network}"
    time_now="$(date +%Y%m%d_%H%M%S)"
    xbridge-cli bridge build \
      --name "${bridge_name}" \
      --bootstrap "${bootstrap_file_path}" \
      --funding-seed "${MASTER_ACCOUNT_SEED}" \
      --fund-locking \
      --no-close-ledgers -v > "${LOG_DIR}/${time_now}_xbridge_create.log" 2>&1
    exit_on_error $?
  fi
}
