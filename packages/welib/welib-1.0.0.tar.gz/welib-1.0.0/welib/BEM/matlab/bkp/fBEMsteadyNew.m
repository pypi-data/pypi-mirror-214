function [BEM] = fBEMsteadyNew(WT,Sim,Wind,Algo)
cone=WT.Rotor.cone;
...
for i=1:nbIt % !!!!!!!!!!!!!!!!!!! in this order for my tip loss correction
    ...
    if(Algo.TipLoss)
        % tip loss correction
        IphiPos=abs(sind(phi))>0.01; %singularity if sin phi close to zero
        if(isequal(Algo.TipLossMethod,'Glauert'))
            Ftip(IphiPos)=2/pi*acos(exp(-nB/2*(R-r(IphiPos))./(r.*sind(phi(IphiPos)))));
        elseif(isequal(Algo.TipLossMethod,'Prandtl'))
            Ftip=2/pi*acos(exp(-nB/2*(1-lambda_r/lambda)*sqrt(1+lambda^2 )));
        elseif(isequal(Algo.TipLossMethod,'GoldsteinSimple'))
            Ftip=interp1(Vr,FGoldstein,r);
            Ftip(Ftip>1)=1;
        elseif(isequal(Algo.TipLossMethod,'XuSankar'))
            IphiPos07=abs(sind(phi))>0.01 & r>=0.7; %singularity if sin phi close to zero
            IphiPos00=abs(sind(phi))>0.01 & r<0.7; %singularity if sin phi close to zero
            FGl=2/pi*acos(exp(-nB/2*(R-r(IphiPos07))./(r(IphiPos07).*sind(phi(IphiPos07)))));
            Ftip(IphiPos07)=0.5*(FGl^0.86+0.5);
            i07=whichvalue(r,0.7*R);
            FGl=2/pi*acos(exp(-nB/2*(R-0.7*R)/(0.7*R*sind(BEM.phi(i07)))));
            Ftip(IphiPos00)=1-r(IphiPos00)/R*(1-FGl)/0.7 ;
        elseif(isequal(Algo.TipLossMethod,'Lindenburg'))
            Ftip=2/pi*acos(exp(-nB/2*(1-lambda_r/lambda).*sqrt(1+lambda_r.^2.*( (1+2*sqrt(Ftip_previous).*aprime/2)./(1-sqrt(Ftip_previous).*a/2) ).^2    )));
        elseif(isequal(Algo.TipLossMethod,'Shen'))
            Ftip=2/pi*acos(exp(-nB/2*(R-r(IphiPos))/(r*sind(phi(IphiPos)))));
            g=exp(-0.125*(nB*lambda-21))+0.1;
            Fshen  =2/pi*acos(exp(-nB/2*(R-r(IphiPos))./(r(IphiPos)*sind(phi(IphiPos))) *g));
...
%  vSSE2=zeros(1,size(M,1));
%     for j=1:size(M,1);
%         [sse Fitted_Curve2]=fFitGamma2(M(j,1:4),rr,Gamma);
%         vSSE2(j)=sse;
%     end
%     params=M(whichmin(vSSE2),1:4);
%     [x0 x2 y3 t0;x0s x2s y3s t0s;params]
%     [sse Fitted_Curve2]=fFitGamma(params,rr,Gamma);
%     plot(rr,Fitted_Curve2,'g');
%     ii=whichvalue(r,min(x0,0.25));
%     vSSE(i)=min(vSSE2);
