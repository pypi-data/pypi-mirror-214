...
global Aero Rotor Algo Environment Profiles Controller
cone=Rotor.cone;

V0=Aero.Wind.V0(3)*cosd(cone);
VHubHeight=Aero.Wind.V0(3);
...
for e=ne:-1:1 % !!!!!!!!!!!!!!!!!! backward for Xu and Sankar correction
    ...
    for i=1:nbIt
        ...
        if(~isequal(Algo.Format,'flex'))
            ClCdCm= fAeroCoeff(alpha,Profiles,Rotor.ProfileSet(:,e),Rotor.thickness_rel(e),Re,bReInterp);
            Cl=ClCdCm(1);
            Cd=ClCdCm(2);
...
BEM.ThrLoc=Rotor.dr.*Pn*cosd(cone);
BEM.ThrLocLn=Pn*cosd(cone);
BEM.CTloc=nB*BEM.ThrLoc./(0.5*rho*VHubHeight^2*(2*pi*r.*cosd(cone).*Rotor.dr)) ;

BEM.TqLoc=Rotor.dr.*r.*Pt*cosd(cone);
BEM.TqLocLn=r.*Pt*cosd(cone);
BEM.CQloc=nB.*BEM.TqLoc./(0.5*rho*VHubHeight^2*(2*pi*r.*cosd(cone).*Rotor.dr.*r*cosd(cone))) ;
...
BEM.Thrust = nB*sum(Rotor.dr.*(Pn*cosd(cone)));    %Rotor shaft thrust at t in Newton
BEM.Torque = nB*sum(Rotor.dr.*Pt.*(r*cosd(cone))); %Rotor shaft torque at t in Newton
BEM.Flap   = sum(Rotor.dr.*(Pn*cosd(cone)).*(r-rhub));% approximate
BEM.Edge   = sum(Rotor.dr.*Pt.*(r*cosd(cone)).*(r-rhub));% approximate
