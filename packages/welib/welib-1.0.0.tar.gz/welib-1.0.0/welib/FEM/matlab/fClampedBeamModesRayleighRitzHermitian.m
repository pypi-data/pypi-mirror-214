function [f,x,U,V,K,MM,KK]=fClampedBeamModesRayleighRitzHermitian(x0,EI0,m0,nel,nModes,gravity,Mtop,xLumped,MLumped,bStiffeningMtop,bStiffeningSelfWeight)
...
if nargin==0
    ...
    Mtop=1*m*L;
    [f,x,U,V,K,MM,KK]=fClampedBeamModesRayleighRitzHermitian(L,EI,m,nel,nModes,9.81,Mtop,[],[],0,0);
    MM
    KK
    x_th=x;
    [f_th,~,U_th] = fUniformBeamTheory('transverse-unloaded-topmass-clamped-free',EI,m,1,L,'x',x_th,'norm','tip_norm','Mtop',Mtop);
    %% Comparison of Frequencies
    for i=1:min(2,nModes)
...
if nModes>2
    error('Rayleigh Ritz with Hermitian mode shapes is limited to two modes max');
end
...
L=max(x0);
%
% Hermitian mode shapes
U_th(1,:)= 3*x.^2/L^2 - 2*x.^3/L^3;
U_th(2,:)=  -x.^2/L   +   x.^3/L^2;
V_th(1,:)= 6*x   /L^2 - 6*x.^2/L^3;
V_th(2,:)=-2*x   /L   + 3*x.^2/L^2;
K_th(1,:)= 6           /L^2 -12*x   /L^3;
K_th(2,:)=-2           /L   + 6*x   /L^2;
...
for i=1:nModes
    for j=1:nModes
        MM0(i,j) = trapz(x, m    .* U0(i,:).*U0(j,:));
        KK0(i,j) = trapz(x, EI   .* K0(i,:).*K0(j,:));
