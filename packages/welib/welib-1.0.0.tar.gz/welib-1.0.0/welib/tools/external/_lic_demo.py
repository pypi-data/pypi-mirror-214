...
if video:
    ...
    ##cs=ax.contourf(X,Y,new_image,30,cmap=cmap)
    #yseed=np.linspace(-0.99,0.99,7)
    #start=np.array([yseed*0,yseed])
    #ax.streamplot(xs,ys,vectors[:,:,0],vectors[:,:,1],color='k',start_points=start.T)
    plt.show()
