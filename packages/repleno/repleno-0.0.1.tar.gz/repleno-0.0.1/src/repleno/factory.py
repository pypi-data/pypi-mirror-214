...
class Factory:
    ...
    def add_parent_child_relationship(
        ...
        child,
        qty=1,
        parent_unit_of_measure=None,
        child_unit_of_measure=None,
    ):
        ...
        if parent_unit_of_measure is not None:
            parent_item.unit_of_measure = parent_unit_of_measure
        if child_unit_of_measure is not None:
            child_item.unit_of_measure = child_unit_of_measure
    ...
    def load_bill_of_materials(
        ...
        Args:
            ...
            with the below columns names or a list of dictionary with the below key names:
                ...
                2. "child_code"
                3. "qty": optional
                4. "parent_unit_of_measure": optional
                5. "child_unit_of_measure": optional
            ...
            in the data are not the default ones. Default to an empty dictionary.
            e.g.:
                {
                    "parent_code": "<field_name_in_data_with_parent_code>",
        ...
        mandatory_field_mapping = {
            "parent_code": field_mapping.get("parent_code", "parent_code"),
            "child_code": field_mapping.get("child_code", "child_code"),
        }

        optional_field_mapping = {
            "qty": field_mapping.get("qty", "qty"),
            "parent_unit_of_measure": field_mapping.get(
                "parent_unit_of_measure", "parent_unit_of_measure"
            ),
            "child_unit_of_measure": field_mapping.get(
                "child_unit_of_measure", "child_unit_of_measure"
            ),
        }
        ...
        if isinstance(data, list) and data:
            for i in data:
                ...
                self.add_parent_child_relationship(
                    ...
                    child,
                    i.get("qty"),
                    i.get("parent_unit_of_measure"),
                    i.get("child_unit_of_measure"),
                )
    ...
    # todo: allow to load from a csv file as well
    def load_adjacency_list_bom(self, data):
        """
        Load data in the following format

        [{"id": 0, "part_number": "A", "child_ids": "[1, 5, 6]"},
        {"id": 1, "part_number": "B", "child_ids": "[4]"},
        {"id": 2, "part_number": "B1", "child_ids": "[1, 3]"},
        {"id": 3, "part_number": "B2", "child_ids": "[4]"},
        {"id": 4, "part_number": "B3", "child_ids": "[]"},
        {"id": 5, "part_number": "C",, "child_ids": "[]"},
        {"id": 6, "part_number": "D", "child_ids": "[]"},
        {"id": 7, "part_number": "D1", "child_ids": "[6]"}]
        """

        # store ids and codes in a dictionary for speed
        ids_dict = {i["id"]: i["part_number"] for i in data}

        for r in data:
            parent = self._get_item_or_create(r["part_number"])
            child_ids = [int(c_id) for c_id in r["child_ids"][1:-1].split(",") if c_id]
            child_item_codes = [ids_dict[c_id] for c_id in child_ids]

            [
                self.add_parent_child_relationship(
                    parent=parent,
                    child=self._get_item_or_create(child_item_code),
                )
                for child_item_code in child_item_codes
            ]

    # todo: allow to load from a csv file as well
    def load_short_adjacency_list_bom(self, data):
        """
        Load data in the following format

        [{"id": "A", "child_ids": "[B, C, D]"},
        {"id": "B", "child_ids": "[B3]"},
        {"id": "B1", "child_ids": "[B2, B]"},
        {"id": "B2", "child_ids": "[B3]"},
        {"id": "D1", "child_ids": "[D]"}]
        ...
        for r in data:
            for c in r["children"][1:-1].split(","):
                if c:
                    self.add_parent_child_relationship(
                        parent=r["part_number_id"],
                        child=c,
                    )
    ...
    def load_bill_of_materials(
        ...
        optional_field_mapping = {
            ...
            "parent_unit_of_measure": mapping.get(
                "parent_unit_of_measure", "parent_unit_of_measure"
            ),
            "child_unit_of_measure": mapping.get(
                "child_unit_of_measure", "child_unit_of_measure"
            ),
        }
    ...
    def get_phase_out_final_orders(self, item_code):
        """
        Assumes your goal is to fully deplete all the inventory affected by this
        specific phase-out item
        """

        # get the tree that will be obsoleted in the bill of materials
        pass

        # get a list of all the lowest items in this tree

        # for each lowest item, get the inventory on hand
        # self_compute_parent_requirements(lowest_item)

    def _compute_parent_requirements(self, item):
        pass
