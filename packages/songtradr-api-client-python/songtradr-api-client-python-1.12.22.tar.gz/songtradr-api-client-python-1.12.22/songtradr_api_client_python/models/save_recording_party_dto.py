...
class SaveRecordingPartyDTO(BaseModel):
    """
    Recording-party-connection to be saved the API.
    """
    full_name: StrictStr = Field(..., alias="fullName")
    contributor_type: StrictStr = Field(..., alias="contributorType")
    __properties = ["fullName", "contributorType"]

    @validator('contributor_type')
    def contributor_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('trackArtist', 'composer', 'author'):
            raise ValueError("must be one of enum values ('trackArtist', 'composer', 'author')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SaveRecordingPartyDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SaveRecordingPartyDTO:
        """Create an instance of SaveRecordingPartyDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SaveRecordingPartyDTO.parse_obj(obj)
