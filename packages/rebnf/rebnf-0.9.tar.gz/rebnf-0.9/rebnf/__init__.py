...
def tokenize(code) -> list[str]:
    ...
    :rtype:     list[lexers.Token]
    """
    scope = globals()
    if '_lexer' not in scope:
        scope['_lexer'] = lexers.REBNFLexer()
    return scope['_lexer'].tokenize(code)
...
def parse(code) -> dict:
    ...
    """
    scope = globals()
    if '_parser' not in scope:
        scope['_parser'] = parsers.REBNFParser()
    return scope['_parser'].parse(code)
