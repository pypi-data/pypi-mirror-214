...
class ConfirmWithS3StorageTests(AdminConfirmIntegrationTestCase):
    def setUp(self):
        ...
        for obj in self.get_all_media():
            obj.delete()
        super().setUp()

    def get_all_media(self):
        return list(self.bucket.objects.filter(Prefix="media/"))
    ...
    def test_should_save_file_additions(self):
        ...
        if selenium_version.major < 4:
            pytest.skip("Known issue `https://github.com/SeleniumHQ/selenium/issues/8762` with this selenium version.")
        ...
        self.selenium.find_element(By.ID, "id_file").send_keys(
            os.getcwd() + "/admin_action_tools/tests/snapshot/screenshot.png"
        )
    ...
    def test_should_remove_file_if_clear_selected(self):
        ...
        self.assertTrue(self.selenium.find_element(By.XPATH, ".//*[@id='file-clear_id']").get_attribute("checked"))
