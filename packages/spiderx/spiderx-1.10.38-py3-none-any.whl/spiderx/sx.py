...
class WINRAR():
    ...
    def 解压(self,文件路径:str,目录:str='',timeout=None)->bool:
        '''WINRAR().解压("a.rar","aaa")'''
        try:
            ...
            os.makedirs(目录, exist_ok=1)
            # x 用绝对路径解压文件
            # -o+ 导出覆盖文件
            command = [self.exe_path, 'x', 文件路径, '-o+', 目录]
            if self.password:
            ...
            打印错误(e)
            return False
    def 压缩(self, 文件路径:str, 目录:str= '', timeout=None):
        '''
        sx.WINRAR().压缩(文件路径="a.jpg",目录="b.rar")
        sx.WINRAR().压缩(文件路径="xxx",目录="b.rar")
        '''
        try:
            目录=os.path.realpath(目录)
            文件路径 = os.path.basename(文件路径)  #相对路径
            #a 添加文件到压缩文件
            # r 递归
            command = [self.exe_path, 'a', '-r', 目录, 文件路径]
            if self.password:
                command.append(f'-p{self.password}')
            si = subprocess.STARTUPINFO()
            si.dwFlags = subprocess.STARTF_USESHOWWINDOW
