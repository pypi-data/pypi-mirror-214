...
from tdm.abstract.datamodel import AbstractDirective, AbstractDomain, AbstractFact, AbstractNode, AbstractNodeLink, EnsureIdentifiable, \
...
class TalismanDocumentImpl(TalismanDocument, ViewContainer):
    ...
    def id2directive(self) -> Dict[str, AbstractDirective]:
        return self.id2element(AbstractDirective)

    @property
    def directives(self) -> Dict[Type[AbstractDirective], Iterable[AbstractDirective]]:
        return self.elements(AbstractDirective)

    def get_directive(self, id_: str) -> AbstractDirective:
        return self.get_element(AbstractDirective, id_)
    ...
    def get_directives(
        ...
        return self.get_elements(AbstractDirective, type_, filter_=filter_)

    def related_directives(
            self, obj: Union[Identifiable, str], type_: Type[_Directive] = AbstractDirective, *,
            filter_: Union[Callable[[_Directive], bool], Iterable[Callable[[_Directive], bool]]] = tuple()
    ) -> Iterator[_Directive]:
        return self.related_elements(AbstractDirective, obj, type_, filter_=filter_)

    @_filter_elements(AbstractDirective)
    def with_directives(self, directives: Iterable[AbstractDirective], *, update: bool = False) -> Self:
        return self.with_elements(directives, update=update)

    @_filter_elements(AbstractDirective)
    def without_directives(self, directives: Iterable[Union[str, AbstractDirective]], *, cascade: bool = False) -> Self:
