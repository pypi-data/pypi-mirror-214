...
def insert_submodule_updates(
    ...
    Args:
        root_change_log (List[ChangeLogEntry]): A list of ChangeLogEntry objects for the root
            repository.
        submodule_name (str): The name of the submodule for which the updates are being added.
    ...
    values.
    """
    bump_commits = list(filter(
        lambda x: submodule_name in x.submodule_updates,
        root_change_log,
    ))
    ...
    for sub_entry in sub_change_log:
        if index_bump_commits + 1 == len(bump_commits):
            pass
        elif bump_commits[index_bump_commits + 1].submodule_updates[submodule_name].to_sha\
                in sub_entry.sha:
            index_bump_commits += 1
        else:
            pass
        ...
        if index_bump_commits >= 0:
            bump_commits[index_bump_commits].submodule_updates[submodule_name].entries.append(
                sub_entry)
