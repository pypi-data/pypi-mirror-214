...
def dedupe(dataset, mode='exact'):
    if mode == 'exact':
        ...
        threshold = 0.9
    deduper = DedupeFilter(dataset)
    return deduper.full_dedupe_dataset(threshold)


# TODO: add flag for deduping input and output, rather than just input
def dedupe_with_indices(dataset, mode='exact'):
    if mode == 'exact':
        threshold = 1.0
    elif mode == 'minimal':
        threshold = 0.99
...
def balance(dataset, field=None, n=None, mode='exact'):
    if mode == 'exact':
        threshold = 1.0
    elif mode == 'minimal':
        threshold = 0.99
        ...
        threshold = 0.9
    balancer = BalanceFilter(dataset)
    if not field:
