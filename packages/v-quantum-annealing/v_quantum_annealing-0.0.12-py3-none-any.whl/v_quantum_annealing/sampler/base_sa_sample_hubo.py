...
from v_quantum_annealing.cxxvqa.graph import BinaryPolynomialModel, IsingPolynomialModel
from v_quantum_annealing.cxxvqa.sampler import make_sa_sampler
from v_quantum_annealing.sampler.response import Response
from v_quantum_annealing.utils.cxx_cast import (
    cast_to_cxx_random_number_engine,
    cast_to_cxx_temperature_schedule,
    cast_to_cxx_update_method,
)
from v_quantum_annealing.variable_type import BINARY, SPIN
...
def base_sample_hubo(
    ...
    if vartype in ("BINARY", BINARY):
        sampler = make_sa_sampler(
            BinaryPolynomialModel(
                key_list=list(hubo.keys()), value_list=list(hubo.values())
            )
        )
    ...
    sampler.set_update_method(update_method=cast_to_cxx_update_method(update_method))
    ...
    # Make v_quantum_annealing response
    ...
    response.info["sampling_time"] = (
        sample_time + define_sampler_time
    ) * 10**6  # micro sec
