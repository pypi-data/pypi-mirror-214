...
APP_TITLE = 'VaultPass'
APP_DESCRIPTION = 'Utility to search for, display, and save CyberArk account passwords'
...
def choose_cyberark_account(cyberark_accounts_list: list):
	...
	if len(cyberark_accounts_list) == 1:
		...
		print()
		choose_cyberark_account_choice = 0
	else:
...
def output_cyberark_password(cyberark_auth_header: dict, cyberark_account: str):
	...
	while output_cyberark_password_quit_flag is False:
		...
		if output_cyberark_password_choice == 'd':	
			...
			cyberark_password_pass_vault_input = ''
			while cyberark_password_pass_vault_input == '':
				cyberark_password_pass_vault_input = input('Enter the name of the pass password vault folder: ')
				
			cyberark_password_pass_name_input = ''		
			while cyberark_password_pass_name_input == '':
			...
			try:
				...
				password_store.set_key(cyberark_password_pass_vault_input + '/' + cyberark_password_pass_name_input, cyberark_password, force=True)
				print(' Success! '.center(50, "-"))			 
			except:
				print(' Error! '.center(50, "-"))
...
def main():	
	print()
	print('>>> VaultPass <<<')
	print('>> Utility to search for, display, and save CyberArk account passwords <<')
	print()
	...
	while main_quit_flag is False:
		if main_choice == 's':		   
			...
			# print(f'cyberark_accounts: {cyberark_accounts}')
			# print()
			...
			if len(cyberark_accounts) == 0:
				...
				if not(cyberark_account is None):
					...
					# print(f'script_output_password_quit_flag: {script_output_password_quit_flag}')
					# print()
