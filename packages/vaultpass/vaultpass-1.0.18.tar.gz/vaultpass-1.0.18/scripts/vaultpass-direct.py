...
APP_TITLE = 'VaultPass Direct'
APP_DESCRIPTION = 'Utility to add one or more CyberArk account passwords to the pass password store'
PASSPY_VAULT = 'umich'
...
def get_auth_userinfo_from_commandline():
	...
	parser.add_argument('--vault', '-v', type=str, required=False, help='The pass password store location where CyberArk account passwords will be added', default=PASSPY_VAULT)
...
def output_cyberark_password(cyberark_auth_header: dict, account: str, cyberark_account: str, vault: str):
	...
	if cyberark_password == '':
		print(f'Account \'{account}\': No password for this account was found in CyberArk.')
	else:	
		try:
			password_store = passpy.Store()
			password_store.set_key(vault + '/' + account, cyberark_password, force=True)
			print(f'Account \'{account}\': Successfully added the password to the pass password vault folder {vault}.')
		except:
			print(f'Account \'{account}\': Failed to save the password to the pass password vault folder {vault}.')
...
def main():	
	...
	print('>> Utility to add one or more CyberArk account passwords to the pass password store <<')
	print()
	...
	if len(sys.argv) > 1:
		...
		print('--vault vault (default: umich)')
		print()
