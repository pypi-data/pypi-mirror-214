...
from .constants import RoleAccessConst, RoleConst
...
class TokenUser(AuthToken):
	...
	def validate_employee_data_access(self, user_ids) -> bool:
		...
		if self.role.access_type in RoleAccessConst.SELF:
			raise MsbAuthExceptions.UnauthorizedAccessToUserData

		if self.role.access_type in RoleAccessConst.DIRECT_REPORTEES:
			for userid in user_ids:
				if userid not in self.dr_list:
					raise MsbAuthExceptions.UnauthorizedAccessToUserData
