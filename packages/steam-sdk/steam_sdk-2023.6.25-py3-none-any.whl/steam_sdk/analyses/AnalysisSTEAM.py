...
class AnalysisSTEAM:
    ...
    def _set_up_working_folders(self):
        ...
        if self.data_analysis.WorkingFolders.library_path == 'same_as_analysis_yaml_use_model_name':
            ...
            self.library_path = os.path.join(os.getcwd(), self.data_analysis.GeneralParameters.analysis_name, 'input')
        else:
    ...
    def run_analysis(self, post_process_line_number = None, verbose: bool = None):
        """
            ** Run the analysis **
        """
        ...
        for s, seq_step in enumerate(self.data_analysis.AnalysisStepSequence):
            ...
            if step.type == 'MakeModel':
                ...
                self.sigma_data_to_export =  self.step_postprocess(step, post_process_line_number, verbose=verbose)
    ...
    def step_postprocess(self, step, line_number = None, verbose: bool = False): # run sequence variable
        ...
        if step.software and 'SIGMA' in step.software:
            for sim_num in step.simulation_numbers:
                ...
                if not os.path.exists(path_result_txt_Bx):
                    ...
                    try:
                        exported_stats = generate_report_from_map2d(os.path.join(self.settings.local_SIGMA_folder, step.simulation_name, f"{step.simulation_name}_{sim_num}", "output"), path_reference_roxie, "Roxie Data", path_new_file, "SIGMA DATA", "coil", save=True)
                    except:
                        print("Can't generate report from map2d. This can only be done for stationary studies")

        elif step.software and 'FiQuS' in step.software:
            for sim_num in step.simulation_numbers:

                # Take latest run by reading file.
                path_log = os.path.join(self.settings.local_FiQuS_folder, step.simulation_name, f"{step.simulation_name}_{sim_num}", "csv_run_log.csv")
                ...
                # Retrieve the last value from the flattened list
                    ...
                    last_line_value = flattened_rows[line_number]
                path_new_file = last_line_value
                path_reference_roxie = self.list_models['model_name'].model_data.Options_FiQuS.multipole.postproc.compare_to_ROXIE
                if path_reference_roxie is None or not os.path.exists(path_reference_roxie):
                    path_reference_roxie = os.path.join(self.settings.local_FiQuS_folder, step.simulation_name,
                                                        f"{step.simulation_name}_{sim_num}", "sources",
                                                        f"{step.simulation_name}_{sim_num}_ROXIE_REFERENCE.map2d")
                if not os.path.exists(path_new_file):
                    raise Warning(f"Output FiQuS file in the location {path_new_file} is not present.")

                path_new_file = os.path.join(path_new_file, step.simulation_name+".map2d")
                exported_stats = generate_report_from_map2d(
                    os.path.join(self.settings.local_FiQuS_folder, step.simulation_name,
                                 f"{step.simulation_name}_{sim_num}", "output"), path_reference_roxie,
                    "DATA REFERENCE", path_new_file, "DATA CURRENT", "coil", save=True)
    ...
    def setup_sim_SIGMA(self, simulation_name, sim_number, magnet_type=None):
        ...
        string_simulation = f"{simulation_name}_{sim_number}_SIGMA"
        string_coordinate_file = f"{simulation_name}_{sim_number}_ROXIE"
        ...
        for fname in files:
            if any(fname.endswith(suffix) for suffix in
                   suffixes_sources) and string_coordinate_file in fname:  # Check if the file has any of the desired suffixes
    ...
    def run_sim(self, software: str, simulation_name: str, sim_number: int, simFileType: str = None,
        ...
        if software == 'FiQuS':
            ...
            dFiQuS = DriverFiQuS(FiQuS_path=self.settings.FiQuS_path,
                                 ...
                                 path_folder_FiQuS_input=os.path.join(self.settings.local_FiQuS_folder,simulation_name,
                                                                      local_analysis_folder), verbose=verbose,
                                 GetDP_path=self.settings.GetDP_path)
            self.summary = dFiQuS.run_FiQuS(sim_file_name=simulation_name + '_' + str(sim_number), output_directory=local_analysis_folder)
        elif software == 'LEDET':
    ...
    def steam_analyze_lhc_event(self, input_csv_file: str, local_PSPICE_folder: str, flag_run_PSPICE: bool):
        circuit_name = self.__get_first_circuit_name(os.path.join(os.getcwd(), "input", input_csv_file))
        if circuit_name.startswith("RCD"):
            self.analyze_RCD_RCO_event(os.path.join(os.getcwd(), "input", input_csv_file), local_PSPICE_folder, flag_run_PSPICE)
        elif circuit_name.startswith(("RD1", "RD2", "RD3", "RD4")):
            circuit_type = "IPD"
            self.analyze_circuit_event(os.path.join(os.getcwd(), "input", input_csv_file), local_PSPICE_folder, circuit_type, flag_run_PSPICE)
        elif circuit_name.startswith(("RQ4", "RQ5", "RQ7", "RQ8", "RQ9", "RQ10")) or (circuit_name.startswith("RQ6.") and len(circuit_name) == 6):
            circuit_type = self.__find_circuit_model(os.path.join(f"../../steam_models/circuits/circuit_parameters/IPQ_circuit_parameters.csv"), input_csv_file.split("_")[0])
            self.analyze_circuit_event(os.path.join(os.getcwd(), "input", input_csv_file), local_PSPICE_folder, circuit_type, flag_run_PSPICE)
        else:
            circuit_type = self.__get_circuit_type(circuit_name)
            self.analyze_circuit_event(os.path.join(os.getcwd(), "input", input_csv_file), local_PSPICE_folder, circuit_type, flag_run_PSPICE)

    def analyze_circuit_event(self, input_csv_file: str, local_PSPICE_folder: str, circuit_type: str, flag_run_PSPICE: bool):
        file_counter = 1
        aSTEAM = AnalysisSTEAM()
        aSTEAM.settings.local_PSPICE_folder = local_PSPICE_folder
        aSTEAM.data_analysis.GeneralParameters = General(analysis_name='test', flag_permanent_settings=True, model= Model(name='all', version='1.0', case='null', state='draft'))
        aSTEAM.data_analysis.AnalysisStepDefinition = {
            'setup_folder_PSPICE': SetUpFolder(type='SetUpFolder', simulation_name=circuit_type, software=['PSPICE']),
            'makeModel_ref': MakeModel(type='MakeModel', model_name='BM', file_model_data=circuit_type,
                                       case_model='circuit', software=['PSPICE'], simulation_name=None,
            ...
            'runParsimEvent': ParsimEvent(type='ParsimEvent', input_file=input_csv_file,
                                          path_output_event_csv=os.path.join(os.getcwd(), "output", 'run_parsim_event_circuit', "events.csv"), path_output_viewer_csv=None,
                                          simulation_numbers=[file_counter], model_name='BM', case_model='circuit',
                                          simulation_name=circuit_type, software=['PSPICE'], t_PC_off=None,
                                          ...
                                          default_keys=DefaultParsimEventKeys(local_LEDET_folder=None,
                                                                              ...
                                                                              path_output=local_PSPICE_folder)),
            'run_simulation': RunSimulation(type='RunSimulation', software='PSPICE', simulation_name=circuit_type, simulation_numbers=[file_counter])}
        aSTEAM.library_path = "../../steam_models"
        aSTEAM.data_analysis.WorkingFolders.library_path = "../../steam_models"
        aSTEAM.output_path = local_PSPICE_folder
        aSTEAM.data_analysis.WorkingFolders.output_path = os.path.join(os.getcwd(), "output", 'run_parsim_event_circuit')
        aSTEAM.data_analysis.PermanentSettings.local_PSPICE_folder = local_PSPICE_folder
        if flag_run_PSPICE== False:
            aSTEAM.data_analysis.AnalysisStepSequence = ['setup_folder_PSPICE', 'makeModel_ref', 'runParsimEvent']
        else:
            aSTEAM.settings.PSPICE_path = r"C:\Cadence\SPB_17.4\tools\bin\psp_cmd.exe" #set according to your system
            aSTEAM.data_analysis.AnalysisStepSequence = ['setup_folder_PSPICE', 'makeModel_ref', 'runParsimEvent', 'run_simulation']
        outputfile = os.path.join(os.getcwd(), 'output', 'run_parsim_event_circuit',
                                  f'TestFile_AnalysisSTEAM_run_parsim_event_circuit_{circuit_type}_{file_counter}.yaml')
    ...
    def analyze_RCD_RCO_event(self, input_csv_file: str, local_PSPICE_folder: str, flag_run_PSPICE: bool):
        # arrange
        file_counter = 1
        aSTEAM_1 = AnalysisSTEAM()
        aSTEAM_1.settings.local_PSPICE_folder = local_PSPICE_folder
        aSTEAM_1.data_analysis.GeneralParameters = {}
        aSTEAM_1.data_analysis.AnalysisStepDefinition = {'setup_folder_PSPICE': SetUpFolder(type='SetUpFolder', simulation_name='RCO', software=['PSPICE']), 'makeModel_ref': MakeModel(type='MakeModel', model_name='BM', file_model_data='RCO', case_model='circuit', software=['PSPICE'], simulation_name=None, simulation_number=None, flag_build=True, flag_dump_all=False, verbose=False, flag_plot_all=False, flag_json=False), 'runParsimEvent': ParsimEvent(type='ParsimEvent', input_file=input_csv_file, path_output_event_csv=os.path.join(os.getcwd(), "output", 'run_parsim_event_circuit', "events.csv"), path_output_viewer_csv=None, simulation_numbers=[file_counter], model_name='BM', case_model='circuit', simulation_name='RCO', software=['PSPICE'], t_PC_off=None, rel_quench_heater_trip_threshold=None, current_polarities_CLIQ=[], dict_QH_circuits_to_QH_strips={}, default_keys=DefaultParsimEventKeys(local_LEDET_folder=None, path_config_file=None, default_configs=[], path_tdms_files=None, path_output_measurement_files=None, path_output= local_PSPICE_folder)), 'run_simulation': RunSimulation(type='RunSimulation', software='PSPICE', simulation_name='RCO', simulation_numbers=[file_counter])}
        aSTEAM_1.library_path = "../../steam_models"
        aSTEAM_1.data_analysis.WorkingFolders.library_path = "../../steam_models"
        aSTEAM_1.output_path = local_PSPICE_folder
        aSTEAM_1.data_analysis.WorkingFolders.output_path = os.path.join(os.getcwd(), "output", 'run_parsim_event_circuit')
        aSTEAM_1.data_analysis.PermanentSettings.local_PSPICE_folder = local_PSPICE_folder
        if flag_run_PSPICE == False:
            aSTEAM_1.data_analysis.AnalysisStepSequence = ['setup_folder_PSPICE', 'makeModel_ref', 'runParsimEvent']
        else:
            aSTEAM_1.settings.PSPICE_path = r"C:\Cadence\SPB_17.4\tools\bin\psp_cmd.exe"  # set according to your system
        ...
        aSTEAM_2.settings.local_PSPICE_folder = local_PSPICE_folder
        aSTEAM_2.data_analysis.GeneralParameters = {}
        aSTEAM_2.data_analysis.AnalysisStepDefinition = {'setup_folder_PSPICE': SetUpFolder(type='SetUpFolder', simulation_name='RCD', software=['PSPICE']), 'makeModel_ref': MakeModel(type='MakeModel', model_name='BM', file_model_data='RCD', case_model='circuit', software=['PSPICE'], simulation_name=None, simulation_number=None, flag_build=True, flag_dump_all=False, verbose=False, flag_plot_all=False, flag_json=False), 'runParsimEvent': ParsimEvent(type='ParsimEvent', input_file=input_csv_file, path_output_event_csv=os.path.join(os.getcwd(), "output", 'run_parsim_event_circuit', "events_2.csv"), path_output_viewer_csv=None, simulation_numbers=[file_counter], model_name='BM', case_model='circuit', simulation_name='RCD', software=['PSPICE'], t_PC_off=None, rel_quench_heater_trip_threshold=None, current_polarities_CLIQ=[], dict_QH_circuits_to_QH_strips={}, default_keys=DefaultParsimEventKeys(local_LEDET_folder=None, path_config_file=None, default_configs=[], path_tdms_files=None, path_output_measurement_files=None, path_output= local_PSPICE_folder)), 'run_simulation': RunSimulation(type='RunSimulation', software='PSPICE', simulation_name='RCD', simulation_numbers=[file_counter])}
        aSTEAM_2.library_path = "../../steam_models"
        aSTEAM_2.data_analysis.WorkingFolders.library_path = "../../steam_models"
        aSTEAM_2.output_path = local_PSPICE_folder
        aSTEAM_2.data_analysis.WorkingFolders.output_path = os.path.join(os.getcwd(), "output", 'run_parsim_event_circuit')
        aSTEAM_2.data_analysis.PermanentSettings.local_PSPICE_folder = local_PSPICE_folder
        if flag_run_PSPICE == False:
            aSTEAM_2.data_analysis.AnalysisStepSequence = ['setup_folder_PSPICE', 'makeModel_ref', 'runParsimEvent']
        else:
            aSTEAM_2.settings.PSPICE_path = r"C:\Cadence\SPB_17.4\tools\bin\psp_cmd.exe"  # set according to your system
    ...
    def __get_circuit_type(self, circuit_name: str):
        if circuit_name.startswith("RCBH") or circuit_name.startswith("RCBV"):
            ...
            magnet_number = self.__find_magnets(os.path.join(f"../../steam_models/circuits/circuit_parameters/RQ_circuit_parameters.csv"), circuit_name)
            ...
            magnet_number = self.__find_magnets(os.path.join(f"../../steam_models/circuits/circuit_parameters/600A_circuit_parameters.csv"), circuit_name)
        ...
        elif self.data_analysis.AnalysisStepDefinition['runParsimEvent'].simulation_name == "RCD": #improvement pending
            return "RCD"
        elif self.data_analysis.AnalysisStepDefinition['runParsimEvent'].simulation_name == "RCO": #improvement pending
            return "RCO"

    def __get_quenching_magnet_number(self, position: str, circuit_family_name: str):
        df = pd.read_csv(os.path.join(f"../../analyses/analysis_{circuit_family_name}_FPA_events/{circuit_family_name}_LayoutDetails.csv"))
