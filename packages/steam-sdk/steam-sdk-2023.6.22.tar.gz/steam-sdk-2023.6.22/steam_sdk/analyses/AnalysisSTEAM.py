    ...
    def run_analysis(self, verbose: bool = None):
        ...
        for s, seq_step in enumerate(self.data_analysis.AnalysisStepSequence):
            ...
            if step.type == 'MakeModel':
                ...
                self.sigma_data_to_export =  self.step_postprocess(step, verbose=verbose)
    ...
    def step_postprocess(self, step, verbose: bool = False):
        ...
        if step.software and 'SIGMA' in step.software:
            ...
            for sim_num in step.simulation_numbers:
                ...
                path_new_file = os.path.join(self.settings.local_FiQuS_folder, step.simulation_name,
                                             f"{step.simulation_name}_{sim_num}", "Geometry_1", "Mesh_1", "Solution_1",  f"{step.simulation_name}.map2d")
                ...
                if not os.path.exists(path_reference_roxie):
                    raise Warning(
                        "No Roxie reference file is found, please check that the file is present in steam-models.")
                elif not os.path.exists(path_new_file):
                ...
                exported_stats = generate_report_from_map2d(
                    os.path.join(self.settings.local_FiQuS_folder, step.simulation_name,
                                 f"{step.simulation_name}_{sim_num}"), path_reference_roxie,
                    "Roxie Data", path_new_file, "FiQuS DATA", "coil", save=True)
