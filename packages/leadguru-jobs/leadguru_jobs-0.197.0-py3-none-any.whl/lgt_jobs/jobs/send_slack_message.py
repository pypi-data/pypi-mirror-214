...
from lgt.common.python.slack_client.web_client import SlackWebClient, return_cred_if_contact_connected, \
    SlackMessageConvertService
...
class SendSlackMessageJob(BaseBackgroundJob):
    ...
    def exec(self, data: SendSlackMessageJobData):
        ...
        cred = return_cred_if_contact_connected(user.id, lead.message.name)
        ...
        message_model = SlackMessageConvertService.from_slack_response(user.email, "slack_files", cred.token, message,
