...
from .api import PluginMessage, PluginMessageAbandonedStreams, PluginMessageCancelledStreams, PluginMessageCurrentActivity, PluginMessageStreamProgressUpdate, PluginMessageStreamState, handle_proc_result
...
class SyncRequest(ABC):
        ...
        while not cancellation_token.is_set():
            ...
            with self._snowflake_query_lock:
                try:
                    ...
                    query_result = self._session.sql(f"call {self._source_app_name}.API.PLUGIN_CANCELLATION_CHECK({self._run_id})").collect()
                    result = json.loads(query_result[0][0])
                    if result['success'] is not True:
                        logger.error(f"Error checking cancellation: {result['error']}")
                    is_cancelled:bool = result['data']['is_cancelled']
                    if is_cancelled:
    ...
    def update_activity(self,current_activity:str):
        ...
        self._plugin_message(PluginMessageCurrentActivity(current_activity=current_activity))
    ...
    def _plugin_message(self,message:PluginMessage):
        ...
        with self._snowflake_query_lock:
            try:
                # this is not ideal, but "Bind variable in stored procedure is not supported yet"
                handle_proc_result(self._session.sql(f"""call {self._source_app_name}.API.PLUGIN_MESSAGE(
                                  {self._run_id},
                                  PARSE_JSON($${json.dumps(message,default=pydantic.json.pydantic_encoder)}$$))""").collect())
            except Exception as e:
                logger.error(f"Error sending plugin message: {e}",exc_info=True,stack_info=True)
...
class OutboundSyncRequest(SyncRequest):
    ...
    def __init__(self,run_id:int,
        ...
        :return: nothing
        """
        SyncRequest.__init__(self,run_id,session,source_app_name,results_schema_name,results_table_name,plugin_instance,api_limits,rate_limit_state,run_deadline,development_mode,test_replay_mode)
        self._full_records_table_name = f"{source_app_name}.{records_schema_name}.{records_table_name}"
...
class InboundSyncRequest(SyncRequest):
    ...
    def apply_results_queue(self):
        ...
        if self._apply_results is not None:
            with self._apply_results_lock:
                ...
                self._plugin_message(message=PluginMessageStreamProgressUpdate(stream_total_counts=self._stream_record_counts,
    ...
    def apply_cancellation(self):
        ...
        self._plugin_message(message=PluginMessageCancelledStreams(cancelled_streams=cancelled_streams))
    ...
    def apply_deadline_reached(self):
        ...
        self._plugin_message(message=PluginMessageAbandonedStreams(abandoned_streams==abandoned_streams))
...
class OmnataPlugin(ABC):
    ...
    def network_addresses(self, parameters:ConnectionConfigurationParameters) -> List[str]:
        ...
        :rtype List[str]
        """
        raise NotImplementedError('Your plugin class must implement the network_addresses method')
