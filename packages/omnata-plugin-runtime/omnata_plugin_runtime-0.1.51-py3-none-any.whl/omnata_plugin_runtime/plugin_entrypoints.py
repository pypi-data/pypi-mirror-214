...
from .omnata_plugin import BillingEvent,BillingEventRequest, OmnataPlugin, HttpRateLimiting, OutboundSyncRequest, InboundSyncRequest
from .configuration import OutboundSyncConfigurationParameters,InboundSyncConfigurationParameters, OutboundSyncStrategy, StoredConfigurationValue,SyncConfigurationParameters,StreamConfiguration, StoredStreamConfiguration
from .rate_limiting import ApiLimits, RateLimitState
from .api import ApplyPayload, handle_proc_result
...
class PluginEntrypoint():
    ...
    def apply(self,apply_request:Dict):
        ...
        base_parameters = SyncConfigurationParameters(connection_method=request.connection_method,
        ...
        if request.sync_direction=='outbound':
            ...
            for stream in streams_list:
                if stream.stream_name in request.latest_stream_state:
                    stream.latest_state = request.latest_stream_state[stream.stream_name]
    ...
    def create_billing_events(self,session, event_request:Dict):
        ...
        events:List[BillingEvent] = self._plugin_instance.create_billing_events(request)
        ...
        for billing_event in events:
            ...
            billing_event_time = int(billing_event.start_timestamp.replace(tzinfo=datetime.timezone.utc).timestamp() * 1000)
    ...
    def network_addresses(self,method:str,connection_parameters:Dict) -> List[str]:
        ...
        return self._plugin_instance.network_addresses(ConnectionConfigurationParameters(connection_method=method,
                                                                connection_parameters=parse_obj_as(Dict[str,StoredConfigurationValue],connection_parameters),
                                                                connection_secrets={}))
