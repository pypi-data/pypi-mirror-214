...
class TestUnitCell(ut.TestCase):

    def setUp(self) -> None:
        self.pc = UnitCell(1.0, 1.0, 1.0, 90, 90, 90)
        return super().setUp()
    ...
    def test_getpoint(self):
        pt = self.pc.get_point(0,0,0,0)
        self.assertSamePoint(pt, [0,0,0])
        pt2 = self.pc.get_point(-1,0,0,0)
        self.assertSamePoint(pt2, [-1,0,0])
        pt3 = self.pc.get_point(0,2,0,0)
        self.assertSamePoint(pt3, [0,2,0])
        pt4 = self.pc.get_point(-1,0,3,0)
