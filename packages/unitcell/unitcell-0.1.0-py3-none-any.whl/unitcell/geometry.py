...
class Vec3:
    ...
    def xrotate(self, theta, degrees=True):
        if degrees:
    ...
    def yrotate(self, theta, degrees=True):
        if degrees:
            theta = m.pi*theta/180
        ct = m.cos(theta)
        st = m.sin(theta)
    ...
    def zrotate(self, theta, degrees=True):
        if degrees:
            theta = m.pi*theta/180
        ct = m.cos(theta)
        st = m.sin(theta)
...
def toVec3(v):
    """Convert a list or tuple to a Vec3"""
...
def unit_vector(v):
    """Return the unit Vec3 parallel to Vec3 v"""
    return v/abs(v)
