...
class Interface:
    ...
    def create_center_text(self, color, text, font="Arial", size = 12, bold = False):
        if bold:
            text = self.canvas.create_text(self.center_x, self.center_y, text=text, fill=color, font=(font, size, "bold"))
        else:
            text = self.canvas.create_text(self.center_x, self.center_y, text=text, fill=color, font=(font, size, "bold"))
    ...
    def remove_item(self, item_id):
        # Find the item with the given item_id in the shapes list
        for item in self.shapes:
            if item == item_id:
                # Remove the item from the shapes list
