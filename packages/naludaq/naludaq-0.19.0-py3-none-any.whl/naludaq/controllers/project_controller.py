...
from naludaq.helpers import get_application_path
from naludaq.io import io_manager, load_pickle_acquisition, save_pickle_acquisition
...
def get_recentsfile():
    """Return the filepath to the recents file.

    The file contains the 10 latest opened projects.
    """
    recentsfile = os.path.join(
...
def load_recent(recentsfile: str) -> list:
    """Load all recent projects from the config file.

    Checks the files still exists, if not, remove them from the list.

    Returns:
        List of recent (projectname, filepath) tuples.
    """
    # Check if load recent file exist, if not, return empty list.
    if not os.path.isfile(recentsfile):
        return []

    try:
        rfile = gzip.GzipFile(recentsfile, "r")
        recents = pickle.load(rfile)
    except IOError as error_msg:
        LOGGER.error("File could not be loaded. %s", error_msg)
    except pickle.PicklingError as error_msg:
        LOGGER.error("Loading recents failed: %s", error_msg)
    else:
        # Make sure the file exists before returning it.
        return [x for x in recents if os.path.isfile(x[1])]


def add_recent(recentsfile, item):
    """Adds project to recents.

    Recents is a queue structure where new projects are added to the top and
    old projects fall off the bottom when the queue gets long enough.
    After each add it saves the info in the config file.
    """
    # If for some reason the configs dir is lost, recreate.
    path, _ = os.path.split(recentsfile)
    if not os.path.isdir(path):
        os.mkdir(path)

    if os.path.isfile(recentsfile):
        recents = load_recent(recentsfile)
    else:
        recents = []

    # Add to top, and pop bottom if it's longer than 10.
    recents.append(item)
    if len(recents) > 10:
        recents.pop(0)

    try:
        rfile = gzip.GzipFile(recentsfile, "w", compresslevel=4)
        pickle.dump(recents, rfile, protocol=pickle.HIGHEST_PROTOCOL)
    except IOError as error_msg:
        LOGGER.error("File could not be saved. %s", error_msg)
    except pickle.PicklingError as error_msg:
        LOGGER.error("Saving recents failed: %s", error_msg)
...
class ProjectController:
    ...
    def remove_aquisition(self, acq):
    ...
    def save(self, filename):
        ...
        # else:
        #     add_recent(str(filename), self.project_name)
    ...
    def load(self, filename):
        ...
        try:
            ...
            LOGGER.error("Loading acquisition failed: %s", error_msg)
        else:
    ...
    def export_evt_to_csv(self, filename, evt_list, max_channels=32):
        """Export the data from and event to CSV.

        Please note this will only export the data, not the information regarding the event.
        This function also assumes filename and evt are in the correct format.
        It will override any existing file.

        Deprecated as of v0.1.22
        """

        acqs = [evt_list]
        io_manager.IOManager().export_acq_to_csv(filename, acqs)

    @deprecation.deprecated(
        deprecated_in="0.1.22",
        details="Use the IO Controller for all csv imports/exports",
    )
    def export_acq_to_csv(self, filename: str, acqs: list):
    ...
    #     if isinstance(key, int) and isinstance(value, deque):
