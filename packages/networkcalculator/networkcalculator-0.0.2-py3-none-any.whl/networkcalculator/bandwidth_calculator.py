...
class Bandwith_Calculator(object):
    """
    ...
    def convert_seconds_to_time(self, seconds):
        ...
        hours = seconds // 3600  # Get the hours and discard the rest
        seconds %= 3600  # Get the remaining seconds
        minutes = seconds // 60  # Get the minutes and discard the rest
        seconds %= 60  # Get the remaining seconds
        return f"{int(hours)} hours {int(minutes)} minutes {seconds} seconds"
...
# Create an instance of the BandwidthCalculator
calculator = Bandwith_Calculator('1 Gbps', 50, 50, '1 TB') # Media Throughput, Read, writes , file size) 
