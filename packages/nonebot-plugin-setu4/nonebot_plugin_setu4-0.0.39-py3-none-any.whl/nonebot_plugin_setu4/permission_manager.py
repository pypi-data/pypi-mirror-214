...
class PermissionManager:
    ...
    # --------------- 逻辑判断 开始 ---------------
    def check_permission(self, session_id: str, r18flag: bool, num: int, user_type: str = 'group'):
        """查询权限, 并返回修正过的参数
        ...
        # 查询冷却时间
        tile_left = self.read_cd(session_id) + \
                        self.read_last_send(session_id) - time.time()
        if tile_left > 0:
        ...
        num_ = num if num <= self.read_max_num(
            session_id) else self.read_max_num(session_id)
