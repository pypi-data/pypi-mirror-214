...
class ManagerHandle:
    ...
    async def open_setu(
        ...
        if 'add' in msg:
            ...
            await matcher.finish(f'无效参数: {msg}, 请输入 add 或 del 为参数, eg: add group_114514')
    ...
    async def set_cd(
        ...
        if msg.isdigit() and isinstance(event, GroupMessageEvent):
            ...
            if len(args) != 2 or not args[0].isdigit() or not self.verify_sid(args[1]):
                await matcher.finish(f'无效参数: {msg}, 请输入\n正整数或零 + 空格 + 会话类型_会话id\n为参数\n例如: 0 group_114514')
            else:
                cd_time = int(args[0])
                sid = args[1]
                await matcher.finish(pm.update_cd(sid, cd_time))
    ...
    async def setu_db(
        ...
        """拉取数据库"""
        await matcher.send('此功能由于大陆对github的半墙, 国内服务器可能造成数据丢失或无法写入等错误, 不确定性较大, 万一数据库丢失请重新clone')
        try:
    ...
    async def query_black_white_list(
        self,
        matcher: Matcher
    ) -> None:
        """查新黑白名单"""
