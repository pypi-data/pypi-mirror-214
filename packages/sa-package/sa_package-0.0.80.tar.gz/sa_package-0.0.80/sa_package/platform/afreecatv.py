...
def get_vod_list_month_range(driver, bj_id, start_month, end_month):
    ...
    while True:
        ...
        for vod in vod_list:
            ...
            if '시간 전' in vod_date:
                before_hours = int(vod_date.split('시간 전')[0])
                vod_date = (datetime.datetime.today() - datetime.timedelta(seconds=3600*before_hours)).strftime('%Y-%m-%d')
...
def get_bj_info(bj_id, driver=None):
    need_to_close = True
    ...
    nick = driver.find_element(By.CSS_SELECTOR, 'div.nick > h2').text.split("\n")[0]
    favor_num = convert_hannum_to_num(driver.find_elements(By.CSS_SELECTOR, "button.favor")[1].text.split("즐겨찾기")[1], is_int=True)
    sub_num = convert_hannum_to_num(driver.find_elements(By.CSS_SELECTOR, "button.subscribe")[0].text.split("구독")[1], is_int=True)
    
    if need_to_close:
        driver.close()

    return {'nick':nick, 'favor_num':favor_num, 'sub_num':sub_num}
...
def get_sub_num(bj_id, driver=None):
