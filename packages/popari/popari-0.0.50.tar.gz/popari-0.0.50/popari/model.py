...
def from_pretrained(pretrained_model: Popari, popari_context: dict, lambda_Sigma_bar: float):
    """Initialize Popari object from a SpiceMix pretrained model.
    
    """

    datasets = [PopariDataset(dataset, dataset.name) for dataset in pretrained_model.hierarchy[0].datasets]
    replicate_names = [dataset.name for dataset in datasets]
