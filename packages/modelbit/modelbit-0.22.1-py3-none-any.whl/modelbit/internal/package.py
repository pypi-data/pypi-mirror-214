...
from modelbit.cli.describe import toYaml
...
def add_module(m: ModuleType, force: bool, api: MbApi) -> Optional[PackageInfo]:
  ...
  if type(m) is not ModuleType:
    raise UserFacingError(f"Expecting 'Module' but received '{type(m)}'.")
  ...
  if "." in m.__name__:
    ...
    if baseModuleName not in sys.modules:
      raise UserFacingError(f"Cannot find module '{baseModuleName}' to upload")
    return add_module(sys.modules[baseModuleName], force, api)
  pkgInfo, priorPkgInfo = builder.fetchNextPackageVersion(m.__name__)
  pkgKind = "rawdir"
  if hasattr(m, "__path__"):
    ...
    raise UserFacingError("Cannot find source of module {m.__name__} to add.")
  printTemplate("package-uploading", None, pkgInfo=pkgInfo, path=os.path.abspath(path))
  wheelPath, pkgInfo, recordHash = builder.buildPackage(path, force, (pkgKind, pkgInfo))

  if not force and priorPkgInfo is not None and priorPkgInfo.version is not None and recordHash == priorPkgInfo.recordHash:
    printTemplate("package-unchanged", None, pkgInfo=priorPkgInfo)
    return PackageInfo(name=priorPkgInfo.name or pkgInfo.name, version=priorPkgInfo.version)
  pkgInfo = builder.uploadPackage(wheelPath, force, pkgInfo, recordHash)
...
class PackageBuilder:
  ...
  def buildPackage(self,
    ...
    with zipfile.ZipFile(wheelPath) as archive:
      record = archive.read(f"{pkgInfo.name}-{pkgInfo.version}.dist-info/RECORD")
      record = re.sub(b"(?m)^.+\\.dist-info.*$", b"", record).strip()
      recordHash = f"sha1:{hashlib.sha1(record).hexdigest()}"
