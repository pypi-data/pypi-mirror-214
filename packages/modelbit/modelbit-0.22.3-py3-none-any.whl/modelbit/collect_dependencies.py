...
from modelbit.cli.describe import MAX_DESCRIBABLE_OBJECT_SIZE
from modelbit.error import UserFacingError

from .helpers import InstancePickleWrapper, JobProps, NamespaceCollection, RuntimePythonProps
...
def getRuntimePythonProps(func: Optional[Callable[..., Any]],
  ...
  if job is not None:
    from modelbit.jobs import ModelbitJobWrapper
    if type(job) is not ModelbitJobWrapper:
      raise UserFacingError('add_job must be passed the result of `modelbit.job()`')
