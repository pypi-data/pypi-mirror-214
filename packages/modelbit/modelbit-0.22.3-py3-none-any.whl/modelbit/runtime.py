...
import re, json, pickle, os, hashlib
...
from .helpers import DeploymentTestDef, DeploymentTestError, RuntimePythonProps, getJsonOrPrintError, getMissingPackageWarningsFromEnvironment, getMissingPackageWarningsFromImportedModules, isAuthenticated, DeploymentInfo
...
class Runtime:
  ...
  def __init__(self,
    ...
    self._deploymentInfo: Optional[DeploymentInfo] = None
  ...
  def _getRuntimePythonProps(self):
    ...
    try:
      props = getRuntimePythonProps(self._deployFunc, sourceOverride=self._sourceOverride, job=self._job)
    except TypeError:
      raise
  ...
  def deploy(self):
    ...
    if not resp or not isAuthenticated():
      return None
    if resp.deploymentInfo:
      self._deploymentInfo = resp.deploymentInfo
    if resp.runtimeOverviewUrl:
      jobName = self._job.desc.jobName if self._job is not None else None
  ...
  def _choosePackagesIfNotSpecified(self):
    if self._pythonPackages is None:
      ...
      if len(missingPips) > 0:
        self.set_system_packages(systemPackagesForPips(missingPips, self._systemPackages))
...
def uploadRuntimeObject(obj: Optional[Any], objData: bytes, uxDesc: str) -> Optional[str]:
  from modelbit.autologin import mbApi
  from .cli.describe import describeFile, describeObject, toYaml
  from .api import ObjectApi
  contentHash = f"sha1:{hashlib.sha1(objData).hexdigest()}"
  if obj is None:
    description = describeFile(objData, 1)
  else:
    description = describeObject(obj, 1)
  ObjectApi(None, mbApi()).uploadRuntimeObject(objData, contentHash, uxDesc)
