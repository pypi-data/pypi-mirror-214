import time
from typing import Any, Callable, List, Optional, Union, cast, Dict

from modelbit.api import JobApi, JobRunDesc, MbApi, JobDesc
from modelbit.error import UserFacingError
from modelbit.helpers import inDeployment
from modelbit.internal.runtime_jobs import getJobOutputFromWeb, getJobOutputFromS3
from modelbit.model_wrappers import RuntimeJobWrapper

from modelbit.ux import renderTemplate
...
def add_job(
  ...
  if type(redeploy_on_success) is not bool:
    raise UserFacingError("The redeploy_on_success parameter must be a boolean")
  ...
  if size is not None and size not in ["small", "medium", "large", "xlarge"]:
    raise UserFacingError('The size parameter must be one of "small", "medium", "large", or "xlarge"')
  ...
  jobDesc = dict(jobName=name,
                 schedule=schedule,
                 redeployOnSuccess=redeploy_on_success,
...
def getJobOutput(mbApi: MbApi,
  ...
  if inDeployment():
    return getJobOutputFromS3(branch=branch,
...
class ModelbitJobRun:
  ...
  def __repr__(self) -> str:
    return f"<{self.__class__.__name__} {repr(self._desc)}>"
