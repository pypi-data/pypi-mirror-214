...
from modelbit.cli.secure_storage import EncryptedObjectInfo
...
class JobDesc:
  ...
  size: Optional[str]
  redeployOnSuccess: Optional[bool]
  ...
  def __init__(self, data: Dict[str, Any]):
    ...
    self.redeployOnSuccess = data.get("redeployOnSuccess", None)
...
class JobApi:
  ...
  def getJobOutputContent(
      self,
      branch: str,
      runtimeName: str,
      jobName: str,
      userFacingId: Optional[int] = None,
      fileName: Optional[str] = None) -> Optional[Dict[str, Union[str, EncryptedObjectInfo]]]:
    ...
    if "encryptedObjectInfo" in resp:
      return dict(workspaceId=resp["workspaceId"],
                  encryptedObjectInfo=EncryptedObjectInfo(**resp["encryptedObjectInfo"]))
    return None
