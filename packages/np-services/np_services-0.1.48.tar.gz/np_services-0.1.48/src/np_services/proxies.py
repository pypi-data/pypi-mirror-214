...
class VideoMVR(MVR):
    ...
    def get_cameras(cls) -> list[dict[str, str]]:
        ...
        cams = super().get_cameras()
        if np_config.Rig().idx == 2:
            # temp vid mon computer on NP2 has the follow cameras:
            """            
            [{'configured_name': 'Camera 1',
            'label': 'Behavior',
            'id': 'DEV_000F315BA0AA',
            'serial_number': '50-0536876074'},
            {'configured_name': 'Camera 3',
            'label': 'Face',
            'id': 'DEV_000F315C1467',
            'serial_number': '50-0536905703'},
            {'configured_name': 'Camera 2',
            'label': 'Eye',
            'id': 'DEV_000F315C1468',
            'serial_number': '50-0536905704'},
            {'configured_name': 'DEV_1AB22C013782',
            'label': 'none',
            'id': 'DEV_1AB22C013782',
            'serial_number': '01PJ6'}]
            """
            # TODO check this isn't also the case on NP.0 & 3
            return [_ for _ in cams if cls.re_aux.search(_["label"]) is not 'none']
