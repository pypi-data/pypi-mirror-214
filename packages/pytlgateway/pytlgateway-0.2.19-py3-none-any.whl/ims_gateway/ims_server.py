...
from utils import (decode_ims_status,side_to_target_type, decode_exchange_id, encode_ims_side, decode_ordtype, decode_ims_side, decode_ims_contract_type)
...
class ImsServer(MongoClientTradeGateway):
    ...
    def req_buy_order_insert(self, obj):
        ...
        begin_time = str(obj['executionPlan']["start_time"])
        ...
        int_contract_type = obj['contract_type']
    ...
    def monitor_trade_update(self):
        while not self.is_stopped:
            with self.trade_lock:
                ...
                with open(trade_filename, 'r') as file:
                    # Read each line in the file
                    for line in file:
                        # Skip lines starting with '#'
                        if line.startswith('#'):
                        ...
                        for record in data_list:
                            if record['成交状态'] in ['F_FILLED', 'CANCELED']: #only need filled/canceled
                                self.on_trade_msg(record)
                            else:
                                self.logger.info(f"[monitor_trade_update] trade_not_finished (msg){record['错误信息']}")

            time.sleep(30)

    @MongoClientTradeGateway.error_handler
    def on_trade_msg(self, record):
        if record['成交数量'] == 0 or record['成交价格'] == 0:
        ...
        trade_ref = str(record['成交编号']) #外部委托编号，出现在这个位置主要是凑数
        ticker = str(record['合约代码'])
        exchange = decode_exchange_id(str(record.Symbol).split('.')[1].strip(' '))
        traded_vol = record['成交数量']
        traded_price = record['成交价格']
        trade_amt = float(traded_price * traded_vol)
        order_type = record['算法ID']
