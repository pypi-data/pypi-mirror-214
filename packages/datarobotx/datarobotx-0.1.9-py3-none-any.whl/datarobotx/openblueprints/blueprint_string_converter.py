...
class BlueprintStringConverter:
    ...
    def format_code_block_with_black(code_block: str) -> str:
        """Format a code block using black"""
        return str(black.format_file_contents(code_block, fast=False, mode=black.FileMode()))

    @classmethod
    def make_blueprint(
        cls, preprocessor_pipeline: List[str], estimator_element: Tuple[str, str]
    ) -> str:
        ...
        preprocessor_pipeline : list[str]
            ...
            See:
                `make_column_transformer_from_pipeline`
        ...
        return blueprint_code_formatted

    @classmethod
    def make_standard_template(cls) -> str:
        """Construct a standard template for the top of the code"""
        return (
            f"{cls.STANDARD_IMPORTS}\n"
            f"{cls.STANDARD_INPUT_TEMPLATE}\n"
            f"{cls.CATEGORICAL_COLUMN_TEMPLATE}".replace("        ", "")
        )
    ...
    def make_pipeline_from_pipeline_elements(pipeline_element_str: str) -> str:
        ...
        Elements should be created from steps with:
