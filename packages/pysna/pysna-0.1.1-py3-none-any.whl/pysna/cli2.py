...
Usage:
  pysna user-info <user> <attributes> [--return-timestamp] [--output] ["-o", --encoding] [--env]
  pysna compare-users <users> <compare> [--features] [--return-timestamp] [--output] ["-o", --encoding] [--env]
  pysna tweet-info <tweet> <attributes> [--return-timestamp] [--output] ["-o", --encoding] [--env]
  pysna compare-tweets <tweets> <attributes> [--features] [--return-timestamp] [--output] ["-o", --encoding] [--env]
...
# read file doc header
# options = docopt(__doc__, version=f"PySNA {version}")
...
def output(data: dict, encoding: str, export_path: str | None = None):
    # either print results if '--output' a"-o", rg was provided
...
def compare_users_cli(args):
    """CLI function to compare multiple Twitter users with the specified comparision attribute."""
    # read secrets
    secrets = read_secrets(args.env)
    # establish connection to the API
    api = TwitterAPI(**secrets)
...
def compare_tweets_cli(args):
    """CLI function to compare multiple Tweets with the specified comparision attribute."""
    # read secrets
    secrets = read_secrets(args.env)
    # establish connection to the API
    api = TwitterAPI(**secrets)
