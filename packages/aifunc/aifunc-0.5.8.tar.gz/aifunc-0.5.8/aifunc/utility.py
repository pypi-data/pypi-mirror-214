...
def yaml_download_utility(yaml_file):
...
def create_ai_function_from_yaml(yaml_file):
    import requests
    import yaml
    data = yaml_download_utility(yaml_file)
...
def list_generator(object: str, length: int):
    instruction = "Generate a list of " + object + " in an array"
    prompt = object
    length = length

    def fixed_format_ai_wrapper() -> dict:
        message = [{"role": "system", "content": instruction}]
        message.append({"role": "user", "content": prompt})
        function_name = "print_parameters"
        function_description = "print the generated list of {} of length {} from assistant ".format(object, length)
        # print(key_list)
        for i in range(20):
            try:
                response = openai.ChatCompletion.create(
                    ...
                    functions=[{
                        ...
                        "parameters": {
                            ...
                            "properties": {"array": {
                                "type": "string",
                                "description": "the string of generated list of {}".format(object),
                            }}},
                        "required": ["array"],
                    }],
                    function_call="auto",
                ...
                if response.get("function_call"):
                    arguments = eval(response["function_call"]["arguments"])
                    # print(response)
                    if "array" not in arguments:
                        raise Exception("No function call.")
                    if len(arguments["array"].split(",")) != length:
                        raise Exception("No function call.")
                    if arguments["array"].startswith("[") and arguments["array"].endswith("]"):
                        # remove the brackets
                        arguments["array"] = arguments["array"][1:-1]
                    # turn in into a list
                    arguments["array"] = arguments["array"].split(",")
                    for i in range(len(arguments["array"])):
                        # remove " if any
                        arguments["array"][i] = arguments["array"][i].replace("\"", "")
                        # remove space if any
...
def dimension_generator(object: str, length: int):
    instruction = "To characterize a thing, we need to look at it from different perspectives. Generate a list of dimensions to describe " + object + " in an array"
    prompt = object
    length = length
