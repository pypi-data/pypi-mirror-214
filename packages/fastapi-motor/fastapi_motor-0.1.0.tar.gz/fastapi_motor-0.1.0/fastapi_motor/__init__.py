...
class BaseModel:
    __coll__: Union[str, None] = None  # collection name
    __dbkey__: Union[str, None] = None  # which database connected to?
    __unique_fields__: list[str] = []
    __motor_client__: Any = None
    __motor_db__: Any = None
    __motor_clients__: Any = {}
    __motor_dbs__: Any = {}
    __app__: Any = None
    __apps__: Any = {}
    __timezone__: Any = None
    ...
    def init_app(
        ...
        cls.__app__ = app
        cls.__apps__[name] = app

        app.add_event_handler(
            "startup",
            cls.default_open_connection(app, uri, name, read_concern, write_concern),
        )
        app.add_event_handler("shutdown", cls.default_close_connection(app))

    @classmethod
    def default_open_connection(cls, app, uri, name, read_concern, write_concern):
        async def start_app() -> None:
            logger.info(f"opening motor connection for [{name}]")
            cls.connect_database(app, name, uri, read_concern, write_concern)
            app.state.motor_client = app.state.motor_clients[name]

        return start_app

    @classmethod
    def connect_database(cls, app, name, uri, read_concern, write_concern):
        client = AsyncIOMotorClient(uri)
        # db = client.get_database()
        # app.state.motor_client.get_io_loop = asyncio.get_running_loop
        ...
        cls.__motor_db__ = db
        if not hasattr(app.state, "motor_clients"):
            app.state.motor_clients = {}

        # for closing use
        app.state.motor_clients[name] = client
        cls.__motor_clients__[name] = client
        cls.__motor_dbs__[name] = db

    @staticmethod
    def default_close_connection(app):
        async def stop_app() -> None:
            if hasattr(app.state, "motor_clients"):
                for name, client in app.state.motor_clients.items():
    ...
    def get_collection(cls, db=None):
        ...
        if not db:
            db = cls.__dbkey__ or cls.__motor_client__.db_name
    ...
    async def find(cls, request=None, *args, **kwargs):
        ...
        _, per_page, skip = cls.get_page_args(
    ...
    async def find_one_and_delete(cls, *args, **kwargs):
        ...
        return await cls.get_collection(db).find_one_and_delete(*args, **kwargs)

    @classmethod
    async def find_one_and_replace(cls, *args, **kwargs):
        db = kwargs.pop("db", None)
        kwargs.update(sort=get_sort(kwargs.pop("sort", None)))
        return await cls.get_collection(db).find_one_and_replace(*args, **kwargs)

    @classmethod
    async def find_one_and_update(cls, *args, **kwargs):
        db = kwargs.pop("db", None)
        kwargs.update(sort=get_sort(kwargs.pop("sort", None)))
        return await cls.get_collection(db).find_one_and_update(*args, **kwargs)
