...
class Ring:
    ...
    def process_data(self, data_column_name=None):
        ...
        print(self.output_dataframe.info())
    ...
    async def EdgeChat(self, prompt):
        try:
    ...
    async def run(self):
        self.output_dataframe['service'] = ""
        self.output_dataframe['response'] = ""
        self.output_dataframe['response_clean'] = ""
        for i, row in self.output_dataframe.iterrows():
            if self.data_column_name:
                ...
                prompt = self.instruction_prompt + row['corpus']
            ...
            try:
                if token['service'] == "EdgeChat":
                    response = await self.EdgeChat(prompt)
