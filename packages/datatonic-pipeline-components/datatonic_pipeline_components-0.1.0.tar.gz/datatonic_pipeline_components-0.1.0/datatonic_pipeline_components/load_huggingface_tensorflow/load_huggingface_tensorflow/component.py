"""FKP component to load TensorFlow model from HuggingFace into GCS"""
import os
...
def load_tf_hf_model(
    ...
    """
    Loads TensorFlow model, config and tokenizer from HuggingFace
    and stores artefacts in GCS

    Args:
        model_class_name [str]: HuggingFace class to import model from (ie: TFAutoModelForCasualLM)
        tokenizer_class_name [str]: HuggingFace class to import tokenizer from (ie: TFAutoTokenizer)
        config_name [str]: HuggingFace class to import config from (ie: TFAutoConfig)
    ...
    if not model_class_name.startswith("TF"):
        raise NameError(
            """
        This component only accepts TensorFlow models but you selected
        a PyTorch model. Please use the other component in this library for PyTorch models
        """
        )
