...
def allSeshMeta(tp,tps):
    ...
    if allMeta['madeBy']=='Incucyte':
        
        # find time of session start datetime from first alphabetically first filename
        tp0 = sorted(tps)[0]
        inc_datetime_reg = re.compile(r'_\d{4}y\d{2}m\d{2}d_\d{2}h\d{2}m.tif$')
        date_string = re.search(inc_datetime_reg,tp0).group(0)
        datetime_format = '_%Yy%mm%dd_%Hh%Mm.tif'
        allMeta['startMom'] = datetime.strptime(date_string, datetime_format)        
        ...
        inc_DTReg = re.compile(r'_\d{4}y\d{2}m\d{2}d_\d{2}h\d{2}m.tif$')
        DTformat = '_%Yy%mm%dd_%Hh%Mm.tif'
        allDTs = [datetime.strptime(re.search(inc_DTReg,tpi).group(0),DTformat) for tpi in tps]
        allMeta['TStep'] = min([abs(DTi - allDTs[0]) for DTi in allDTs if DTi!=allDTs[0]])        
        ...
        # have contacted incucyte to ask where to get pixel sizes
        allMeta['pixSizeY'] = 1
        allMeta['pixSizeX'] = 1
        allMeta['pixUnit'] = 'Inch'
