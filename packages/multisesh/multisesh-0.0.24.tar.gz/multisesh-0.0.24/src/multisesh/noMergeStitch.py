...
def noMergeStitch2(images,centreList,sizes,xMon):
    """
    Stitch montage tiles together with no merging based on a provided list of 
    centres.
    
    Parameters
    ----------
    images : numpy array
        The dimensions are (NM,NY,NX), i.e. all the tiles to be montaged are 
        in the first dimension.
    centreList : list
        The list of centres as given by findCentres().
    sizes : tuple
        The size of the returned image - (ySizeOut,xSizeOut). The top and left 
        are cropped or padded with zeros to make it this size.
    xMon : int
        The number of tiles along x.
    
    Notes
    ------
    This version of the function does it in the 2nd most stupid way. Images 
    are partially cropped so overlapping sections generally consist of half 
    one image and half the next. Its easy to allow blank sections if you 
    don't think carefully about which edges to crop and which to allow to be 
    overlaid upon. We do it in a bit of a lazy but satifactory way here.
    """
    ...
    # make limits of the images
    limits = []
    limits2 = [[[0,ysize,1],[0,xsize,1]] for i in centres] # start with all
    for c,cen in enumerate(centres):
        if c == 0: # first tile goes completely in
            limits.append(imageRange_M(cen,ysize,xsize))
        elif c < xMon: # top row tiles have just left side cropped
            LXMax = imageRange_M(centres[c-1],ysize,xsize,False)[1][1]
            slices = imageRange_M(cen,ysize,xsize,False)
            CXMin = slices[1][0]
            if CXMin < LXMax: # only do this correction if there is overlap
                # otherwise you leave the black space...
        ...
        elif c % xMon == 0: # non-top row, left-most tile just has top cropped
            ...
            limits.append((slice(*slices[0]),slice(*slices[1])))
        else: # the rest have top and right clipped
            LXMax = imageRange_M(centres[c-1],ysize,xsize,False)[1][1]
            slices = imageRange_M(cen,ysize,xsize,False)
            CXMin = slices[1][0]
            if CXMin < LXMax:
...
def imageRange_M(centre_M,ysize,xsize,returnSlice=True):
    """
    This returns the edge coordinates of an image within another larger image, 
    given the coordinates of the centre of the smaller image within the larger 
    image, as well as the size of the smaller image.
    
    Parameters
    ----------
    centre_M : tuple
        The (y,x) coordinates of the centre of the smaller image, in the 
        coordinate system of the larger image.
    ysize,xsize : int
        The size of pixels in each dimension.
        
    Returns
    -------
    slices : list
        The y,x [slicey,slicex] slices that define the position of the smaller 
        image within the larger.
