        ...
        def endpoint_func(pipe: SmartPipe = smart_pipe) -> Any:            
            if request.method == "POST" or request.method == "PUT":
                ...
                if is_batch_processing(body):
                    ...
                    content["_metadata"] = metadata                    
                    loop.run_until_complete(
                        publish_message(pipe.id, id, content, 1, smart_pipe_first_coprocessors)
                    )
                    requests_datasource.save_request(id, 1)
...
def start() -> Optional[Flask]:
    ...
    if not coprocessor_id:
        ...
        save_result = bool(os.getenv("SAVE_RESULT_COPROCESSOR"))
