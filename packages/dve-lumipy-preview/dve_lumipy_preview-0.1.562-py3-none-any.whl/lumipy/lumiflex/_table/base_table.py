...
class BaseTable(Node, ABC):
    ...
    """
    client_: Union[None, Client] = Field(alias='client')
    from_: Optional[StrictStr] = None
    ...
    def _assert_in_table(self, name, *cols):
        ...
        if len(errs) > 0:
            err_str = indent_str("\n".join(errs), 4)
            raise ValueError(
                f'There are columns in the input to {name} that do not belong to {self._get_name()}:\n{err_str}'
            )
    ...
    def get_columns(self, main_only: Optional[bool] = False) -> Tuple[Column, ...]:
        ...
        """
        cols = tuple(self._add_prefix(c) for c in self.__dict__.values() if isinstance(c, Column))
        if main_only:
