...
class Status(Enum):
    ...
    REQUESTED, IN_PROGRESS, and COMPLETED; and REQUESTED, IN_PROGRESS, and
    ...
    actions skip directly from REQUESTED to either COMPLETED or FAILED.
    """
    REQUESTED = 10
    IN_PROGRESS = 20
    COMPLETED = 30
...
class Activity(DataObject, SameMimeType, ABC):
    ...
    def generate_requested_datetime(self):
        """
        Sets the requested attribute to the current datetime.
        """
        self.requested =  datetime.now().astimezone()
...
class Action(Activity, ABC):
    ...
    def __init__(self):
        super().__init__()
        self.__requested: date | None = None
    ...
    @requested.setter
    def requested(self, requested: date | None):
        if requested is None or isinstance(requested, date):
