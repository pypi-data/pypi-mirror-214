...
class Engine:
    ...
    def split_sentence(seg_src: str, max_len: int):
    ...
    async def process_batch(self, batch: List, lock: Lock = None):
        ...
        for i, src in enumerate(batch):
            ...
            # So we split long segments and translate them before 
                ...
                chunked_segment = split_sentence(seg.src, max_len)
                if lock is not None:
                async with lock:
