...
class MQNDDF:
    ...
    def __init__(self, data,year,sent = "inputvar=outputvar", gy = [1], gx = [1], gb = [1], rts=RTS_VRS, emf=EMF_SAME,\
                 tech = TOTAL,dynamic = LUE,solver="mosek"):
        ...
        if self.tech== TOTAL:
            ...
            for tindex in self.tlt.index:
                ...
                model =NDDF(data,sent = sent, gy =gy, gx =gx, gb = gb,\
            ...
            dataz11 = pd.DataFrame()      # D11
            for tindex in self.tlt.index:
                model = NDDF(data=data,sent=sent, gy =gy, gx =gx, gb = gb, rts=rts,emf=emf,\
                                baseindex="{}=[{}]".format(year,self.tlt.iloc[tindex]),\
                                refindex="{}=[{}]".format(year,self.tlt.iloc[tindex]))
                data11  = model.optimize(solver)
                dataz11 = pd.concat([dataz11,data11])

            datazz = pd.concat([data,dataz11[["obj"]]],axis=1).rename(columns = {"obj":"D11"} )
            ...
            datazz = pd.concat([datazz,dataz12[["obj"]]],axis=1).rename(columns = {"obj":"D12"} )
            ...
            datazz = pd.concat([datazz,dataz21[["obj"]]],axis=1).rename(columns = {"obj":"D21"} )
