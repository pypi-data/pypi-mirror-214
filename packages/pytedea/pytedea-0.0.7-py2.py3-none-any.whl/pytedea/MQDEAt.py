...
class MQDEAt:
    ...
    def __init__(self, data,year,sent = "inputvar=outputvar",  orient=ORIENT_IO, rts=RTS_VRS, emf=EMF_SAME,\
        ...
        self.outputvars, self.inputvars  = tools.assert_valid_mqdeat(sent )

        self.xcol = self.inputvars
        self.ycol = self.outputvars
        if orient in [ORIENT_IO, ORIENT_OO]:
            self.orient = orient
            self.xindexs = None
            self.yindexs = None
        else:
        ...
        if self.tech== TOTAL:
            ...
            for tindex in self.tlt.index:
                model =DEAt(data,sent = sent,  orient=orient,\
            ...
            dataz11 = pd.DataFrame()      # D11
            for tindex in self.tlt.index:
                model = DEAt(data=data,sent=sent, orient=orient, rts=rts,\
