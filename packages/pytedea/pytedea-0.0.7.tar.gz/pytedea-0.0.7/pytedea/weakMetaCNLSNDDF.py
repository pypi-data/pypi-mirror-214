...
from .constant import CET_ADDI, FUN_COST, FUN_PROD, RTS_VRS, RTS_CRS,OPT_DEFAULT, OPT_LOCAL,RED_QLE
from .utils import tools
from . import  weakCNLSy,weakCNLSNDDF,StoNED
import pandas as pd
import numpy as np


class weakMetaCNLSNDDF(weakCNLSy.weakCNLSy):
    """Convex Nonparametric Least Square with non-radial directional distance function
    """

    def __init__(self,data,Eineff,sent, z=None, gy=[1], gx=[1], gb=[1],weight =None, \
        ...
        Args:
            ...
            group(string,optional): The group to calculate E(u_i|epsilon_i).
        ...
        self.weight2 = self.iweight+self.oweight+self.bweight
        ...
        if self.rts == RTS_VRS:
            if type(self.z) != type(None):
                def regression_rule(model, i):
                    return self.actrual_value.loc[i,self.deduce]+self.Eineff.loc[i,'Eineff1']\
                        == model.alpha[i] \
