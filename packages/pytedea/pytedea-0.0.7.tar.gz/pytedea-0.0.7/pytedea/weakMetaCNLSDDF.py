...
class weakMetaCNLSDDF(weakCNLSy.weakCNLSy):
    ...
    def __init__(self, data,Eineff,sent = "inputvar=outputvar:unoutputvar",z=None,  gy=[1], gx=[1], gb=[1],\
        ...
        self.__model__.objective = Objective(rule=self._weakCNLSy__objective_rule(),
                                             sense=minimize,
    ...
    def __translation_property(self):
        ...
        def translation_rule(model, i):
            return sum(model.beta[i, k] * self.gx[k] for k in model.K) \
                + sum(model.gamma[i, l] * self.gy[l] for l in model.L) \
                + sum(model.delta[i, j] * self.gb[j] for j in model.J) == 1
