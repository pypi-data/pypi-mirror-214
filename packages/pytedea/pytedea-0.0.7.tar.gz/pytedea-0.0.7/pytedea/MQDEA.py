...
from .constant import CET_ADDI, ORIENT_IO, ORIENT_OO,ORIENT_UO, RTS_VRS, RTS_CRS, OPT_DEFAULT, OPT_LOCAL,\
            TOTAL,CONTEMPORARY,EMF_SAME,EMF_DIFFERENT
...
class MQDEA:
    ...
    def __init__(self, data,year,sent = "inputvar=outputvar",  orient=ORIENT_IO, rts=RTS_VRS, emf=EMF_SAME,\
                 tech = TOTAL,solver="mosek"):
        ...
        self.outputvars, self.inputvars ,self.unoutputvars = tools.assert_valid_mqdea(sent )

        self.xcol = self.inputvars
        self.ycol = self.outputvars
        self.bcol = self.unoutputvars
        ...
        if self.tech== TOTAL:
            ...
            for tindex in self.tlt.index:
                model =DEA(data,sent = sent,  orient=orient,\
            ...
            dataz11 = pd.DataFrame()      # D11
            for tindex in self.tlt.index:
                model = DEA(data=data,sent=sent, orient=orient, rts=rts,emf=emf,\
                                baseindex="{}=[{}]".format(year,self.tlt.iloc[tindex]),\
                                refindex="{}=[{}]".format(year,self.tlt.iloc[tindex]))
                data11  = model.optimize(solver)
                dataz11 = pd.concat([dataz11,data11])

            datazz = pd.concat([data,dataz11[["te"]]],axis=1).rename(columns = {"te":"D11"} )
            ...
            datazz = pd.concat([datazz,dataz12[["te"]]],axis=1).rename(columns = {"te":"D12"} )
            ...
            datazz = pd.concat([datazz,dataz21[["te"]]],axis=1).rename(columns = {"te":"D21"} )
