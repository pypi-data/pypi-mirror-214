...
class QConnection(Q):
    ...
    def _send_sock(self,
        ...
        msg_view = _wrappers._to_bytes(6, K(query), 1 if wait else 0)
...
class RawQConnection(QConnection):
    ...
    def _send_sock_server(self, sock, response, level):
        try:
            if isinstance(response, tuple):
                # Return an error
                response = response[1]
                msg_view = list(bytes(_wrappers._to_bytes(level, response, 2)))
                msg_view[8] = 128
    ...
    def _poll_server(self, amount: int = 1): # noqa
        ...
        try:
            ...
            if level != -1:
                ...
                reader.register(conn, selectors.EVENT_READ, self._recv_socket_server)
                writer = selectors.DefaultSelector()
                writer.register(conn, selectors.EVENT_WRITE, self._send_sock_server)
    ...
    def clean_open_connections(self):
        for i in range(len(self.open_cons) - 1, -1, -1):
            try:
                self.open_cons[i][2].send(
                    bytes(_wrappers._to_bytes(self.open_cons[i][3], q('::'), 0))
                )
            except BaseException:
