    ...
    def __init__(self):
        ...
        if licensed or '_connection_info' in self.__dict__:
            if '_connection_info' in self.__dict__ and self._connection_info['no_ctx']:
                object.__setattr__(self, 'ctx', QContext(self, '', None, no_ctx=True))
                pass
            else:
                object.__setattr__(self, 'ctx', QContext(self, '', None))
                object.__setattr__(self, '_q_ctx_keys', {
                    *self.ctx.q._context_keys,
                    *self.reserved_words,
                })
        object.__setattr__(self, 'console', QConsole(self))
        object.__setattr__(self, 'insert', Insert(self))
        object.__setattr__(self, 'upsert', Upsert(self))
        object.__setattr__(self, 'query', None) # placeholder
        object.__setattr__(self, 'qsql', QSQL(self))
        object.__setattr__(self, 'sql', SQL(self))
        object.__setattr__(self, 'read', QReader(self))
        object.__setattr__(self, 'write', QWriter(self))
        object.__setattr__(self, 'system', SystemCommands(self))
