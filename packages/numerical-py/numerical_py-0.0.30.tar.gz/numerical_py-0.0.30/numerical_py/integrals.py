    ...
    def rectangles_rule(self, rect_type="middle") -> float:
        ...
        if rect_type == "left":
            for i in range(0, self.n):
                rectangles_integral += self.f(self.a + (i * self.h)) * (
                    (self.a + (i + 1) * self.h) - (self.a + i * self.h)
                )
        elif rect_type == "right":
            for i in range(1, self.n + 1):
                rectangles_integral += self.f(self.a + (i * self.h)) * (
                    (self.a + i * self.h) - (self.a + (i - 1) * self.h)
                )
        elif rect_type == "middle":
            for i in range(0, self.n):
                rectangles_integral += self.f(
                    (self.a + (i * self.h) + self.a + ((i + 1) * self.h)) / 2
                ) * ((self.a + (i + 1) * self.h) - (self.a + i * self.h))
        else:
            print("Correct options: left\nright\nmiddle")
