...
class Equations:
    def __init__(
        self, f: Callable, a: float, b: float, eps: float, max_iterations: int
    ) -> None:
        self.f = f
        self.a = a
        self.b = b
    ...
    def iteration_method(self, g: Callable , x0: float) -> float:
    ...
    def dihotomy(self) -> float:
        if (self.f(self.a) * self.f(self.b) >= 0):
            print("f(a) should be negative, f(b) should be positive")
            return

        current_iter = 0
        c = self.a
        while ((self.b-self.a) >= 2*self.eps) and current_iter < self.max_iterations:
            c = (self.a+self.b)/2
    ...
    def newton_rule(self, df_dx: Callable, x0: float) -> Tuple[float, float]:
        current_iter = 0
        x1 = x0 - self.f(x0) / df_dx(x0)
