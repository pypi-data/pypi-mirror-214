...
class Integrate:
    ...
    def cotes_rule(self, x_values: np.ndarray, f_x_values: np.ndarray) -> float:
        a, b = sp.symbols('a b')
        L5_integr = np.sum([Integrate.cotes_coef(i, len(x_values)) * f_x_values[i] for i in range(len(x_values))])
    ...
    def simpson_rule(self) -> float:
        ...
        for step in range(self.n):
            ...
            simpson_integral += (x2 - x1) / 6.0 * (self.f(x1) + 4.0 * self.f(0.5 * (x1 + x2)) + self.f(x2))
