...
from sj_tool.fjsp.erp_next.util import select_data_from_table, connect_mysql
...
def transfer_sale_order(conn, owner="Administrator"):
    print("同步销售订单相关表数据。。。")
    ...
    # 客户表
    customers = select_data_from_table(conn, table_sale_customer, where_clause=f"owner = %s", where_params=(owner,))
    customer_dict = {}
...
def transfer_bom(conn, owner="Administrator"):
    print("同步BOM表和替换料表数据。。。")
    st = time.perf_counter()
    table_bom = "tabBOM"
    ...
    bom_children_dict = select_data_from_table(
        ...
        columns=["item_code", "qty_consumed_per_unit", "parent"],
        where_clause=f"owner = %s and docstatus = %s",
        where_params=(owner, 1),
        to_dict=True,
        dict_key="parent",
    ...
    rows = select_data_from_table(
        conn,
        table_alternative,
        columns=["item_code", "alternative_item_code", "qty", "priority", "two_way"],
        where_clause=f"owner = %s",
        where_params=(owner,),
    )
    result_alternative = []
...
def transfer_inventory(conn, owner="Administrator"):
    print("同步物料库存数据。。。")
    st = time.perf_counter()
    # 物料表
    table_item = "tabItem"
    ...
    for record in out_records:
        for item in out_data_detail[record["name"]]:
            ...
            if item_code not in out_inventory:
                out_inventory[item_code] = {record["posting_date"]: item["qty"]}
    ...
    for part_number, part_quantity in on_hand_inventory.items():
        result_on_hand.append({"part_number": part_number, "part_quantity": part_quantity})
    ...
    inventory_on_hand = pd.DataFrame(result_on_hand)
    # df_bom.to_csv(filename, index=False)

    # filename = os.path.join(get_project_root(), "output", "inventory_in_transit.csv")
    inventory_in_transit = pd.DataFrame(result_in_transit)
    # df_bom.to_csv(filename, index=False)

    et = time.perf_counter()
    print(f"消耗时间: {round(et - st, 3)} s")
    return {"inv_on_hand": inventory_on_hand, "inv_in_transit": inventory_in_transit}


def transfer_wip(conn, owner="Administrator"):
    print("同步在制库存数据。。。")
    st = time.perf_counter()
    table_wip = "tabWIP"
    ...
    results = []
    for row in wip_data:
        results.append({"mo": row["mo"], "so": row["so"], "unfinished_qty": row["unfinished_qty"]})
...
def transfer_process_table(conn, owner="Administrator"):
    table_process = "tabOperation"

    process_table = select_data_from_table(
        conn, table_process, columns=["operationnumber", "name"], where_clause=f"owner = %s", where_params=(owner,)
    )
    results = []
    for row in process_table:
        results.append({"id": row["operationnumber"], "name": row["name"]})
...
def transfer_machine_table(conn, owner="Administrator"):
...
# 设备停运
def transfer_machine_stop(conn, owner="Administrator"):
...
def transfer_prod_process_table(conn, owner="Administrator"):
    print("同步产品-工艺表")
    st = time.perf_counter()
...
def transfer_standing_table(conn, owner="Administrator"):
    print("同步静置时间表")
    st = time.perf_counter()
    table_prod_process = "tabProd process"
...
def transfer_changeover_table(conn, owner="Administrator"):
    print("同步换型时间表")
    st = time.perf_counter()
...
def tansfer_processtime_table(conn, owner="Administrator"):
    print("同步设备生产时间")
    st = time.perf_counter()
    ...
    processtime_table = select_data_from_table(
        conn,
        table_prod_process,
        columns=["item_code", "workstation_name", "operation", "process_time"],
        where_clause=f"owner = %s",
        where_params=(owner,),
    )
...
def get_data(owner, need_data_list):
    """
    owner：用户邮箱
    need_data_list：需要获取的所有数据名的列表，[key1, key2, ...]，key需要在key_dict中
    ...
    for k in need_data_list:
        if k in key_dict:
            result_dict.update(key_dict[k](conn, owner))
    conn.close()
    return result_dict


if __name__ == "__main__":
    result = get_data("1091076149@qq.com", list({
        "machine": transfer_machine_table,
        "standing": transfer_standing_table,
        "changeover": transfer_changeover_table,
        "process_time": tansfer_processtime_table,
