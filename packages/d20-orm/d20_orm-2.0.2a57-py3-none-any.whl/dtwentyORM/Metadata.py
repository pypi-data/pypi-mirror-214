...
class Metadata():
    ...
    class DataField(BasicElement):                    
        ...
        def build(self):
            ...
            qf = self.build_filter_string_from_dict({'obj_type' : self.get('obj_type')})
        ...
        def add(self, dfs):
            self.wipeByFilter(query_list=[{'obj_type':dfs[0].get('obj_type')}])
            for df in dfs:
                to_insert = {}
        ...
        def install(self):
            self.wipeByFilter(query_list=[{'obj_type':self.get('obj_type')}])
