...
class DBNodeTree(DBItem):
    """DBNodeTree Class.
    Object Relational Mapping for NodeTree.
    ...
    def __init__(self, uuid=None) -> None:
        ...
        self.worker_name = self.record["metadata"]["worker_name"]
    ...
    def edit_node_from_yaml(self, uuid, filename=None, string=None):
        ...
        # print("edit_node_from_yaml, ndata: ", ndata)
        ntdata = get_nt_full_data({"uuid": self.uuid})
        ntdata["nodes"][ndata["name"]] = deepcopy(ndata)
        self.save(ntdata)
        return ndata
    ...
    def copy(
        ...
        missing_node_type="REF",
        missing_node_other_type=[],
    ):
        ...
        if add_missing_node:
            ...
            for name in missing_nodes:
                is_ref = True if missing_node_type == "REF" else False
                if name in missing_node_other_type:
                    is_ref = not is_ref
                ndata = record_nodes[name]
                node = DBNode(uuid=ndata["uuid"])
                ndata = node.copy(
        ...
        for link in record["links"]:
            if link["from_node"] in namelists and link["to_node"] in namelists:
                ...
                from_socket = [
                    s for s in from_node["outputs"] if s["name"] == link["from_socket"]
                ][0]
                link["from_socket_uuid"] = from_socket["uuid"]
                #
                to_node = record["nodes"][link["to_node"]]
                to_socket = [
                    s for s in to_node["inputs"] if s["name"] == link["to_socket"]
                ][0]
