...
def save_results(run_id, y_true, y_predicted, problem_type = 'binary classification', hist_bins=20):
      
    y_predicted = np.array(y_pred_list).flatten()
    y_true = np.array(y_label_list).flatten()
    ...
    data = {'run_id' : run_id, 'precision': round(prec, 4), 'recall': round(rec, 4), 'specificity': round(spec, 4),
            ...
            'username': username, 'key': key, 'test_auc': roc_auc, 'freq': freq, 'bins': hist_bins,
            'fpr': fpr, 'tpr': tpr}   
...
def dataset(run_id, dataset_dict = {}):
  data = {'run_id' : run_id, 'dataset_dict': str(dataset_dict), 'username': username, 'key': key}
  
  response = requests.post(protocol+'://'+IP+'/api/add_dataset', data=data)


def variables(run_id, variables_dict = {}):
  data = {'run_id' : run_id, 'variables_dict': str(variables_dict), 'username': username, 'key': key}
