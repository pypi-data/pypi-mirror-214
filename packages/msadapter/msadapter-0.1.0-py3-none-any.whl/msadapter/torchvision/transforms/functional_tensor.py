...
from msadapter.pytorch.nn.functional import grid_sample, conv2d, interpolate, pad as torch_pad
...
#     i = i % 6
#
#     mask = i.unsqueeze(dim=-3) == torch.arange(0, 6, device=None).view(-1, 1, 1)
#
...
def _pad_symmetric(img: Tensor, padding: List[int]) -> Tensor:
    ...
    temp_x_indices = left_indices + _x_indices + right_indices
    temp_x_indices = [i if i >= 0 else i + in_sizes[-1] for i in temp_x_indices]
    x_indices = torch.tensor(temp_x_indices, device=None)

    _y_indices = [i for i in range(in_sizes[-2])]
    top_indices = [i for i in range(padding[2] - 1, -1, -1)]
    bottom_indices = [-(i + 1) for i in range(padding[3])]
    temp_y_indices = top_indices + _y_indices + bottom_indices
    temp_y_indices = [i if i >= 0 else i + in_sizes[-2] for i in temp_y_indices]
    y_indices = torch.tensor(temp_y_indices, device=None)
...
def _cast_squeeze_out(img: Tensor, need_cast: bool, need_squeeze: bool, out_dtype) -> Tensor:
    if need_squeeze:
        img = img.squeeze(dim=0)
...
def _apply_grid_transform(img: Tensor, grid: Tensor, mode: str, fill: Optional[List[float]]) -> Tensor:
    ...
    if fill is not None:
        dummy = torch.ones((img.shape[0], 1, img.shape[2], img.shape[3]), dtype=img.dtype, device=None)
...
def _gen_affine_grid(
    ...
    d = 0.5
    x_grid = torch.linspace(-ow * 0.5 + d, ow * 0.5 + d - 1, steps=ow, device=None)
    x_grid = ms.ops.broadcast_to(x_grid, (1, oh, ow))
    y_grid = torch.linspace(-oh * 0.5 + d, oh * 0.5 + d - 1, steps=oh, device=None)
...
def affine(
    ...
    dtype = img.dtype if torch.is_floating_point(img) else torch.float32
    theta = torch.tensor(matrix, dtype=dtype, device=None).reshape(1, 2, 3)
    shape = img.shape
...
def _compute_output_size(matrix: List[float], w: int, h: int) -> Tuple[int, int]:
    ...
    tol = 1e-4
    cmax = torch.ceil((max_vals / tol).trunc() * tol)
    cmin = torch.floor((min_vals / tol).trunc() * tol)
    size = cmax - cmin
...
def _perspective_grid(coeffs: List[float], ow: int, oh: int, dtype, device) -> Tensor:
    ...
    y_grid = ms.ops.broadcast_to(y_grid, (1, oh, ow))
    z_grid = ms.ops.ones((1, oh, ow), ms.float32)
    base_grid = ms.ops.stack((x_grid, y_grid, z_grid), 3).to(dtype)
...
def perspective(
    ...
    dtype = img.dtype if torch.is_floating_point(img) else torch.float32
    grid = _perspective_grid(perspective_coeffs, ow=ow, oh=oh, dtype=dtype, device=None)
    return _apply_grid_transform(img, grid, interpolation, fill=fill)
...
def _get_gaussian_kernel2d(
    kernel_size: List[int], sigma: List[float], dtype, device
) -> Tensor:
...
def gaussian_blur(img: Tensor, kernel_size: List[int], sigma: List[float]) -> Tensor:
    ...
    dtype = img.dtype if torch.is_floating_point(img) else torch.float32
    kernel = _get_gaussian_kernel2d(kernel_size, sigma, dtype=dtype, device=None)
...
def invert(img: Tensor) -> Tensor:
    ...
    bound = torch.tensor(1 if img.is_floating_point() else 255, dtype=img.dtype, device=None)
    return bound - img
...
def solarize(img: Tensor, threshold: float) -> Tensor:
    ...
    out = torch.where(img >= threshold, inverted_img, img)
    return out
...
def _blurred_degenerate_image(img: Tensor) -> Tensor:
    ...
    kernel = torch.ones((3, 3), dtype=dtype, device=None)
    kernel[1, 1] = 5.0
    kernel /= kernel.sum()
    kernel = kernel.expand(img.shape[-3], 1, kernel.shape[0], kernel.shape[1])
    ...
    result[..., 1:-1, 1:-1] = result_tmp.to(torch.float32)
...
def _scale_channel(img_chan: Tensor) -> Tensor:
    ...
    # TODO: nonzero_hist = hist[hist != 0]
    mask = hist != 0
    nonzero_hist = ms.ops.masked_select(hist, mask)
...
def normalize(tensor: Tensor, mean: List[float], std: List[float], inplace: bool = False) -> Tensor:
    ...
    dtype = tensor.dtype
    mean = torch.as_tensor(mean, dtype=dtype, device=None)
    std = torch.as_tensor(std, dtype=dtype, device=None)
    if (std == 0).any():
...
def erase(img: Tensor, i: int, j: int, h: int, w: int, v: Tensor, inplace: bool = False) -> Tensor:
    ...
    img_dtype = img.dtype
    img = img.to(torch.float32)
    img[..., i : i + h, j : j + w] = v.to(torch.float32)
    return img.to(img_dtype)
