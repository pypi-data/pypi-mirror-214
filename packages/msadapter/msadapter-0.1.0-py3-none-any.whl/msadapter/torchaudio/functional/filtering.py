...
from mindspore.dataset.audio import DensityFunction, Modulation, Interpolation

DENSITY_MODE = {
    "TPDF"  : DensityFunction.TPDF,
    "RPDF"  : DensityFunction.RPDF,
    "GPDF"  : DensityFunction.GPDF,
    "None"  : DensityFunction.TPDF,
}
...
def _generate_wave_table(
    ...
    t = torch.arange(0, table_size, dtype=torch.int32)
    point = (t + phase_offset) % table_size

    d = torch.zeros_like(point, dtype=torch.float64)
...
def allpass_biquad(waveform: Tensor, sample_rate: int, central_freq: float, Q: float = 0.707) -> Tensor:
    ...
    if _is_numpy(waveform):
        op = msaudio.AllpassBiquad(sample_rate=sample_rate, central_freq=central_freq, Q=Q)
        return op(waveform)

    dtype = waveform.dtype
    central_freq = torch.as_tensor(central_freq, dtype=dtype)
...
def band_biquad(
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.BandBiquad(sample_rate=sample_rate, central_freq=central_freq, Q=Q, noise=noise)
        return op(waveform)
...
def bandpass_biquad(
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.BandpassBiquad(sample_rate=sample_rate, central_freq=central_freq, Q=Q, const_skirt_gain=const_skirt_gain)
        return op(waveform)
...
def bandreject_biquad(waveform: Tensor, sample_rate: int, central_freq: float, Q: float = 0.707) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.BandrejectBiquad(sample_rate=sample_rate, central_freq=central_freq, Q=Q)
        return op(waveform)
...
def bass_biquad(
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.BassBiquad(sample_rate=sample_rate, gain = gain, central_freq=central_freq, Q=Q)
        return op(waveform)

    dtype = waveform.dtype
    central_freq = torch.as_tensor(central_freq, dtype=dtype)
    Q = torch.as_tensor(Q, dtype=dtype)
    gain = torch.as_tensor(gain, dtype=dtype)
...
def biquad(waveform: Tensor, b0: float, b1: float, b2: float, a0: float, a1: float, a2: float) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.Biquad(b0=b0, b1=b1, b2=b2, a0=a0, a1=a1, a2=a2)
        return op(waveform)

    dtype = waveform.dtype
    b0 = torch.as_tensor([b0], dtype=dtype)
    b1 = torch.as_tensor([b1], dtype=dtype)
    b2 = torch.as_tensor([b2], dtype=dtype)
    a0 = torch.as_tensor([a0], dtype=dtype)
    a1 = torch.as_tensor([a1], dtype=dtype)
    a2 = torch.as_tensor([a2], dtype=dtype)
...
def contrast(waveform: Tensor, enhancement_amount: float = 75.0) -> Tensor:
    ...
    if _is_numpy(waveform):
        op = msaudio.Contrast(enhancement_amount=enhancement_amount)
        return op(waveform)
...
def dcshift(waveform: Tensor, shift: float, limiter_gain: Optional[float] = None) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.DCShift(shift=shift, limiter_gain=limiter_gain)
        return op(waveform)
...
def deemph_biquad(waveform: Tensor, sample_rate: int) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.DeemphBiquad(sample_rate=sample_rate)
        return op(waveform)
...
def _add_noise_shaping(dithered_waveform: Tensor, waveform: Tensor) -> Tensor:
    ...
    zeros = torch.zeros(1, dtype=error.dtype)
...
def _apply_probability_distribution(waveform: Tensor, density_function: str = "TPDF") -> Tensor:
    ...
    random_channel = (int(np.random.randint(0, channel_size,[1,],).item()) if channel_size > 0 else 0)
    random_time = (int(np.random.randint(0, time_size,[1,],).item()) if time_size > 0 else 0)
    ...
    if density_function == "RPDF":
        ...
        for ws in num_rand_variables * [time_size]:
            ...
            rand_chan = int(np.random.randint(0, channel_size,[1,],).item())
            gaussian += waveform[rand_chan][int(np.random.randint(0,ws,[1,],).item())]


        signal_scaled_dis = signal_scaled + gaussian
    else:
        # dtype needed for https://github.com/pytorch/pytorch/issues/32358
        TPDF = torch.bartlett_window(time_size + 1, dtype=signal_scaled.dtype)
...
def dither(waveform: Tensor, density_function: str = "TPDF", noise_shaping: bool = False) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.Dither(density_function=DENSITY_MODE[density_function], noise_shaping=noise_shaping)
        return op(waveform)
...
def equalizer_biquad(
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.EqualizerBiquad(sample_rate=sample_rate, center_freq=center_freq, gain=gain, Q=Q)
        return op(waveform)

    dtype = waveform.dtype
    center_freq = torch.as_tensor(center_freq, dtype=dtype)
    Q = torch.as_tensor(Q, dtype=dtype)
    gain = torch.as_tensor(gain, dtype=dtype)
...
def filtfilt(
    ...
    """
    if _is_numpy(waveform):
        if isinstance(a_coeffs, (torch.Tensor, np.ndarray)):
            a_coeffs = a_coeffs.tolist()
        if isinstance(b_coeffs, (torch.Tensor, np.ndarray)):
            b_coeffs = b_coeffs.tolist()
        op = msaudio.Filtfilt(a_coeffs=a_coeffs, b_coeffs=b_coeffs, clamp=clamp)
        return op(waveform)
...
def flanger(
    ...
    if _is_numpy(waveform):
        op = msaudio.Flanger(sample_rate=sample_rate, delay=delay, depth=depth, regen=regen, width=width,
                             speed=speed, phase=phase, modulation=MODULATION_MODE[modulation],
    ...
    delay_bufs = torch.zeros(waveform.shape[0], n_channels, delay_buf_length, dtype=dtype)
    delay_last = torch.zeros(waveform.shape[0], n_channels, dtype=dtype)
    ...
    output_waveform = torch.zeros_like(waveform, dtype=dtype)
...
def gain(waveform: Tensor, gain_db: float = 1.0) -> Tensor:
    ...
    if _is_numpy(waveform):
        op = msaudio.Gain(gain_db=gain_db)
        return op(waveform)
...
def highpass_biquad(waveform: Tensor, sample_rate: int, cutoff_freq: float, Q: float = 0.707) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.HighpassBiquad(sample_rate=sample_rate, cutoff_freq=cutoff_freq, Q=Q)
        return op(waveform)

    dtype = waveform.dtype
    cutoff_freq = torch.as_tensor(cutoff_freq, dtype=dtype)
    Q = torch.as_tensor(Q, dtype=dtype)
...
def _lfilter_core_generic_loop(input_signal_windows: Tensor, a_coeffs_flipped: Tensor, padded_output_waveform: Tensor):
    ...
    input_signal_windows = input_signal_windows.permute(2, 0, 1)
    for i_sample in range(input_signal_windows.shape[0]):
...
def lfilter(waveform: Tensor, a_coeffs: Tensor, b_coeffs: Tensor, clamp: bool = True, batching: bool = True) -> Tensor:
    ...
    if _is_numpy(waveform):
        ...
        if isinstance(b_coeffs, (torch.Tensor, np.ndarray)):
            b_coeffs = b_coeffs.tolist()
        op = msaudio.LFilter(a_coeffs=a_coeffs, b_coeffs=b_coeffs, clamp=clamp)
        return op(waveform)
...
def lowpass_biquad(waveform: Tensor, sample_rate: int, cutoff_freq: float, Q: float = 0.707) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.LowpassBiquad(sample_rate=sample_rate, cutoff_freq=cutoff_freq, Q=Q)
        return op(waveform)
...
def overdrive(waveform: Tensor, gain: float = 20, colour: float = 20) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.Overdrive(gain=gain, color=colour)
        return op(waveform)
    ...
    colour = colour / 200
    last_in = torch.zeros(waveform.shape[:-1], dtype=dtype)
    last_out = torch.zeros(waveform.shape[:-1], dtype=dtype)

    temp = waveform * gain + colour

    mask1 = temp < -1
    temp[mask1] = torch.tensor(-2.0 / 3.0, dtype=dtype)
    # Wrapping the constant with Tensor is required for Torchscript

    mask2 = temp > 1
    temp[mask2] = torch.tensor(2.0 / 3.0, dtype=dtype)
...
def phaser(
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.Phaser(sample_rate=sample_rate, gain_in=gain_in, gain_out=gain_out, delay_ms=delay_ms, decay=decay,
    ...
    delay_buf = torch.zeros(waveform.shape[0], delay_buf_len, dtype=dtype)
    ...
    output_waveform = torch.stack(output_waveform_pre_gain_list, dim=1).to(dtype=dtype)
...
def riaa_biquad(waveform: Tensor, sample_rate: int) -> Tensor:
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.RiaaBiquad(sample_rate=sample_rate)
        return op(waveform)
...
def treble_biquad(
    ...
    """
    if _is_numpy(waveform):
        op = msaudio.TrebleBiquad(sample_rate=sample_rate, gain=gain, central_freq=central_freq, Q=Q)
        return op(waveform)
...
def _measure(
    ...
    dftBuf[measure_len_ws:dft_len_ws] = 0
    ...
    spectrum[spectrum_start:spectrum_end]= spectrum[spectrum_start:spectrum_end].mul(mult).add(_d * (1 - mult))
    ...
    noise_spectrum[spectrum_start:spectrum_end] = noise_spectrum[spectrum_start:spectrum_end].mul(_mult).add(_d * (1 - _mult))
    _d = torch.sqrt(
        torch.maximum(
            _zeros,
    ...
    _cepstrum_Buf[spectrum_end: dft_len_ws >> 1] = 0
