...
import msadapter.torchaudio as torchaudio
from msadapter.pytorch import Tensor
from msadapter.torchaudio._internal import module_utils as _mod_utils
...
def _get_complex_dtype(real_dtype):
    if real_dtype == torch.double:
        return torch.cdouble
    if real_dtype == torch.float:
        return torch.cfloat
    # if real_dtype == torch.half: # TODO mindspore don't support complex32
...
def griffinlim(
    ...
    *librosa* [:footcite:`brian_mcfee-proc-scipy-2015`], *A fast Griffin-Lim algorithm* [:footcite:`6701851`]
    ...
    if rand_init:
        angles = torch.rand(specgram.size(), dtype=_get_complex_dtype(specgram.dtype))
    else:
        angles = torch.full(specgram.size(), 1, dtype=_get_complex_dtype(specgram.dtype))

    # And initialize the previous iterate to 0
    tprev = torch.tensor(0.0, dtype=specgram.dtype)
...
def amplitude_to_DB(
    ...
    """
    if _is_numpy(x):
        multiplier = "power" if multiplier==10 else "magnitude"
        op = msaudio.AmplitudeToDB(stype=SCALE_MODE[multiplier],ref_value=1.0, amin=amin, top_db=top_db)
        return op(x)
    ...
    if top_db is not None:
        ...
        x_db = torch.maximum(x_db, (x_db.amax(dim=(-3, -2, -1)) - top_db).view(-1, 1, 1, 1))
...
def DB_to_amplitude(x: Tensor, ref: float, power: float) -> Tensor:
    ...
    """
    if _is_numpy(x):
        op = msaudio.DBToAmplitude(ref=ref, power=power)
        return op(x)
...
def create_dct(n_mfcc: int, n_mels: int, norm: Optional[str]) -> Tensor:
    ...
    n = torch.arange(0, float(n_mels))
    k = torch.arange(0, float(n_mfcc)).unsqueeze(1)
...
def mu_law_encoding(x: Tensor, quantization_channels: int) -> Tensor:
    ...
    """
    if _is_numpy(x):
        op = msaudio.MuLawEncoding(quantization_channels=quantization_channels)
        return op(x)
...
def mu_law_decoding(x_mu: Tensor, quantization_channels: int) -> Tensor:
    ...
    """
    if _is_numpy(x_mu):
        op = msaudio.MuLawDecoding(quantization_channels=quantization_channels)
        return op(x_mu)
...
def phase_vocoder(complex_specgrams: Tensor, rate: float, phase_advance: Tensor) -> Tensor:
    ...
    if _is_numpy(complex_specgrams):
        ...
        op = msaudio.PhaseVocoder(rate=rate, phase_advance=phase_advance)
        out = op(complex_specgrams)
        real = out[..., 0]
        imag = out[..., 1] * 1j
        out = real + imag
        return out
    ...
    time_steps = torch.arange(0, complex_specgrams.size(-1), rate, dtype=real_dtype)

    alphas = time_steps % 1.0
    phase_0 = complex_specgrams[..., :1].angle()

    # Time Padding
    # complex_specgrams = torch.nn.functional.pad(complex_specgrams, [0, 2]) #TODO
    complex_specgrams = ms.ops.pad(complex_specgrams, [0,2])
    complex_specgrams = torch.cast_to_adapter_tensor(complex_specgrams)
...
def mask_along_axis_iid(
    ...
    """
    if _is_numpy(specgrams):
        mask_param = _get_mask_param(mask_param, p, specgrams.shape[axis])
        op = msaudio.MaskAlongAxisIID(mask_param=mask_param, mask_value=mask_value, axis=axis)
        return op(specgrams)
    ...
    # value = torch.rand(specgrams.shape[:2], dtype=dtype) * mask_param
    # min_value = torch.rand(specgrams.shape[:2], dtype=dtype) * (specgrams.size(axis) - value)
    value = torch.tensor(np.random.random(specgrams.shape[:2]), dtype=dtype) * mask_param
    min_value =  torch.tensor(np.random.random(specgrams.shape[:2]), dtype=dtype) * (specgrams.size(axis) - value)

    # Create broadcastable mask
    mask_start = min_value.long()[..., None, None]
    mask_end = (min_value.long() + value.long())[..., None, None]
    mask = torch.arange(0, specgrams.size(axis), dtype=dtype)
...
def mask_along_axis(
    ...
    if _is_numpy(specgram):
        ...
        mask_start, mask_param = _get_mask_start(mask_param, specgram.shape[axis])
        op = msaudio.MaskAlongAxis(mask_start=mask_start, mask_width=mask_param, mask_value=mask_value, axis=axis)
        return op(specgram)
...
def compute_deltas(specgram: Tensor, win_length: int = 5, mode: str = "replicate") -> Tensor:
    ...
    """
    if _is_numpy(specgram):
        op = msaudio.ComputeDeltas(win_length=win_length, pad_mode=PAD_MODE[mode])
        return op(specgram)
    ...
    # specgram = torch.nn.functional.pad(specgram, (n, n), mode=mode) #TODO
    specgram = ms.ops.pad(specgram, (n, n), mode=mode)
...
def detect_pitch_frequency(
    ...
    if _is_numpy(waveform):
        op = msaudio.DetectPitchFrequency(sample_rate=sample_rate, frame_time=frame_time, win_length=win_length,
...
def sliding_window_cmn(
    ...
    """
    if _is_numpy(specgram):
        op = msaudio.SlidingWindowCmn(cmn_window=cmn_window, min_cmn_window=min_cmn_window, center=center,
                                      norm_vars=norm_vars)
        return op(specgram)
...
def apply_codec(
    ...
    # return augmented
    raise NotImplementedError("apply_codec is not supported currently")


@_mod_utils.requires_kaldi() #TODO kaldi
...
#     return result
    raise NotImplementedError("compute_kaldi_pitch is not supported currently")
...
def rnnt_loss(
    ...
    loss = ms.ops.RNNTLoss(blank)
    costs, grads = loss(logits, targets, logit_lengths, target_lengths)
    ...
    #     clamp=clamp,
    # )
    grads = ms.ops.clamp(grads, min=clamp)
    if reduction == "mean":
        return costs.mean()
    elif reduction == "sum":
        return costs.sum()
...
def psd(
    ...
    psd = np.einsum("...ct,...et->...tce", specgram.numpy(), specgram.conj().numpy())
...
def _compute_mat_trace(input: torch.Tensor, dim1: int = -1, dim2: int = -2) -> torch.Tensor:
    ...
    input = np.diagonal(input.numpy(), 0,  axis1=dim1,  axis2=dim2)
...
def mvdr_weights_souden(
    ...
    if isinstance(reference_channel, int):
        ...
        beamform_weights = np.einsum("...c,...c->...", ws, reference_channel[..., None, None, :].numpy)
        beamform_weights = torch.tensor(beamform_weights)
    else:
...
def mvdr_weights_rtf(
    ...
    denominator = np.einsum("...d,...d->...", rtf.conj().numpy(), numerator.numpy())
    ...
    if reference_channel is not None:
        if isinstance(reference_channel, int):
            ...
            scale = np.einsum("...c,...c->...", rtf.conj(), reference_channel[..., None, :].numpy())
...
def rtf_power(
    ...
    if isinstance(reference_channel, int):
        ...
        rtf = np.einsum("...c,...c->...", phi, reference_channel[..., None, None, :].numpy())
        rtf = torch.tensor(rtf)
    else:
        raise TypeError(f"Expected 'int' or 'Tensor' for reference_channel. Found: {type(reference_channel)}.")
...
def apply_beamforming(beamform_weights: Tensor, specgram: Tensor) -> Tensor:
    ...
    specgram_enhanced = np.einsum("...fc,...cft->...ft", beamform_weights.conj().numpy(), specgram.numpy())
    return torch.tensor(specgram_enhanced)
