...
from msadapter.pytorch.nn.parameter import Parameter #TODO
import mindspore.dataset.audio as msaudio
from msadapter.torchaudio import functional as F
from msadapter.torchaudio.functional.functional import (
    _apply_sinc_resample_kernel,
    _get_sinc_resample_kernel,
    _stretch_waveform, #TODO
    _fix_waveform_shape,
)
from mindspore.dataset.audio import BorderType, FadeShape, GainType, MelType, \
    NormMode, NormType, ResampleMethod, ScaleType, WindowType
...
WINDOW_DICT = {
    ...
    "kaiser_window"  : WindowType.KAISER,
    "None"           : WindowType.HANN,
}
...
SCALE_MODE = {
    ...
    "None" : ScaleType.POWER,
}

NORM_MODE = {
    "None" : NormType.NONE,
    "slaney" : NormType.SLANEY,
}
...
RESAMPLE_MODE = {
    "sinc_interpolation" : ResampleMethod.SINC_INTERPOLATION,
    "kaiser_window" : ResampleMethod.KAISER_WINDOW,
}
...
def _is_numpy(audio) -> bool:
    return isinstance(audio, np.ndarray)
...
class Spectrogram(torch.nn.Module):
    ...
    def __init__(
        ...
        self.window_name = window_fn.__name__
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                out = self.op(waveform)
            else:
                if self.power is None:
                    power = 0
                self.op = msaudio.Spectrogram(n_fft=self.n_fft, win_length=self.win_length, hop_length=self.hop_length,
                                              pad=self.pad, window=WINDOW_DICT[self.window_name], power=power,
                                              normalized=self.normalized, center=self.center,
                                              pad_mode=PAD_MODE[self.pad_mode],
...
class InverseSpectrogram(torch.nn.Module):
    ...
    def __call__(self, spectrogram: Tensor, length: Optional[int] = None) -> Tensor:
        if _is_numpy(spectrogram):
            if spectrogram.dtype in (np.complex64, np.complex128):
                real = spectrogram.real
                imag = spectrogram.imag
                spectrogram = np.stack([real, imag], axis=-1)
            self.op = msaudio.InverseSpectrogram(length = length, n_fft=self.n_fft, win_length=self.win_length,
                                                 hop_length=self.hop_length,
                                                 pad=self.pad, window=WINDOW_DICT[self.window_name],
                                                 normalized=self.normalized, center=self.center,
                                                 pad_mode=PAD_MODE[self.pad_mode],
                                                 onesided=self.onesided)
            out = self.op(spectrogram)
            return out
        return super().__call__(spectrogram, length)
...
class GriffinLim(torch.nn.Module):
    ...
    *librosa* [:footcite:`brian_mcfee-proc-scipy-2015`], *A fast Griffin-Lim algorithm* [:footcite:`6701851`]
    ...
    def __call__(self, specgram):
        if _is_numpy(specgram):
            if self.init_ops:
                out = self.op(specgram)
            else:
                self.op = msaudio.GriffinLim(n_fft=self.n_fft, n_iter=self.n_iter,
                                             win_length=self.win_length, hop_length=self.hop_length,
                                             window_type=WINDOW_DICT[self.window_name], power=self.power,
                                             momentum=self.momentum, length=self.length,rand_init=self.rand_init)
                self.init_ops = True
                out = self.op(specgram)
            return out
        return super().__call__(specgram)
    ...
    def __call__(self, x):
        if _is_numpy(x):
            if self.init_ops:
                ...
                self.op = msaudio.AmplitudeToDB(SCALE_MODE[self.stype], ref_value=self.ref_value, amin=self.amin,
                                                top_db=self.top_db)
                self.init_ops = True
                out = self.op(x)
            return out
    ...
    def __call__(self, specgram):
        if _is_numpy(specgram):
            if self.init_ops:
                ...
                self.op = msaudio.MelScale(n_mels=self.n_mels, sample_rate=self.sample_rate, f_min=self.f_min,
                                           f_max=self.f_max, n_stft=self.n_stft, norm=NORM_MODE[str(self.norm)],
                                           mel_type=MELSCALE_MODE[self.mel_scale])
                self.init_ops = True
                out = self.op(specgram)
            return out
        return super().__call__(specgram)
...
class InverseMelScale(torch.nn.Module):
    ...
    def __init__(
        ...
        self.ms_sgdargs = {"sgd_lr":self.sgdargs["lr"], "sgd_momentum":self.sgdargs["momentum"]}
        self.n_stft = n_stft
        self.norm = norm
        self.mel_scale = mel_scale
        assert f_min <= self.f_max, "Require f_min: {} < f_max: {}".format(f_min, self.f_max)
    ...
    def __call__(self, melspec: Tensor) -> Tensor:
        if _is_numpy(melspec):
            if self.init_ops:
                out = self.op(melspec)
            else:
                self.op = msaudio.InverseMelScale(
                    n_stft=self.n_stft, n_mels=self.n_mels, sample_rate=self.sample_rate, f_min=self.f_min,
                    f_max=self.f_max, max_iter=self.max_iter, tolerance_loss=self.tolerance_loss,
                    tolerance_change=self.tolerance_change, sgdargs=self.ms_sgdargs, norm=NORM_MODE[str(self.norm)],
                    mel_type=MELSCALE_MODE[self.mel_scale]
                )
                self.init_ops = True
                out = self.op(melspec)
            return out
        return super().__call__(melspec)
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.MelSpectrogram(
                    ...
                    n_mels=self.n_mels, window=WINDOW_DICT[self.window_name], power=self.power,
                    normalized=self.normalized, center=self.center, pad_mode=PAD_MODE[self.pad_mode],
                    onesided=self.onesided, norm = NORM_MODE[str(self.norm)], mel_scale=MELSCALE_MODE[self.melscale]
                                                 )
                self.init_ops = True
                out = self.op(waveform)
            return out
...
class MFCC(torch.nn.Module):
    ...
    def __init__(
        ...
        self.ms_melkwargs = {}
        self.ms_melkwargs["n_fft"] = melkwargs.get("n_fft") or 400
        self.ms_melkwargs["win_length"] = melkwargs.get("win_length") or self.ms_melkwargs.get("n_fft")
        self.ms_melkwargs["hop_length"] = melkwargs.get("hop_length") or self.ms_melkwargs.get("win_length") // 2
        self.ms_melkwargs["f_min"] = melkwargs.get("f_min") or 0.0
        self.ms_melkwargs["f_max"] = melkwargs.get("f_max") or sample_rate // 2
        self.ms_melkwargs["pad"] = melkwargs.get("pad") or 0
        self.ms_melkwargs["n_mels"] =melkwargs.get("n_mels") or 128
        window = melkwargs.get("window")
        if window is not None:
            window_name = window.__name__
        else:
            window_name = "None"
        self.ms_melkwargs["window"] = WINDOW_DICT[window_name] or WindowType.HANN
        self.ms_melkwargs["power"] = melkwargs.get("power") or 2.0
        self.ms_melkwargs["normalized"] = melkwargs.get("normalized") or False
        self.ms_melkwargs["center"] = melkwargs.get("center") or True
        self.ms_melkwargs["pad_mode"] = PAD_MODE[str(melkwargs.get("pad_mode"))] or BorderType.REFLECT
        self.ms_melkwargs["onesided"] = melkwargs.get("onesided") or True
        self.ms_melkwargs["norm"] = NORM_MODE[str(melkwargs.get("norm"))] or NormType.NONE
        self.ms_melkwargs["mel_scale"] =MELSCALE_MODE[str(melkwargs.get("mel_scale"))] or MelType.HTK
        ...
        self.ms_norm = NORM_TYPE[str(norm)]
        self.init_ops = False
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.MFCC(
                    sample_rate=self.sample_rate, n_mfcc=self.n_mfcc, dct_type=self.dct_type, norm=self.ms_norm,
                    log_mels=self.log_mels, melkwargs=self.ms_melkwargs
                )
                self.init_ops = True
                out = self.op(waveform)
            return out
...
class LFCC(torch.nn.Module):
    ...
    def __init__(
        ...
        self.ms_melkwargs = {}
        self.ms_melkwargs["n_fft"] = speckwargs.get("n_fft") or 400
        self.ms_melkwargs["win_length"] = speckwargs.get("win_length") or self.ms_melkwargs.get("n_fft")
        self.ms_melkwargs["hop_length"] = speckwargs.get("hop_length") or self.ms_melkwargs.get("win_length") // 2
        self.ms_melkwargs["pad"] = speckwargs.get("pad") or 0
        window = speckwargs.get("window")
        if window is not None:
            window_name = window.__name__
        else:
            window_name = "None"
        self.ms_melkwargs["window"] = WINDOW_DICT[window_name] or WindowType.HANN
        self.ms_melkwargs["power"] = speckwargs.get("power") or 2.0
        self.ms_melkwargs["normalized"] = speckwargs.get("normalized") or False
        self.ms_melkwargs["center"] = speckwargs.get("center") or True
        self.ms_melkwargs["pad_mode"] = PAD_MODE[str(speckwargs.get("pad_mode"))] or BorderType.REFLECT
        self.ms_melkwargs["onesided"] = speckwargs.get("onesided") or True
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.LFCC(
                    sample_rate=self.sample_rate, n_filter=self.n_filter, n_lfcc=self.n_lfcc, f_min=self.f_min,
                    f_max=self.f_max, dct_type=self.dct_type, norm=self.ms_norm, log_lf=self.log_lf,
                    speckwargs=self.ms_melkwargs
                )
                self.init_ops = True
                out = self.op(waveform)
            return out
    ...
    def __call__(self, x):
        if _is_numpy(x):
            if self.init_ops:
                ...
                self.op = msaudio.MuLawEncoding(quantization_channels=self.quantization_channels)
                self.init_ops = True
                out = self.op(x)
            return out
        return super().__call__(x)
    ...
    def __call__(self, x):
        if _is_numpy(x):
            if self.init_ops:
                ...
                self.op = msaudio.MuLawDecoding(quantization_channels=self.quantization_channels)
                self.init_ops = True
                out = self.op(x)
            return out
        return super().__call__(x)
...
class Resample(torch.nn.Module):
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.Resample(
                    orig_freq=self.orig_freq, new_freq=self.new_freq, resample_method=RESAMPLE_MODE[self.resampling_method],
                    lowpass_filter_width=self.lowpass_filter_width, rolloff=self.rolloff, beta=self.beta
                )
                self.init_ops = True
                out = self.op(waveform)
            return out
    ...
    def forward(self, waveform: Tensor) -> Tensor:
        ...
        if not self.kernel_init:
            self.kernel, self.width = _get_sinc_resample_kernel(
                    self.orig_freq,
                    self.new_freq,
                    self.gcd,
                    self.lowpass_filter_width,
    ...
    def __call__(self, specgram):
        if _is_numpy(specgram):
            if self.init_ops:
                ...
                self.op = msaudio.ComputeDeltas( win_length=self.win_length, pad_mode=PAD_MODE[self.mode])
                self.init_ops = True
                out = self.op(specgram)
            return out
        return super().__call__(specgram)
...
class TimeStretch(torch.nn.Module):
    ...
    def __call__(self, complex_specgrams, overriding_rate = None):
        if _is_numpy(complex_specgrams):
            if complex_specgrams.dtype in (np.complex64, np.complex128):
                real = complex_specgrams.real
                imag = complex_specgrams.imag
                complex_specgrams = np.stack([real, imag], axis=-1)
            if self.init_ops:
                out = self.op(complex_specgrams)
            else:
                self.op = msaudio.TimeStretch( hop_length=self.hop_length, n_freq=self.n_freq, fixed_rate=self.fixed_rate)
                self.init_ops = True
                out = self.op(complex_specgrams)
            return out
        return super().__call__(complex_specgrams, overriding_rate)
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.Fade(
                    fade_in_len=self.fade_in_len, fade_out_len=self.fade_out_len, fade_shape=FADE_MODE[self.fade_shape]
                )
                self.init_ops = True
                out = self.op(waveform)
            return out
...
def _get_mask_start(mask_param: int, axis_length: int):
    value = np.random.rand(1) * mask_param
    min_value = np.random.rand(1) * (axis_length - value)
    mask_start = int(min_value)
    mask_end = int(int(min_value) + int(value))
    mask = mask_end - mask_start
    assert mask < mask_param
    return mask_start, mask
...
class FrequencyMasking(_AxisMasking):
    ...
    def __call__(self, specgram, mask_value: float = 0.0) -> Tensor:
        if _is_numpy(specgram):
            mask_start, mask_param = _get_mask_start(self.mask_param, specgram.shape[self.axis])
            self.op = msaudio.FrequencyMasking(iid_masks=self.iid_masks, freq_mask_param=mask_param,
                                               mask_start=mask_start, mask_value=mask_value)
            out = self.op(specgram)
            return out
        return super().__call__(specgram, mask_value)
...
class TimeMasking(_AxisMasking):
    ...
    def __call__(self, specgram, mask_value: float = 0.0) -> Tensor:
        if _is_numpy(specgram):
            mask_param = _get_mask_param(self.mask_param, self.p, specgram.shape[self.axis])
            mask_start, mask_param = _get_mask_start(mask_param, specgram.shape[self.axis])
            self.op = msaudio.TimeMasking(iid_masks=self.iid_masks, time_mask_param=mask_param,
                                          mask_start=mask_start, mask_value=mask_value)
            out = self.op(specgram)
            return out
        return super().__call__(specgram, mask_value)
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.Vol(gain=self.gain, gain_type=GAIN_MODE[self.gain_type])
                self.init_ops = True
                out = self.op(waveform)
            return out
        return super().__call__(waveform)
    ...
    def __call__(self, specgram):
        if _is_numpy(specgram):
            if self.init_ops:
                ...
                self.op = msaudio.SlidingWindowCmn(
                    cmn_window=self.cmn_window, min_cmn_window=self.min_cmn_window, center=self.center, norm_vars=self.norm_vars
                )
                self.init_ops = True
                out = self.op(specgram)
            return out
...
class Vad(torch.nn.Module):
    ...
    def __init__(
        ...
        self.ms_measure_duration = measure_duration or 2.0 / measure_freq
        self.init_ops = False
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.Vad(
                    sample_rate=self.sample_rate, trigger_level=self.trigger_level, trigger_time=self.trigger_time,
                    search_time=self.search_time, allowed_gap=self.allowed_gap, pre_trigger_time=self.pre_trigger_time,
                    boot_time=self.boot_time, noise_up_time=self.noise_up_time, noise_down_time=self.noise_down_time,
                    noise_reduction_amount=self.noise_reduction_amount, measure_freq=self.measure_freq,
                    measure_duration=self.ms_measure_duration, measure_smooth_time=self.measure_smooth_time,
                    hp_filter_freq=self.hp_filter_freq, lp_filter_freq=self.lp_filter_freq,
                    hp_lifter_freq=self.hp_lifter_freq, lp_lifter_freq=self.lp_lifter_freq
                )
                self.init_ops = True
                out = self.op(waveform)
            return out
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.SpectralCentroid(
                    sample_rate=self.sample_rate, n_fft=self.n_fft, win_length=self.win_length, hop_length=self.hop_length,
                    pad=self.pad, window=WINDOW_DICT[self.window_name]
                )
                self.init_ops = True
                out = self.op(waveform)
    ...
    def __call__(self, waveform):
        if _is_numpy(waveform):
            if self.init_ops:
                ...
                self.op = msaudio.PitchShift(
                    sample_rate=self.sample_rate, n_steps=self.n_steps, bins_per_octave=self.bins_per_octave,
                    n_fft=self.n_fft, win_length=self.win_length, hop_length=self.hop_length,
                    window=WINDOW_DICT[self.window_name]
                )
                self.init_ops = True
                out = self.op(waveform)
            return out
    ...
    def forward(self, waveform: Tensor) -> Tensor:
        ...
        if not self.kernel_init:
            if self.orig_freq != self.sample_rate:
                ...
                self.kernel = Parameter(kernel, requires_grad=True)
