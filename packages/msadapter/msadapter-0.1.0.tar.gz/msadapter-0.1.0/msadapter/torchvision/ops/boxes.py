...
def nms(boxes, scores, iou_threshold):
    boxes = cast_to_ms_tensor(boxes)
    scores_keep = cast_to_ms_tensor(scores)
    ordered_scores, order = _get_cache_prim(ms.ops.Sort)(descending=True)(scores_keep)
    ordered_boxes = boxes[order, :]
    bboxes = ms.ops.concat((ordered_boxes, ordered_scores.reshape((-1, 1))), -1)
    _, output_idx, selected_mask = _get_cache_prim(ms.ops.NMSWithMask)(iou_threshold)(bboxes)
    output_idx = cast_to_adapter_tensor(output_idx)
...
def generalized_box_iou(boxes1: Tensor, boxes2: Tensor) -> Tensor:
    ...
    lti = torch.minimum(boxes1[:, None, :2], boxes2[:, :2])
    rbi = torch.maximum(boxes1[:, None, 2:], boxes2[:, 2:])
