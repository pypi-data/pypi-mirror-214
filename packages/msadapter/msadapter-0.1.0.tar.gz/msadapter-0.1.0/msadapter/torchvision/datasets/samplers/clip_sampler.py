...
# import torch.distributed as dist TODO:dist
import mindspore.communication as dist
from msadapter.pytorch.utils.data import Sampler
from msadapter.torchvision.datasets.video_utils import VideoClips
...
class DistributedSampler(Sampler):
    ...
    def __init__(
        ...
        if num_replicas is None:
            # if not dist.is_available(): TODO:dist
            #     raise RuntimeError("Requires distributed package to be available")
            # num_replicas = dist.get_world_size() TODO:dist
            num_replicas = dist.get_group_size()
        if rank is None:
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        rnd = np.random.RandomState(self.epoch)
        # indices = rnd.choice(np.arange(len(self.dataset)), len(self.dataset), replace=False).tolist()
        if self.shuffle:
            indices = rnd.choice(np.arange(len(self.dataset)), len(self.dataset), replace=False).tolist()
        else:
