...
from msadapter.tools.utils import try_import
...
def info(
    ...
    num_frames = sox.file_info.num_samples(filepath)
    num_channels = sox.file_info.channels(filepath)
    bits_per_sample = sox.file_info.bitdepth(filepath)
    encoding = sox.file_info.encoding(filepath)
    return AudioMetaData(int(sample_rate), num_frames, num_channels, bits_per_sample, encoding)
...
def load(
    ...
    sox = try_import('sox')
    tfm = sox.Transformer()
    audio = tfm.build_array(input_filepath = filepath)
    if audio.ndim == 1:
        audio = audio[:, None]
    if num_frames == -1:
...
def save(
    ...
    if channels_first:
        src = src.T
    status = tfm.build_file(output_filepath = filepath, input_array = src, sample_rate_in = sample_rate)