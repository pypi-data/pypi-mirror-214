...
def _get_subtype_for_wav(dtype, encoding: str, bits_per_sample: int):
    if not encoding:
...
def _get_subtype(dtype, format: str, encoding: str, bits_per_sample: int):
    if format == "wav":
...
def save(
    ...
    soundfile.write(file=filepath, data=src.numpy(), samplerate=sample_rate, subtype=subtype, format=format)
