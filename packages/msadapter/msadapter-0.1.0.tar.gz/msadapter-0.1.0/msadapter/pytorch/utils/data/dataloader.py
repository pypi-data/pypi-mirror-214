...
# import msadapter.pytorch.utils.data.graph_settings

from msadapter.pytorch._utils import ExceptionWrapper
from msadapter.pytorch._six import string_classes
...
# from msadapter.pytorch.utils.data.datapipes.datapipe import _IterDataPipeSerializationWrapper, _MapDataPipeSerializationWrapper
...
#     msadapter.pytorch.utils.data.graph_settings.apply_sharding(datapipe, total_workers, global_worker_id)
...
class DataLoader(Generic[T_co]):
    ...
    def __init__(self, dataset: Dataset[T_co], batch_size: Optional[int] = 1,
        ...
        if self.pin_memory:
            warnings.warn("Don't support pin_memory now.")

        if len(pin_memory_device) != 0:
            warnings.warn("Don't support set pin_memory_device now.")
        ...
        #     else:
        #         msadapter.pytorch.utils.data.graph_settings.apply_sharding(self.dataset, ws, rank)
        ...
        if isinstance(dataset, IterableDataset):
            ...
            #         dataset = msadapter.pytorch.utils.data.graph_settings.apply_shuffle_settings(dataset, shuffle=shuffle)
        ...
        if self.num_workers > 0:
            temp_dataloaderiter = _SingleProcessDataLoaderIter(self, init_maxsize=True)
            ...
            if single_data is not None:
                ...
                del single_data
                nbytes = int((int(nbytes) >> 20) * relaxation_ratio) #This factor is used for share cache redundancy in multiprocessing Queue
...
class _BaseDataLoaderIter(object):
    def __init__(self, loader: DataLoader) -> None:
        ...
        #     self._dataset = msadapter.pytorch.utils.data.graph_settings.apply_shuffle_seed(self._dataset, shared_rng)
