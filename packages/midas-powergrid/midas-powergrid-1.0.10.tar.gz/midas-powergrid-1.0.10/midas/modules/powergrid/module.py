...
class PowergridModule(UpgradeModule):
    ...
    def __init__(self):
        ...
        self.models = {
            "Line": [
                ("loading_percent", "float", 0.0, 1.0),
                ("in_service", "bool", 0, 2),
            ],
            "Trafo": [("loading_percent", "float", 0.0, 1.0)],
            "Bus": [
                ("vm_pu", "float", 0.8, 1.2),
                ("va_degree", "float", -1.0, 1.0),
                ("p_mw", "float", 0, 1.0),
                ("q_mvar", "float", -1.0, 1.0),
            ],
            "Load": [
                ("p_mw", "float", 0, 1.0),
                ("q_mvar", "float", -1.0, 1.0),
        ...
        for entity in grid.children:
            ...
            if entity.type in ("Load", "Sgen"):
                ...
                try:
                    ...
                    try:
                        p_max = pp_grid[etype].max_p_mw.loc[int(eidx)]
                        p_min = pp_grid[etype].min_p_mw.loc[int(eidx)]
                    except Exception:
                        LOG.warning(
                            "No max_p_mw or min_p_mw found, using p_mw as p_max and set p_min to 0"
                        )
                        p_max = pp_grid[etype].p_mw.loc[int(eidx)]

                    try:
                        q_max = pp_grid[etype].max_q_mvar.loc[int(eidx)]
                        q_min = pp_grid[etype].min_q_mvar.loc[int(eidx)]
                    except Exception:
                        LOG.warning(
                            "No max_p_mw or min_p_mw found, using q_mvar as q_max and set q_min to 0"
                        )
                        q_max = pp_grid[etype].q_mvar.loc[int(eidx)]

                except Exception as err:
                    print("In PowergridModule: Caught exception:", err)
                    p_max = 0.5
                    q_max = 0.5
                ...
                self.scenario.actuators.append(
                    {
                        ...
                        "action_space": (
                            f"Box(low={p_min}, high={p_max}, shape=(1,), dtype="
                            "np.float32)"
                        ),
                    }
                )
                self.scenario.actuators.append(
                    {
                        ...
                        "action_space": (
                            f"Box(low={q_min}, high={q_max}, shape=(1,), dtype="
                            "np.float32)"
                        ),
                    }
                )
