...
# 通用回流函数
def export_back(export_type, content, suffix='', result_json=None):
    threading.Thread(target=_export_back_async, args=(export_type, content, suffix, result_json)).start()


def _export_back_async(export_type, content, suffix, result_json=None):
    key = gen_path(suffix)
...
# 以下为辅助函数
def export_back_base64(content, suffix, result_json=None):
    _export_back_async('base64', content, suffix, result_json)


def export_back_bytes(content, suffix, result_json=None):
    _export_back_async('bytes', content, suffix, result_json)


def export_back_string(content, suffix, result_json=None):
    _export_back_async('string', content, suffix, result_json)


def export_back_json(content, result_json=None):
    _export_back_async('json', content, 'json', result_json)


def export_back_file(path_to_file, result_json=None):
    suffix = os.path.splitext(path_to_file)[-1].lower()
    _export_back_async('file', path_to_file, suffix, result_json)
...
if __name__ == "__main__":
    ...
    export_back('string', 'abc', 'txt', {'back_test': 'abc'})
