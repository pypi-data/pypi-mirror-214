...
from dls_bxflow_lib.bx_logstores.bx_logstores import (
    BxLogstores,
    bx_logstores_set_default,
)

logger = logging.getLogger(__name__)


thing_type = "dls_bxflow_lib.bx_logstores.context"


class Context(ContextBase):
    """
    Asyncio context for a bx_logstore object.
    ...
    async def aenter(self):
        ...
        self.server = BxLogstores().build_object(self.specification())

        # If there is more than one logstore, the last one defined will be the default.
        bx_logstores_set_default(self.server)
