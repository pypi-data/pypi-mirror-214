...
class TestIsWhitelisted(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_is_whitelisted(self):
        ...
        service_bot.db.is_user_whitelisted = AsyncMock(return_value=True)

        result = await service_bot.is_whitelisted(ctx)
        self.assertTrue(result)

        service_bot.db.is_user_whitelisted.return_value = False

        result = await service_bot.is_whitelisted(ctx)
        self.assertFalse(result)
        ctx.send.assert_called_once_with("You are not whitelisted to use this command.")
...
class TestDatabase(unittest.TestCase):
    ...
    async def test_user_data_storage(self):
        ...
        await db.store_user_data(test_user_id, player_token, player_tag, webhook_url, clan_tag)

        user_data = await db.get_user_data(test_user_id)

        expected_data = (test_user_id, player_token, player_tag, webhook_url, clan_tag, 0)
        ...
        await db.store_user_data(test_user_id, new_player_token, new_player_tag, new_webhook_url, new_clan_tag)

        updated_user_data = await db.get_user_data(test_user_id)

        expected_updated_data = (test_user_id, new_player_token, new_player_tag, new_webhook_url, new_clan_tag, 0)
