...
from GCoCF.models import ClanInfo, WarInfo, WarStats
...
class ClanInfoTestCase(unittest.TestCase):
    ...
    def test_is_warlog_public(self):
        assert self.claninfo.is_warlog_public is True
...
class WarInfoTestCase(unittest.TestCase):
    def setUp(self):
        self.warinfo = WarInfo(
            json.load(
                open(os.path.join("data", "inWar_40.json"), "r", encoding="utf8"))
        )
        self.op_member = {
            "tag": "#2GCR2YLP8",
            "name": "captain spock",
    ...
    def test_create_war_id(self):
        self.assertEqual(
            self.warinfo.create_war_id(), "#YVL0C8UY#JC0L922Y20170602T201148.000Z"
        )
...
class WarInfoNotInWarTestCase(unittest.TestCase):
    def setUp(self):
        self.warinfo = WarInfo(
            json.load(
                open(os.path.join("data", "notInWar.json"), "r", encoding="utf8"))
        )
...
class WarStatsTestCase(unittest.TestCase):
    def setUp(self):
        warinfo = WarInfo(
            json.load(
                open(os.path.join("data", "warEnded_50.json"), "r", encoding="utf8")
            )
        )
...
class MessageFactoryTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.msg_factory = MessageFactory(None, None, self.clan_info)
        self.setlocale_en()

    @staticmethod
    def _setlocale(language):
        os.environ["LANGUAGE"] = language
        gettext.bindtextdomain(
            "messages", localedir=os.path.join(os.curdir, "locales"))
...
class WarMonitorTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.monitor.clan_info = our_claninfo
        self.monitor.update()
...
class WarMonitorInWarTestCase(WarMonitorTestCase):
    ...
    def test_is_attack_msg_sent(self):
        self.assertTrue(
            self.monitor.is_msg_sent(
                self.monitor.get_attack_id(self.clan_attack))
        )

    def test_get_attack_id(self):
        self.assertEqual(
            self.monitor.get_attack_id(
                self.clan_attack), "attack98VVJ8LV88CCLRP2JC"
        )
...
class WarMonitorFullDestructionTestCase(WarMonitorTestCase):
    def get_warinfo(self):
        return WarInfo(
            json.load(
                open(
                    os.path.join("data", "full_destruction.json"), "r", encoding="utf8"
                )
            )
        )
...
class WarMonitorOpFullDestructionTestCase(WarMonitorTestCase):
    def get_warinfo(self):
        return WarInfo(
            json.load(
                open(
                    os.path.join("data", "op_full_destruction.json"),
                    "r",
                    encoding="utf8",
                )
            )
...
class TestErrorHandling(unittest.TestCase):
    ...
    def test_error_handling(self):
        ...
        with patch(
            "GCoCF.formatters.MessageFactory.create_error_embed",
            lambda error_message, title=None: Embed(),
        ):
            try:
                self.monitor.start()
