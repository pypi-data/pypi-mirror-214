...
def sanitize_input(text: str) -> str:
    ...
    return text.replace("||", "ǁ").replace("*", "∗").replace("_", "ˍ").replace("~", "˜").replace("`", "ˋ")
...
class MessageFactory(object):
    ...
    def create_preparation_msg(self):
        ...
        embed.add_field(name="\u200b", value="\u200b", inline=True)  # Add an empty field
        ...
        #war_info_text = self.create_war_info_msg(self.warstats.get_latest_war_stats())
    ...
    def create_players_msg(self):
        ...
        # Add fields to embed
        embed.add_field(name="Home", value=sanitize_input(home_players_text), inline=True)
        embed.add_field(name="Opponent", value=sanitize_input(op_players_text), inline=True)
    ...
    def create_war_msg(self):
        ...
        embed.add_field(name=f"{sanitize_input(self.warinfo.clan_name)}", value=sanitize_input(players_embed.fields[0].value), inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)  # Add an empty field
        embed.add_field(name=f"{sanitize_input(self.warinfo.op_name)}", value=sanitize_input(players_embed.fields[1].value), inline=True)
        #embed.add_field(name="\u200b", value="\u200b", inline=False)
        #embed.add_field(name="Statistics", value="\u200b", inline=False)
        #war_info_text = self.create_war_info_msg(self.warstats.get_latest_war_stats())
        #embed.set_footer(text=war_info_text)
    ...
    def create_clan_full_destruction_msg(self, attacker, attack, war_stats):
        embed = discord.Embed(title=_("⚫️ We achieved the 100% perfect war!"), color=0x00ff00)
        last_attacks_home = self.create_war_summary_msg().fields[2].value.strip()

        embed.add_field(name=_("Last Attacks"), value=sanitize_input(last_attacks_home), inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=False)
        embed.add_field(name="Statistics", value="\u200b", inline=False)
        war_info_text = self.create_war_info_msg(self.warstats.get_latest_war_stats())
        embed.set_footer(text=war_info_text)
    ...
    def create_attack_msg(self, member, attack, war_stats, color):
        ...
        embed = discord.Embed(title=f"{sanitize_input(self.warinfo.clan_name)} vs {sanitize_input(self.warinfo.op_name)}", color=color)
        embed.add_field(name="Attacker", value=f"TH {member['townhallLevel']} MP {member['mapPosition']} {sanitize_input(member['name'])}", inline=True)
        embed.add_field(name="Defender", value=f"TH {defender['townhallLevel']} MP {defender['mapPosition']} {sanitize_input(defender['name'])}", inline=True)
...
▪ {opponent_attack_count: >{atkwidth}}/{total} ⭐ {opponent_stars: <{swidth}} ⚡ {opponent_destruction:.2f}%""")
    ...
    def create_opponent_full_destruction_msg(self, attacker, attack, war_stats):
        embed = discord.Embed(title=_("⚫️ They achieved the 100% perfect war!"), color=0xff0000)
        last_attacks_op = self.create_war_summary_msg().fields[3].value.strip()

        embed.add_field(name=_("Last Attacks"), value=sanitize_input(last_attacks_op), inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=False)
        embed.add_field(name="Statistics", value="\u200b", inline=False)
        war_info_text = self.create_war_info_msg(self.warstats.get_latest_war_stats())
        embed.set_footer(text=war_info_text)
    ...
    def create_war_over_msg(self):
        ...
        embed = discord.Embed(title=self.create_win_or_lose_title(), color=color)
        players_embed = self.create_players_msg()
    ...
    def create_war_summary_msg(self):
        ...
        embed.add_field(name="Home", value=f"{sanitize_input(clan_value)}\n", inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)  # Add an empty field
        embed.add_field(name="Opponent", value=f"{sanitize_input(op_value)}\n", inline=True)

        home_clan_tags = set(player["tag"] for player in self.warinfo.clan_members.values())
        op_clan_tags = set(player["tag"] for player in self.warinfo.opponent_members.values())

        def filter_attacks(attacks, clan_tags, defending=True):
            return [(player, attack) for player, attack in attacks if (attack["defenderTag"] in clan_tags) == defending]

        all_attacks = list(self.warinfo.ordered_attacks.values())

        last_defences_home = filter_attacks(all_attacks, home_clan_tags, defending=True)[-5:]
        last_defences_op = filter_attacks(all_attacks, op_clan_tags, defending=True)[-5:]

        last_attacks_home = filter_attacks(all_attacks, home_clan_tags, defending=False)[-5:]
        last_attacks_op = filter_attacks(all_attacks, op_clan_tags, defending=False)[-5:]
        ...
        home_activity = {
            "defences": format_attack_text(last_defences_home, home_clan_tags, self.warinfo),
            "attacks": format_attack_text(last_attacks_home, home_clan_tags, self.warinfo)
        }
        opponent_activity = {
            "defences": format_attack_text(last_defences_op, home_clan_tags, self.warinfo),
            "attacks": format_attack_text(last_attacks_op, home_clan_tags, self.warinfo)
        }

        embed.add_field(name="Last Defences", value=home_activity["defences"], inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)  # Add an empty field
        embed.add_field(name="Last Defences", value=opponent_activity["defences"], inline=True)

        embed.add_field(name="Last Attacks", value=home_activity["attacks"], inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)  # Add an empty field
        embed.add_field(name="Last Attacks", value=opponent_activity["attacks"], inline=True)
