...
DEV_API_KEY = os.getenv("COC_API_TOKEN")
...
async def start_instance_for_user(user_id: int):
    ...
    if user_data and user_data[5] == 0:
        ...
        run_command = f'docker run -d --name {container_name} ghcr.io/gillesmaster/gcocf/gcocf:latest --coc-token {key} --webhook-url {webhook_url} --clan-tag \'{clan_tag}\''
