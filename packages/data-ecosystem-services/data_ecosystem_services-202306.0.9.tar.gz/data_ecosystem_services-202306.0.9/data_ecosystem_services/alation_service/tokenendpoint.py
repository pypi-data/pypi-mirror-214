...
class TokenEndpoint:
    ...
    @staticmethod
    def create_api_access_token_via_refresh(config):

        edc_alation_base_url = config.get("edc_alation_base_url")
        az_kv_edc_refresh_secret_key = config.get("az_kv_edc_refresh_secret_key")
        az_kv_edc_env_var =  az_kv_edc_refresh_secret_key.replace("-", "_")
        edc_alation_user_Id = config.get("edc_alation_user_id")
        # Retrieve the value of the environment variable
        edc_alation_refresh_token = os.getenv(az_kv_edc_env_var)
    ...
    @classmethod
    def get_api_access_token(cls, config):

        # Get the value for an invalid key
        default_value = "Key not found"
        edc_alation_base_url = config.get("edc_alation_base_url", default_value)
        print(edc_alation_base_url)  # Output: Key not found

        az_kv_edc_refresh_secret_key = config.get("az_kv_edc_refresh_secret_key", default_value)
        print(az_kv_edc_refresh_secret_key)  # Output: Key not found

        az_kv_edc_env_var =  az_kv_edc_refresh_secret_key.replace("-", "_")
        
        print(f"az_kv_edc_env_var:{az_kv_edc_env_var}")
        # Retrieve the value of the environment variable
        # refresh_token = cls.create_alation_refresh_token(config)
        api_token = cls.create_api_access_token_via_refresh(config)
