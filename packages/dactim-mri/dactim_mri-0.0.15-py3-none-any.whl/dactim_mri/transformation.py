...
path = r"E:\Leo\data\sub-001\ses-01\anat\sub-001_ses-01_FLAIR_corrected_brain_cropped_resampled_flirt.nii.gz"
atlas = r"E:\Leo\data\sub-009\ses-01\anat\sub-009_ses-01_FLAIR_corrected_brain_cropped_resampled.nii.gz"
dactim = Dactim()

pixdim=nib.load(path).header["pixdim"][1:4]
# norm = dactim.normalize(nib.load(path).get_fdata())
norm = dactim.normalize(nib.load(path).get_fdata())
# norm = dactim.n4_bias_field_correction(path, pixdim, force=True)
# norm = dactim.normalize(path)
dactim.plot(norm, pixdim=[1,1,0.5], slice=10)
