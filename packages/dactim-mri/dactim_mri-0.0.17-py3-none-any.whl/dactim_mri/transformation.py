...
class Dactim():
    ...
    def skull_stripping(self, input_data, model_path, mask=True, force=True, suffix="brain"):
        ...
        self.output_data, self.nii_mask_data = run_hd_bet(sitk.ReadImage(input_data), model_path=model_path)
    ...
    def n4_bias_field_correction(self, input_data, pixdim=None, mask=False, force=True, suffix="corrected"):
        ...
        self.output_data = corrector.Execute(self.nii_data, mask_otsu)
        log_bias_field = corrector.GetLogBiasFieldAsImage(self.nii_data)
