...
class Graph:
    ...
    def __init__(self):
        self.operations = {}
        self._initialize_composite_namespaces()
    ...
    def _initialize_composite_namespaces(self):
        ...
        for _composite_namespace in COMPOSITE_NAMESPACE_REGISTRY:
            ...
            setattr(self, _namespace, getattr(self, _namespace)(self))
...
for composite_namespace in COMPOSITE_NAMESPACE_REGISTRY:
    assert hasattr(composite_namespace, COMPOSITE_ATTR)
    assert inspect.isclass(composite_namespace)
    assert hasattr(composite_namespace, NAMESPACE_ATTR)

    # Add composite node namespace.
    _extend_method(
        Graph,
        composite_namespace,
        getattr(composite_namespace, NAMESPACE_ATTR),
    )
