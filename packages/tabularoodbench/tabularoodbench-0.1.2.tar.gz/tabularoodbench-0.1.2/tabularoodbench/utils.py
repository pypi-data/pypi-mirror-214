...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="US Accident data preprocessing")
    parser.add_argument('--dir', type=str, help='path of us accident data')
    args = parser.parse_args()
    os.makedirs('%s/USAccident'%args.dir, exist_ok=True)
    X = preprocess(args.dir)
    
    ALL_STATES= ['CA' ,'AZ' ,'AR', 'SC' ,'FL' ,'WA' ,'NY', 'VA', 'UT', 'OR', 'MD' ,'MN', 'LA', 'TN',
                    'PA', 'NC', 'TX', 'NJ', 'MS', 'CT', 'MO', 'ND', 'CO', 'OH', 'MI', 'AL' ,'GA', 'IL',
                    'IA', 'DE', 'MT', 'ME', 'KS', 'WV', 'IN', 'NE', 'WI' ,'NV' ,'MA' ,'DC', 'OK' ,'ID',
                    'NH', 'NM', 'RI' ,'KY', 'WY' ,'VT', 'SD']
    
    for state in ALL_STATES:
        sample = X[X["State"]==state]
        sample.to_csv("%s/USAccident/%s.csv"%(args.dir, state))