...
class TokenHolder:
    def __init__(self, **kwargs: str) -> None:
        ...
        self.keycloak_openid = KeycloakOpenID(server_url=f'https://{self.domain}/auth/',
                                              client_id=self.client_id,
                                              realm_name=self.realm)

    def get_tokens_with_credentials(self, username: str, password: str) -> None:
        self.token = self.keycloak_openid.token(username, password)
        self.access_token = self.token["access_token"]
        self.refresh_token = self.token["refresh_token"]

    def _refresh_tokens(self) -> None:
        self.token = self.keycloak_openid.refresh_token(self.refresh_token)
        self.access_token = self.token["access_token"]
        self.refresh_token = self.token["refresh_token"]
