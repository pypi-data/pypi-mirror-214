...
class MigratorTable:
    ...
    def __get_source_create_table_code(self):
        query = f"""
            ...
            (
            '::text) || array_to_string(array_agg((((('    '::text || '"' || tabledefinition.column_name::text) || '"' || ' '::text) || case when tabledefinition.type ilike '%character varying%' then 'text' else tabledefinition.type end) || ' '::text) || tabledefinition.not_null), ',
            '::text)) || '
            );
            '::text AS code
