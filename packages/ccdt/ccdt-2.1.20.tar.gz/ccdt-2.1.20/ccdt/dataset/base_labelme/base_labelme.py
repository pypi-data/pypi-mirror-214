            ...
            if dataset.get('labelme_info').get('shapes'):
                ...
                for poly_index, polygon in enumerate(polygon_list):
                    ...
                    for rect_index, rectangle in enumerate(rectangle_list):
                        ...
                        #     print("判断矩形是否包含多边形")
            # 对列表进行去重
            # new_judge_shape = [dict(t) for t in {tuple(sorted(d.items())) for d in judge_shape}]
            new_judge_shape = []
            # seen = set()
            for shape in judge_shape:
                # sorted_items = sorted(shape.items())
                # sorted_shape = tuple(sorted_items)
                if shape not in new_judge_shape:
                    new_judge_shape.append(shape)
                    # new_judge_shape.append(dict(sorted_items))
    ...
    def rectangle_cross_the_border(self, bbox, dataset, automatic_correction):
        ...
        if isinstance(bbox, list):
            ...
            if x1 < 0 or y1 < 0 or x2 < 0 or y2 < 0 or \
                    ...
                    x1 < dataset.get('image_width') or y1 < dataset.get('image_height') or \
                    x2 < dataset.get('image_width') or y2 < dataset.get('image_height'):
                return True
                ...
                if len(bbox.get('points')) == 2:
                    ...
                    if x1 < 0 or y1 < 0 or x2 < 0 or y2 < 0 or \
                        ...
                        if automatic_correction:  # 自动矫正越界标注形状
                            ...
                            # is_negative_x = all(x > dataset.get('image_width') for x in [x1, x2])
                            # is_negative_y = all(y > dataset.get('image_height') for y in [y1, y2])
            ...
            if bbox.get('shape_type') == 'polygon':
                if len(bbox.get('points')) == 4:
                    ...
                    if x1 < 0 or y1 < 0 or x2 < 0 or y2 < 0 or x3 < 0 or y3 < 0 or x4 < 0 or y4 < 0 or \
                            ...
                            x4 > dataset.get('image_width') or y4 > dataset.get('image_height') or \
                            x1 < dataset.get('image_width') or y1 < dataset.get('image_height') or \
                            x2 < dataset.get('image_width') or y2 < dataset.get('image_height') or \
                            x3 < dataset.get('image_width') or y3 < dataset.get('image_height') or \
                            x4 < dataset.get('image_width') or y4 < dataset.get('image_height'):
                        if automatic_correction:  # 自动矫正越界标注形状
                            is_negative_x = all(x < 0 or x > dataset.get('image_width') for x in [x1, x2, x3, x4])
                            is_negative_y = all(y > dataset.get('image_height') for y in [y1, y2, y3, y4])
                            if is_negative_x or is_negative_y:
                                # 删除为负数的shape标注，然后重写
                                ...
                                clamp_x2 = np.clip(x2, 0, dataset.get('image_width'))
                                clamp_y2 = np.clip(y2, 0, dataset.get('image_height'))
                                clamp_x3 = np.clip(x3, 0, dataset.get('image_width'))
                                clamp_y3 = np.clip(y3, 0, dataset.get('image_height'))
                                clamp_x4 = np.clip(x4, 0, dataset.get('image_width'))
                                clamp_y4 = np.clip(y4, 0, dataset.get('image_height'))
                                # 替换
                                bbox.get('points')[0][0] = clamp_x1
                                bbox.get('points')[0][1] = clamp_y1
                                bbox.get('points')[1][0] = clamp_x2
