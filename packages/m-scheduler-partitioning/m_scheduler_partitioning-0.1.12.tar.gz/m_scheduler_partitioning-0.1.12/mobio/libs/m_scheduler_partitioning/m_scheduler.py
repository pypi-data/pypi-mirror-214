...
class MobioScheduler:
    ...
    def update_state(self):
        if self.state_in_db != self.current_state:
            result = SchedulerStateModel(url_connection=self.url_connection).set_state(self.node_id,
                                                                                       state=self.current_state)
            if result:
                print(datetime.utcnow(),
                      "result update state: {} old_state: {}, new_state: {}".format(result, self.state_in_db,
                                                                                    self.current_state))
    ...
    def __init__(
        ...
        child_watch = ChildrenWatch(
            client=self.zk_client, path=self.root_path, func=self.watch_children, send_event=True
        )
        data_watch_root = DataWatch(
            client=self.zk_client, path=self.root_path, func=self.watch_data_root, send_event=True
        )
        data_watch_child = DataWatch(
            client=self.zk_client, path=self.my_node_path, func=self.watch_data_children, send_event=True
        )
    ...
    def watch_children(self, lst_children, event=None):
        print(datetime.utcnow(), " Children are now: {}, event: {}".format(lst_children, event if event else ""))
    ...
    def watch_data_root(self, data, stat, event=None):
        print(datetime.utcnow(), " Version: %s, data: %s, event: %s" % (stat.version, data.decode("utf-8"), event if event else ""))
    ...
    def watch_data_children(self, data, stat, event=None):
        print(datetime.utcnow(),  " watch_data_children:: Version: %s, data: %s, event: %s" % (stat.version, data.decode("utf-8"), event))
        d = data.decode("utf-8")
        ...
        if d:
            ...
            raise Exception("current partitions is: {}, please check......".format(d))
