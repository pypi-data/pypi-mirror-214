...
class Message:
    ...
    def validate(self):
        if self.role != "user" and self.role != "assistant":
...
class RequestBody:
    ...
    def validate(self):
        if not isinstance(self.messages,list) or len(self.messages)%2==0 :
            raise CustomError(9528,"messages requires a list and cannot be empty and The number of dialog bars is an odd number")
