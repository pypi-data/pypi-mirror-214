...
class NWONDjangoTestSettings(BaseModel):
    ...
    celery_folder_parent: str = Field(
        default="celery",
        description="If we test celery tasks we make use of file based task queues. When run in parallel it may be necessary to have a single folder for each worker.",
    )
