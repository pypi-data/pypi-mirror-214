...
def clean_all_celery_messages(unifier: Optional[str] = None):
    for folder in __get_all_celery_message_folder(unifier):
        clean_directory(folder)


def create_celery_folder(unifier: Optional[str] = None):
    if "broker_transport_options" in settings.CELERY_SETTINGS:
        create_paths(__get_all_celery_message_folder(unifier))


def get_path_to_celery_folder(
    celery_folder: CeleryFolder, unifier: Optional[str] = None
) -> Optional[str]:
    if "broker_transport_options" in settings.CELERY_SETTINGS:
        folder = settings.CELERY_SETTINGS["broker_transport_options"][
            celery_folder.value
        ]
        return path.join(folder, unifier) if unifier else folder

    return None


def __get_all_celery_message_folder(unifier: Optional[str]) -> List[str]:
    celery_message_folders = [
        get_path_to_celery_folder(folder, unifier)
        for folder in [
